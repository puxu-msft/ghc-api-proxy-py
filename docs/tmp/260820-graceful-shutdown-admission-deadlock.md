...
## 顺带更正增量评审的一处结论（这条更正本身随后又被证伪了一半，见下节）

评审判定 `refused_requests` 在真实关停路径上**恒为 0**（11 组实测同向）。上面的实测表明并发负载下它可以非零（11 / 8 / 9）。机制不变——池化连接被关掉之后到达的字节确实不经过闸——但同时存在若干请求赶在关闭前进到了闸上并被答成 503。

这不改变把它移出 incidents 的处置：503 仍然是温和的那一端。
...
# 第四轮评审处置（2026-08-20）：探针本身

两份报告：`260820-severed-probe-review.md`（对抗评审，blocker 0 / major 1 / minor 4 / nit 4 / verified_ok 19）与 `260820-severed-measurement-audit.md`（测量复算）。

## 我又一次把一个数字说大了——第三次同类错误

**S-1（major）不是缺陷，是含义超限。** 实测：一条在途请求把排空撑开（生产形状），SIGTERM 之后 0.3 秒，10 条池化连接各发一个 `POST`，**10 个全部一无所获**，而 `severed = 0`、收尾行读作 `[ OK ]`。另一组：7 个客户端没拿到答复，计数报 3。

机制是必然的：第一档在 t=0 就把空闲连接关了，探针只能看见**关闭前那约一次事件循环迭代**里已经躺在内核缓冲中的字节。此后整个排空窗口（生产里可以几十秒）内客户端写进来的一切，得到 RST 或裸 EOF，**这个进程里没有任何东西看得见**。

这与前两次是同一个错误的第三次：`refused_requests` 被说成「唯一说明有人没被服务的那个数」→ 被证伪；`severed_connections` 被说成「这个数就是那个数」→ 又被证伪。**处置：把它明确定位成下界。** 非零 ⇒ 确实有人没被服务；零 ⇏ 没人。收尾行的 `ok` 不是「这次重启没伤到人」的证书，只是「没抓到」。docstring 与 `cli.py` 均已改写。

## 采纳并已落地

| 发现 | 问题 | 处置 |
|---|---|---|
| S-2（minor，唯一建议改代码的） | `socket.socket(fileno=-1)` 抛的是 **`ValueError` 而非 `OSError`**，接不住就会从锁里逃出 `stop_admitting()` → 逃出 `serve()`，于是 `_descend()` 与 `_finalize()` 一次都不跑：没有 lifespan shutdown、没有资源释放、没有收尾报告。与 `lifecycle.md` 第 22 行直接冲突。评审实测两条候选可达路径均被上游清理关上，故判潜伏 | 改用 `raw_socket.dup()` + `finally: probe.close()`。拿到的是自己的 fd——忘了关只是泄漏一个 fd，而借来的 fd 关错了是**关掉别人的活连接**；且 fd 为 -1 时 `dup` 抛的正是已被接住的 `OSError` |
| S-1（major） | 见上 | 定位改为下界；`ok` 不再被表述为证书 |
| S-3（minor） | 「两条限制方向都是少报」**不成立**：TLS 下等待的字节可能是重协商或 session ticket 而非请求，那是**多报**（已实测）；而且限制不止两条 | docstring 改为三条并逐条标明方向 |
| 审计附一 | 「全量 1464 是在脏树上量的、不可复现」**归因错误**。干净 checkout 是 **1463 / 3**；补上未跟踪的 `docs/.human-controlled/config.example.yaml` 即精确复现 **1464 / 2**——差的那条是 `test_authoritative_example_config_parses` 的 skipif | 见下节「测量口径」。真正该记的是：**这个套件的通过数依赖一个不在版本控制里的输入** |
| 审计第 5 条 | 我对第三轮评审的更正**两半都被证伪**：单客户端下不是恒为 0（23 次 gap=0 运行里 3 次 `refused=1`，约 13%）；并发下也不是稳定非零（15 次运行 `refused` 全 0 而 `severed` 非零） | 正文已就地改软；正确表述是**两者都是概率性的，此消彼长**——同一批字节，被读走的进 `refused`，没被读走的进 `severed` |

## 测量口径（供下一个人对账）

- **干净 checkout = 1463 passed / 3 skipped**；补上未跟踪的 `docs/.human-controlled/config.example.yaml` = **1464 / 2**。
- 我在正文更早处写的 1415 / 1424 / 1425 / 1455 / 1464 都是在主工作树上量的。主工作树同时混有并行会话的在飞改动**和**这个未跟踪文件，两个因素都在动这个数。
- 命中率：我报的「3 次命中 2 次」是**保守说法**，审计在合适的写入节奏下量到 90%–100%。
- 窗口宽度：tick 计数器实测 **5/5 次恰为一次事件循环迭代**，可以说得比「约」更死。
- 正样本测试稳定性：审计扩展到 **60 次连跑全绿**（空闲 20 + 负载 20 + load average 40 的重负载 20）。

## 记录但不改

- **S-4（minor）：另外三处关连接的地方同样在丢字节。** `interrupt_connections()`（第二档）不必补——走到那一档必有 incidents，判定不会改变。`shutdown_lifespan()`（收尾）是真缺口，且**可能让本该 `fail` 的关停读作 `ok`**。`both` 模式路由器的 `_pending_sockets` 就发生在 `stop_admitting()` 的前一句、是同一时刻同一种伤害，而且最好补（socket 就在手上，`_peek_first_byte` 已证明这套写法可行）。**不补的理由**：S-1 已经确立这个数字**结构上不可能完备**，加探测点只是把下界抬高一点、代价是更多耦合点；与其让它看起来更像总数，不如让它明确是下界。若要收紧，是用户的取舍。
- **S-5（minor）：让 fd 借用安全的两条性质没有守卫。** 去掉 `MSG_PEEK`（探针真的消费字节）与去掉 `detach`，51 条测试都全绿——因为被探测的连接下一句就被关掉，两条性质在今天的接线下都不承重。改用 `dup()` 之后，其中最阴险的一条（误关别人的 fd）已经不复存在；剩下「不消费」这条仍无守卫，且**结构上无法守**：每条被探测的连接随即被关，观察不到「字节仍被交付」。
