...
**状态：候选，未落笔。** 取代 r2（`260820-spec-revision-candidate-r2.md`）。复评见 `260820-review-spec-revision-candidate-r2.md`：r1 八条中 5 条生效、1 条未生效、2 条改法引入新问题，另有新 blocker 2、新 major 2。
...
因此 carrier 里能放的只有「我是第几个」（`i`），放不了「一共几个」（`n`）。这直接决定了位置信息**能验出什么、验不出什么**：
...
| 丢掉开头若干块 | **能**（序号不从 0 起） |
| 丢掉中间块 | **能**（序号有洞）——但见下方歧义 |
| **丢掉末尾若干块** | **不能**。`[0,1,2]` 丢掉末两块后是 `[0]`，与本来就只有一块无法区分 |
| **把末尾若干块合并成一块** | **不能**，同上 |
| 丢掉 summary-only 块 | **不能**（那类块按冻结裁决发 bare marker，不带 `i`），且它造成的洞与「中间块被丢」无法区分 |

**所以位置校验是尽力而为的，不是完备的。** 这是块级交付的直接代价，不是可以靠补测试或补字段消解的东西——要完备就得知道总数，要知道总数就得等整个响应，那正是你否掉的做法。
...
`tag` 恒为 `openai.responses.reasoning.encrypted_content`；`i` 为非负整数，是该 item 在**同一 assistant turn** 内 reasoning item 中的 0 基序号。

**bare marker 保持 v1 语义不变**——`spec.md:221`、`:325`、`acceptance.md:141-144` 与独立裁决 `260807-arbitrate-empty-reasoning.md` 冻结了「absent／empty `encrypted_content` 恰好产生一个 bare marker block、consumer 不得伪造 `encrypted_content`」。v2 不改这一条。**代价已在上文能力表中列明**：summary-only item 因此不带 `i`。

### 检出后的 wire 行为（新 M2）

恢复出的 `i` 序列出现非升序、重复或空洞时，归入 **`DEGRADE`**：

> 按客户端给出的 block 源序原样重建 reasoning item（不依 `i` 重排、不丢弃、不使请求失败），并记录一条 conversion fact 标明观察到的序列与异常类型。客户端可观察结果与正常路径一致，差别只在 conversion fact。

理由：`i` 的检出能力本就不完备（见上表），据一个不完备的信号丢弃内容或拒绝请求，风险高于它能挡住的问题。**不用 `i` 主动重排**——上游按源序接受，Anthropic 自身规则又要求 thinking 领起回合，保持源序已经是对的形状。
...
- **`spec.md:204`** → 「resolved model 与 upstream identity 只能作为内部 typed facts 保存，不得塞进 carrier。carrier 可以承载 **assistant turn 内的位置序号**（`i`），用于回送时尽力校验 item 是否被移位、重复或前缀丢失；除此之外不得承载任何 item 标识，尤其不得承载上游 item `id`。」
...
- **新增 `i` 判据**：完整升序序列通过；非升序、重复、前缀缺失各产生一条 conversion fact 且 wire 输出保持源序（`DEGRADE`）。**并显式断言尾部丢失不产生 fact**——把能力边界钉进 oracle，避免后人误以为漏检是缺陷。
