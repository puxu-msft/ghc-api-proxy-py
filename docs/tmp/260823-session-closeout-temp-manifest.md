...
**枚举**（2026-08-23 06:33 重新枚举，两法一致）：

| 方法 | 结果 |
|---|---|
| `find "$R" \( -type f -o -type l \)` | **66** |
| `fd -H -I --type f --type l . "$R"` | **66** |
| `fd -H --type f --type l . "$R"`（对照，不加 `-I`） | 63 |

那 3 个差额是 `dist/` 下被 `.gitignore` 覆盖的项——**本次实测复现了「`fd` 不加 `-I` 会少报，且少报的正是没人评审过的那几行」这条已知陷阱**。顶层条目 33 个，总体积 1.0M。清单由 `find` 产出（下表所有 66 个路径逐一出现，计数可机械核对）。
...
`msg3.txt`、`msg4.txt`、`msg5a.txt`、`msg5b.txt`、`msg6.txt`、`dotdev-msg1.txt`

**长期价值：无，解除条件已满足。** 技能规定提交信息文件在「命名的提交存在且带着那条信息」之前不自动可弃。逐一对应并已核对：`28c1a7a`（`msg3.txt`）、`b973ed0`（`msg4.txt`）、`c01191f`（`msg5a.txt`）、`ef4defb`（`msg5b.txt`）、`2248a69`（`msg6.txt`）、`.dev` 的 `44d9859`（`dotdev-msg1.txt`）。
...
**长期价值：无，且已经是错的。** 它是会话中途从 transcript 收割的 17 条提交回执，但其中 **`main 56da22a` 已被同伴 rebase 成 `2248a69`**，**`main 002d0db` 是那个只带了 14 个文件里 3 个、后被 amend 掉的破损提交**。

**替代证据**：`.dev/docs/server-layout/status.md` 的六步表格，SHA 是 rebase 之后重新核对过的当前值，且该文件开头就写明了「同伴在此期间 rebase 过 `main`，早先文档里引用的部分 SHA 已失效」。

**这一行本身就是「陈旧回执比没有回执更危险」的实例**，因为它的形状（一列整齐的 `分支 SHA`）读起来完全像是可信的。
...
33 + 5 + 11 + 6 + 3 + 6 + 2 + 1 = **67**

比枚举出的 66 多 1，因为 `dotdev-msg1.txt` 是**枚举之后**（06:36 提交 `.dev` 时）新建的。技能要求「关闭处置前立刻重列根目录，任何新路径回到分类而不是被扫进计数」——这一个正是那样被接住并分类的（组 6）。**关闭处置前需再重列一次**，届时若又有新增，同样回到分类。
