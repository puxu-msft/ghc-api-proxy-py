# 候选 r5：`spec.md` / `acceptance.md` 修订（carrier v2 ＋ 等待式准入）

**状态：候选，未落笔。** 取代 r4（`260820-spec-revision-candidate-r4.md`）。r4 复评见 `260820-review-spec-revision-candidate-r4.md`：blocker 0、major 3，全部采纳。三条都是**同一类问题**——v2 只改到了矩阵，没有传播到冻结的详细识别算法、malformed 分类与 encrypted-only 行；外加一条我自己的过度概括。

## r4 复评处置表

| 发现 | 处置 | 级别 | 说明 |
|---|---|---|---|
| M1 详细识别顺序仍会把 v2 判成 `project_unknown_version` | **采纳** | A | 已核 `spec.md:238-242` 第 2 步确实把本 namespace 下其他 version 一律归为 unknown，而 unknown 按 `:164` 是**整个 block 不进 wire**——不改这里，v2 carrier 会被直接丢弃。`:246` 也只定义了 `project_malformed_v1` |
| M2 `spec.md:183` encrypted-only 行仍强制 v1 producer | **采纳** | A | 已核 `:183` 是独立冻结的 no-loss 合法形态，不能被 `:182` 的改写隐含覆盖 |
| M3 「只有尾部不可检」过强 | **采纳** | C | 评审对。detector 只比对 payload carrier，因此**任何只发生在 bare marker 之间的破坏都不可检**，与位置无关。我那十行脚本只证明了我想到的十行——这正是它不足以支撑全称结论的原因 |
...
| M2 bare marker 是否占 `i` 序位未定义 | **采纳，并更正能力表** | C | 评审对，而且这一条**推翻了我自己写的一句悲观结论**：冻结校验算法后，中途 bare marker 被丢**是可检出的**（该行的「只有尾部不可检」结论已被 r4 复评 M3 进一步收窄，见本稿顶部） |
...
**所以边界是「detector 只看得见 payload carrier」**，由此推出两类不可检：

1. **任何尾部丢失或合并**——尾部没有后继可比。
2. **任何只发生在 bare marker 之间的破坏**，与位置无关。交换两个 bare、或用一个 bare 等长替换另一个，都不改变任何可比的 `i`：`[bareA, bareB, p2]` 与交换后的 `[bareB, bareA, p2]` 在算法看来都是 `[None, None, 2]`，都不触发。

其余（乱序、重复、前缀丢失、中途空洞，无论丢的是 payload 还是 bare）都能检出。

> **一条方法论上的教训，值得写下来。** r4 我跑了十行脚本、全绿，就据此写下「只有尾部不可检」的全称结论。但那十行**全都是我自己想得到的情形**——脚本证明的是「我列举的十行与算法一致」，不是「算法的边界就是这十行」。上面第 2 类正是评审找出来、而我没想到的。反例现已并入 `exp/carrier-v2/check_i_algorithm.py`（12 行），但脚本的作用仍是**证伪已列举的断言**，不是给出全称保证。
...
**识别顺序**：项目 v2 → 项目 v1 → 项目未知 version → upstream v1 → upstream legacy bare → foreign。完整六步替换文本见下文「详细识别顺序与 malformed 分类」——**只改摘要不改 `spec.md:238-242` 会让 v2 被判成 `project_unknown_version` 并整块丢弃**。**v1 永久保留可读**，否则客户端手里的历史会话全部失效。
...
理由：`i` 的检出能力本就不完备（尾部损失与 bare-only 破坏都检不出，见头号问题一节），据一个不完备的信号丢弃内容或拒绝请求，风险高于它能挡住的问题。**不用 `i` 主动重排**——上游按源序接受，Anthropic 自身规则又要求 thinking 领起回合，保持源序已经是对的形状。
...
**response 矩阵 `:183` 调整（r4 复评 M2）**：encrypted-only 是**独立冻结的 no-loss 合法形态**，不能被 `:182` 的改写隐含覆盖，必须单独改：「非空 opaque payload 形成空 visible thinking 加本项目主 **v1** carrier 的合法 thinking block」→ 改为 **v2** payload carrier（因其 `encrypted_content` 非空，故是 payload 形态、携带 `i`）。**空 visible thinking、value-exact opaque、不得丢失 payload、显式 strip 另行定义——四项语义全部不变。**

### 详细识别顺序与 malformed 分类（r4 复评 M1）

这是最容易漏、后果也最重的一处：`spec.md:238-242` 是**冻结的详细识别算法**，其第 2 步把本项目 namespace 下「其他 version」一律归为 `project_unknown_version`，而 unknown 按 `:164` 是 `DEGRADE` 且**整个 thinking block 不进入 Responses wire**。只改矩阵不改这里，**v2 carrier 会被自己的实现整块丢弃**。

`spec.md:238-242` 五步替换为六步：

> 1. 精确识别本项目主 **v2** payload prefix 或 v2 bare marker；payload form 进入项目 v2 decode／schema gate。
> 2. 精确识别本项目主 v1 payload prefix 或 v1 bare marker；payload form 进入项目 v1 decode／schema gate。
> 3. 识别 `ghc-api-proxy:synthetic-reasoning:` namespace 下的其他 version 为 `project_unknown_version`；不得 fallback 到 upstream 或 foreign decoder。
> 4. 识别 `copilot-api:synthetic-reasoning:v1:` upstream payload／bare prefix；payload form 只保证合法 canonical 主路径兼容。
> 5. 精确识别 upstream legacy bare sentinel `copilot-api:synthetic-reasoning:v1`。
> 6. 其余 signature 分类为 foreign；原生 `redacted_thinking` 继续走自己的既有合同。

`spec.md:246` 增加 `project_malformed_v2` 的对称定义：

> 项目 v2 payload 必须是非空、无 `=` padding、只含 RFC 4648 URL-safe alphabet 的 canonical base64url；decode 后再以同算法 encode 必须等于原 payload。该 gate、严格 UTF-8、JSON object、**v2 唯一 key 集合 `{tag, encrypted_content, i}`**、duplicate key、`tag` 常量、`encrypted_content` 非空字符串、**`i` 为非负整数**任一校验失败时，分类为 `project_malformed_v2`，止血行为与 `project_malformed_v1` 一致（记录稳定分类，整个 thinking block 不进入 Responses wire，不恢复 visible summary 或 `encrypted_content`）。

**注意 `project_malformed_v2` 与「位置校验失败」是两件不同的事**，止血也不同：前者是**单个 carrier 自身**解不开，整块丢弃；后者是**跨 block 的序号**对不上，内容照发、只记 fact。矩阵与 acceptance 都必须分别列出，不能合并。
...
- **新增识别顺序 oracle**：按冻结的六步顺序依次喂 v2 payload／v2 bare／v1 payload／v1 bare／项目未知 version／upstream v1 payload／upstream legacy bare sentinel／foreign，逐一断言分类与恢复结果；**特别断言 v2 不被判成 `project_unknown_version`**——这是不改 `spec.md:238-242` 时会出现的确切失效。
- **新增 `project_malformed_v2` oracle**：非 canonical base64url、非 UTF-8、非 JSON object、key 集合多一个／少一个、duplicate key、`tag` 不符、`encrypted_content` 为空、`i` 为负数或非整数，各产生 `project_malformed_v2` 且整个 thinking block 不进 wire。**与位置校验失败分开断言**——后者内容照发、只记 fact，两者止血不同，合并会让其中一条失去分辨力。
...
- **新增 `i` 判据**，每条都要有正反样本，且**必须含 bare marker 参与序号的样本**（这是 r3 复评 M2 指出会误报的地方）：
  ...
  - **显式断言不产生 fact**（把能力边界钉进 oracle，避免后人当缺陷去修）：`[p(i=0)]` 由 `[p(i=0), p(i=1), p(i=2)]` 丢掉尾部两块而来；`[p(i=0)]` 由 `[p(i=0), bare]` 丢掉尾部 bare 而来；**`[bareB, bareA, p(i=2)]` 由 `[bareA, bareB, p(i=2)]` 交换两个 bare 而来**（r4 复评 M3 找出的第二类不可检，位置不在尾部，必须与尾部两条一并钉进 oracle）。
