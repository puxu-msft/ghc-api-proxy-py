...
| `anthropic-messages` ↔ `anthropic-messages` | **同形缺陷，而且是 Claude 模型的关键可达 route，不是项目定义的 primary product path。**[`260822-round2-disposition.md`](../anthropic-direct-request-shape/history/260822-round2-disposition.md) 记着实测：`claude-sonnet-5` 不支持 Responses API（`unsupported_api_for_model`），该模型只能走直连；项目 primary product path 仍是 Anthropic Messages inbound → OpenAI Responses upstream。这条腿今天走 `AnthropicAssembler` ＋ `AnthropicFramer` 的往返，未知 block 类型同样被 framer 拒绝。**落地前须先解决 §2.7 的整形问题** | 词汇已实现并单测（`anthropic_messages_passthrough.py`），**接线待 §2.8 的 continuation 问题闭合** |
| `openai-chat-completions` ↔ `openai-chat-completions` | **成功 wire 仍是 terminal-only one-shot。**这条客户端腿没有 framer，最终选中 attempt 的原始 SSE 整轮缓冲后一次交付；2026-08-22 推迟的是按推断 block 边界增量交付与 Chat continuation，不是提交前 transparent replay。当前已有 `ChatCompletionsAssembler` 能读取 `choices[].delta`、`finish_reason`、`[DONE]` 与流内 error，因此“本项目没有 reader”已是过期事实 | §5.4 现定义 SSE-buffered transaction：必须见 `[DONE]` 才成功，semantic commit 前复用统一 retry ledger，最终成功 attempt 的 bytes 不改写。当前 `one_shot_delivery` 尚未实现 replay／keepalive／cap，属于实现偏离；Chat block delivery 与 continuation 仍按既有裁决推迟 |
...
### 5.4 Chat Completions 的 terminal-only buffered transaction

本节覆盖两种把OpenAI Chat Completions SSE缓冲成一个完整交付单位的路径：direct Chat客户端请求 `stream:true`、代理把upstream SSE整轮缓冲后一次交付；以及客户端请求 `stream:false`、resolved model endpoint的capability不支持non-stream response，pipeline因此请求upstream streaming并聚合成标准Chat JSON。前者在HTTP 200 headers提交后由delivery owner replay，后者在attempt success事件发布前由direct driver owner重试；两者共享request的同一个 `RetryLedger`、同一Chat event reader与下列完整性合同，但**一个失败只能由一个owner消费ledger并重开**。

**Provider只声明能力，不执行内容适配。** Resolved model endpoint以immutable capability数据声明response modes与streaming request defaults。Pipeline解释capability、准备最终model-protocol payload并按需执行SSE→JSON adaptation；provider client只负责URL、认证、provider HTTP headers／签名、原样发送与transport/status exception normalization，不得改写Chat payload、强制mode、解析SSE或聚合JSON。Replay复用第一次routing选中的同一capability snapshot，不在attempt之间重查provider。当前CodeBuddy的streaming-only声明是参考实现兼容假设，P6 `stream:false` control尚未运行，**不得描述成真实upstream测量**；用户于2026-09-06决定本次不实测并保守维持该capability。

**一个SSE-buffered attempt只有见到第一条合法 `data: [DONE]` 才取得成功语义。** 非空 `choices[].finish_reason` 是要保留的原生终止事实，但单独不足以证明transport已完整收口；其后clean EOF或tear且没有 `[DONE]` 仍按无终局截断处理。Native non-stream JSON不是SSE transaction，不适用 `[DONE]` 判据。

Collector必须保留raw SSE frame的起止offset，而不是只保留失去边界的 `SseEvent`。第一条 `[DONE]` 到达后冻结全部semantic state；后续JSON、第二条 `[DONE]` 或error event不再改变success、finish reason、usage或observation，只作为raw tail保留。Collector继续读到EOF以保留尾随bytes，但tail collection不得反转已取得的成功语义。

这条腿的semantic commit frontier只有最终选中attempt的raw body被客户端send接受。HTTP 200 headers与SSE comment keepalive均不提交attempt；keepalive可以跨replay出现。透明replay成功时，旧attempt的raw buffer、choice／tool／reasoning／usage facts、failure与内存计量在replacement response成功建立后一起丢弃；在此之前旧attempt保留为最终carrier候选。最终成功streaming body必须逐字来自一个attempt，不得拼接两轮或由reader重写。

`done_seen == false` 时的ending动作如下：

| Ending | Replay与最终动作 |
|---|---|
| Transport tear、upstream idle timeout、attempt deadline、clean EOF | 归 `NETWORK`，在non-draining、client deadline未到且ledger有预算时transparent replay |
| 明确Chat error carrier归一化为 `SERVER_ERROR` | 允许transparent replay |
| 明确Chat error carrier归一化为rate limit | 进入既有provider rate limiter；无可信 `Retry-After` 时沿用其默认间隔，并使用现有server-error预算 |
| 明确但unknown／malformed／冲突的Chat error carrier | 不retry。Direct streaming最终候选只提交截至该error frame结束offset的raw SSE且不补 `[DONE]`；streaming-upstream→non-stream adapter按error-envelope §8写OpenAI JSON failure |
| Client deadline | 不retry。Direct streaming按§8交付当前候选partial后裸断并记fail；non-stream adaptation尚未提交client headers，返回504 |
| Client cancellation | 不retry、不再写client body，取消upstream并清理资源 |
| Draining | 在消费ledger前拒绝replay，按当前候选的最终carrier结束 |
| Buffer cap | 本侧保护，不retry；direct streaming交付不超过cap的当前partial后裸断，non-stream adaptation返回proxy error |
| 无法投影成标准Chat JSON的content event | Direct raw streaming只记issue并保留raw；non-stream adaptation以具名 `unassemblable` upstream failure结束且不retry，不得丢内容后合成成功 |
| 本地reader／collector bug | 不retry，不归因给upstream；按当前腿可用的proxy failure carrier结束并保留原异常 |

明确Chat error carrier先于code taxonomy判定。以下任一形状成立即为error carrier：SSE `event` 名为 `error`；JSON object含顶层 `error` 键，无论是否同时含 `choices`；或flat JSON object的 `type == "error"`。Nested carrier从 `error` mapping读取 `code`／`type`，flat carrier从顶层读取；非mapping值仍是明确但malformed的error carrier。把所有存在且非空的 `code`／`type` 值分别归类：只有它们全部落在同一已知瞬时类时才retry；`server_error`归 `SERVER_ERROR`，`rate_limited`／`rate_limit_error`／`rate_limit_exceeded`／`upstream_rate_limited`归rate limit；冲突、unknown或缺值一律不retry。该顺序保证unknown error不会在EOF时退化成“缺 `[DONE]` 的network truncation”。

`done_seen == true` 后的ending优先级如下：

| Tail ending | 动作 |
|---|---|
| Clean EOF | 提交全部已收raw body，request成功 |
| Transport tear、upstream idle timeout、attempt deadline | 不retry；提交已收raw body并把具体tail ending记入旁路observation，request成功 |
| Client deadline | 立即停止tail collection，提交已收raw body并记录 `tail-drain client deadline`，request成功 |
| Buffer cap | 在会越界的raw frame／chunk前停止；若同一输入片段同时包含首个 `[DONE]` 与越界tail，至少保留至该完整 `[DONE]` frame且不保留越界tail；提交已持有body并记录tail cap，request成功 |
| 后续JSON、error或第二个 `[DONE]` | 不再解释，按raw tail保留；不能反转success或污染frozen observation |
| 本地tail collector／accounting failure | 若截至首个 `[DONE]` 的buffer仍完整，则提交已持有body并把本侧tail failure保留在operator observation；不retry、不冒充upstream failure |
| Client cancellation／downstream write failure | 没有可写通道；取消upstream并退出，不为已经离开的读者补写body |

Known transient stream error须用带明确 `RetryReason` 与原始error facts的typed pipeline failure承载，不得为了复用HTTP分类而伪造一个并未收到的500 status。Unknown exception仍在closed set外并abort。Provider rate limiter需要独立的event-rate-limit输入；buffered transaction的HTTP 200 headers不得先记作limiter success，success observation延后到body verdict，event rate limit使用默认interval并让下一attempt的常规 `acquire()`等待。

**Retry owner分开而collector共享。** `BufferedAttemptCollector`只读取一个attempt、返回raw bytes／facts／ending并关闭source，绝不消费ledger或调用replacement。Non-stream mode adaptation在response headers返回之后、`attempt.succeeded`之前、whole-attempt deadline以内运行；collector的retry verdict抛给唯一的 `DirectDriver` owner，driver用该attempt已冻结的final payload、admission与capability snapshot开启下一attempt，不重新发布mutable preparation。Post-header direct streaming只有delivery transaction runner能消费ledger，并通过现有prepared replay seam重开。任何组件不得先花预算，再把同一failure交给另一个retry owner。`response_header_timeout`不得扩大到body；provider内部不得另开retry loop。

Attempt-local facts与request-level projection分开。Replacement开始不销毁旧state；replacement response成功建立时才原子切换candidate并作废旧state。Replacement在headers前最终失败、旧buffer成为final fallback时，发布旧snapshot并另记replacement failure；最终action选定之前不得仅靠 `context.current_attempt` 决定哪个observer代表客户端实际收到的candidate。

Replay不发生、draining拒绝、预算耗尽或replacement未建立时，direct streaming Chat使用 [`../error-envelope/spec.md`](../error-envelope/spec.md) §10.2 的最终carrier；该carrier不授权提前跳过transparent replay。Streaming-upstream→non-stream adaptation尚未向客户端提交headers，按OpenAI JSON error carrier返回。Chat block-level incremental delivery与continuation仍不在本节定义域。

完整设计、决策来源与被否决方案见 [`../direct-buffered-chat-completions/design.md`](../direct-buffered-chat-completions/design.md) 与 [`../direct-buffered-chat-completions/decisions.md`](../direct-buffered-chat-completions/decisions.md)。
...
Chat terminal-only transaction同样受当前持有量约束：raw buffer、协议state及同时保留的预渲染／observation副本都计入；collector保留raw frame offset，使cap可在完整frame边界决定保留范围。这里的「字节」沿用`DeliveryUnit.size_bytes`的既有口径——统计每份同时存活representation的逻辑编码大小，不声称等于CPython allocator的heap footprint；容器头、引用与encoder的有界临时栈不计，随payload规模增长的完整working copy必须计。`[DONE]`前会越界时按本代理保护失败，不进入upstream retry；`[DONE]`后的tail会越界时停止tail collection并提交已含完整 `[DONE]` frame、且不超过cap的body，记tail-cap observation。若 `[DONE]` frame本身使持有量越界，则仍属于pre-terminal cap failure，不能越过cap只为制造成功。Direct streaming尚有可写连接时，pre-terminal cap按error-envelope §10.2交付不超过cap的当前partial后裸断；non-stream adaptation尚未提交client headers，按OpenAI proxy error carrier返回。
...
### 9.3 Chat endpoint capability 与 non-stream mode adaptation

Chat request先读resolved model endpoint的immutable capability。Capability由provider/catalog声明、route快照携带、pipeline解释；provider client只执行pipeline给出的最终payload与mode。`response_modes` 是非空集合，成员只能是 `streaming`／`non_streaming`；空集或其它值使descriptor在构造／目录装载时失败，不能等到请求期猜。

Client contract与upstream transport mode必须分槽，`RequestContext.stream`只表示client contract，attempt-local `ChatSendPlan.upstream_stream`表示实际发送mode，`Attempt.payload`保存真正发送的最终payload。Pipeline必须同时把 `Attempt.payload["stream"]` 写成 `ChatSendPlan.upstream_stream`，并把同一个bool传给 `ModelProvider.send(stream=...)`；JSON body与transport mode不得各说一个答案：

| Client mode | Endpoint capability | Upstream mode与动作 |
|---|---|---|
| non-stream | 支持 `non_streaming` | `upstream_stream=false`，不做SSE adaptation，成功response按§9既有JSON value合同处理 |
| non-stream | 不支持 `non_streaming`、支持 `streaming` | `upstream_stream=true`，在shared attempt内按§5.4收集并聚合；客户端仍走non-stream JSON分支 |
| streaming | 支持 `streaming` | `upstream_stream=true`，client端走§5.4 terminal-only raw transaction |
| streaming | 不支持 `streaming`、支持 `non_streaming` | 联网前以typed `ResponseModeNotSupported`拒绝；本项目没有JSON→SSE synthesizer，不把non-stream body伪装成stream。该错误按error-envelope §5.1映射为OpenAI `CLIENT`／400／`unsupported_response_mode` |

Streaming request defaults与mode support是两类字段。`stream_options_include_usage_default`／`tool_stream_default` 的值为 `true` 或 `None`；`true`只表示upstream streaming mode下“字段缺席时补true”，**不是required**。显式client值（含 `false` 与非mapping `stream_options`）原样保留；non-stream upstream mode不应用这两个defaults。当前CodeBuddy capability为 `{streaming}` 且 `stream_options_include_usage_default=true`，这是未运行P6情况下的保守compatibility声明，不得写成已测upstream事实。Xingchen为 `{streaming, non_streaming}` 且两个streaming defaults为true。GitHub Copilot为 `{streaming, non_streaming}` 且两个defaults均为 `None`。Capability必须显式填值与provenance，不能用缺席同时表示“未知”和“支持两种mode”。

#### 9.3.1 SSE→`chat.completion` 聚合合同

用户于2026-09-06裁定采用标准多choice聚合。OpenAI SDK公开的 `ChatCompletionStreamState` 可作为成熟实现基础，但本表才是行为权威；升级SDK不得静默改变输出。

| 输入事实 | 最终non-stream JSON |
|---|---|
| `id`、`created`、`model` | 取第一个合法值；后续缺席不清空，重复相同值允许，冲突值使adaptation `unassemblable`。三项在 `[DONE]` 时仍缺任一项也为 `unassemblable`，不得由代理新造 |
| `object` | 输入若出现必须为 `chat.completion.chunk`；输出固定为 `chat.completion`。其它非空值为 `unassemblable` |
| `service_tier`、`system_fingerprint`、`moderation` | 这三个字段构成当前封闭的last-explicit-wins集合：取最后一个显式出现的JSON值，字段缺席不覆盖已观察值。新增成员必须先修本表，不得由SDK类型自动扩大 |
| `choices[]` | 以合法非负整数 `choice.index` 为key独立累计，最终按index升序输出；同一event内重复index记issue并按event内顺序累计。缺失、bool、负数或非整数index使adaptation `unassemblable` |
| `delta.role` | 每个choice取第一个非null值，必须是 `assistant`；冲突或其它值为 `unassemblable`。最终缺席时按Chat completion响应合同写 `assistant`，并记 `role_synthesized` issue |
| `delta.content`、`delta.refusal`、`delta.reasoning_content` | 各自在同一choice内按event顺序连接字符串片段；只有null／缺席且无片段时，`content`／`refusal`输出null，`reasoning_content`保持缺席；非字符串非null为 `unassemblable` |
| legacy `delta.function_call` | `name`取第一个非空字符串且冲突失败，`arguments`按event顺序连接；最终保留为message `function_call` |
| `delta.tool_calls[]` | 以 `(choice.index, tool.index)` 为key累计，tool index必须是合法非负整数；最终每个choice内按tool index升序并去掉stream-only `index`。`id`／`type`／`function.name`取第一个非空值且冲突为 `unassemblable`，`function.arguments`按event顺序连接；重复、无名调用均保留，不合并不同index |
| `choice.logprobs` | `content`与`refusal`各自保留absent／explicit-null／array三态：只见null时最终仍写null；见一个或多个array时按event顺序连接并输出array，之后的null不清空已观察array；非null非array为 `unassemblable` |
| `choice.finish_reason` | 每个choice取第一个非null值；后续相同值允许，冲突为 `unassemblable`。`[DONE]`到达时每个已观察choice都必须已有合法字符串finish reason，否则不能生成标准final choice，adaptation `unassemblable` |
| `usage` | 取 `[DONE]` 前最后一个显式object，按JSON value写回；此前不同snapshot记issue但不阻止成功。非object非null为 `unassemblable` |
| Unknown top-level／choice／delta-message／tool／function字段 | 凡未在本表逐字枚举的字段一律归本行，不因SDK新版本把它加入typed model就改变分类。单次出现或重复相同JSON value时，按其原JSON层级分别投影到对应最终top-level／choice／message／tool／function object；choice与message、tool与function各有独立unknown槽，同名不得互相覆盖。同一路径出现冲突值时因没有合法merge语义而 `unassemblable`。非法choice／tool index的完整可读object与frame ordinal／offset进入独立unattributed槽；mode adaptation为 `unassemblable`，direct raw streaming仍原样交付并记issue＋raw fact |
| 只有 `[DONE]`、没有可构成identity的chunk | `unassemblable`，不得生成随机id、当前时间或 `model="unknown"` |

Carrier识别与code/type分类先于整份JSON进入`FrozenJson`域。若JSON语法可读但某个数值无法被当前FrozenJson／client writer有限表示，例如stdlib把`1e400`读成非有限float，则明确error carrier仍保留其carrier身份与已读taxonomy，完整raw值记为unreadable；ordinary content event则成为具名 `unassemblable`。不得让一个无关future字段的数值范围把known `server_error`变成本地未捕获exception。

`unassemblable` 是已收到但无法无损投影成标准non-stream Chat JSON的upstream response，不是transport瞬时失败，也不是本地parser bug；返回502 OpenAI error且不retry，并在 `upstream_error` 扩展中保存原始frame／字段路径。它不得像旧aggregator那样跳过坏event后继续合成成功。OpenAI SDK accumulator内部的 `parsed`、`parsed_arguments`、stream-only tool `index` 等辅助字段不得进入client JSON。

#### 9.3.2 Raw upstream exchange 与 client projection分槽

Mode adaptation不得用一个synthetic `httpx2.Response`冒充upstream exchange。Attempt／driver outcome分别保存：raw request bytes、upstream status／headers／HTTP version／connection snapshot、实际读取的SSE byte count与最终client JSON bytes。Rate limiter与response-header policy读取raw exchange；server response读取client projection；request completion的 `upstream_response_body_bytes`记录SSE bytes而不是synthetic JSON长度。Raw response在collector结束时恰好关闭一次，cleanup failure不得覆盖primary failure。

任何SSE failure都在driver success事件前判定；transport tear、无 `[DONE]` EOF和可重试流内error回到shared attempt owner，unknown本地parser bug不消费upstream预算。Driver对body-phase retry复用本attempt已冻结的final payload、admission与capability snapshot，不重新发布mutable `attempt.prepare`。
...
权威 terminal status 与「后续是否仍需客户端行动」是两个可以同时为真的事实，必须占两个槽，不能把后者折进前者，也不能拿前者覆盖后者。原生 Responses 直连路径的两种交付形态均已建立旁路事实：流式从 `response.output_item.*` 与 terminal event 观察，非流式从翻译前的完整 upstream response body 观察。两者使用同一个 `ResponsesObserver` 与同一套三态 action classifier；whole-body 的 `output` 数组与流式 terminal response 的 `output` 数组同为最终集合 authority，显式空数组与缺席／类型错误必须保持可区分。
...
Chat buffered transaction同样必须建立attempt-scoped provider observation，而且与retry／aggregation共用一份Chat event facts，不得由console、durable writer或mode-adapted synthetic body各自重推一次。Attempt draft、当前candidate与request-level projection是三个不同状态：replacement开始不销毁旧draft，replacement response成功建立时才原子切换candidate，最终action选定时才把被提交或作为fallback交付的candidate snapshot发布到 `RequestContext.response_observation` 与 `RequestTrace`；replacement在headers前失败且旧candidate成为最终carrier时，发布旧snapshot并另记replacement failure。Chat observation的唯一字段／absent／排序／展示authority是 [`../tui/spec.md`](../tui/spec.md) 的“Chat provider observation schema”；本规格只约束producer生命周期，不平行维护第二张字段表。
