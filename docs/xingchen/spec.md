...
状态：Xingchen provider 已在当前 `main` 实现；2026-09-06新增的“provider声明capability、pipeline准备Chat内容”边界尚未迁移，当前client仍注入streaming字段。

本规格定义 `ghc-api-proxy` 如何把 Xingchen 云端 LLM 网关接入 `app.model_provider`。它是本 feature 的目标行为权威；外部 TeleAgent 协议材料提供上游事实，用户在本项目中的裁决决定本次产品范围。当前实现状态以 [`status.md`](status.md) 与本段日期为准，不得从历史报告 PASS 反推尚未迁移的边界已经实现。
...
chat.response_modes = {streaming, non_streaming}
chat.stream_options_include_usage_default = true
chat.tool_stream_default = true
chat.provenance = "xingchen protocol measurement 2026-09-04"
```
...
Provider 不翻译格式、不解析别名、不选择 fallback，不覆盖上层已经决定的 `model`，也**不修改任何model-protocol字段**。Xingchen随resolved model endpoint向上层声明Chat capability；pipeline解释它并形成最终payload，provider client只接收该最终payload。

Xingchen Chat capability声明两个streaming request defaults：字段缺席时把 `stream_options.include_usage` 与 `tool_stream` 设为true，但调用方显式设置的值优先；它们不是强制requirement。Non-stream upstream mode不注入这两个字段。具体准备规则由provider-agnostic OpenAI Chat pipeline执行：
...
顶层和嵌套对象均不得被变异。Provider client不得重复执行上述规则；它只对pipeline准备好的最终payload序列化和签名。

Provider client只调用一次 `app.wire_json.dumps`，得到最终 `body_bytes`。SHA-256、HMAC 和 HTTP request 的 `content` 必须共用这一份字节，禁止签名后再次格式化或通过 SDK 重新序列化。

`WireJsonEncodeError` 在联网前原样传播。Capability与内容owner的跨provider合同见 [`../direct-passthrough/spec.md`](../direct-passthrough/spec.md) §5.4／§9.3。
...
- 2026-09-06：按用户确认的provider／pipeline边界修订§5：Xingchen只声明per-model endpoint Chat capability，`stream_options.include_usage`／`tool_stream` 的defaulting上移provider-agnostic pipeline；provider client只序列化、签名并原样发送最终payload。同步纠正“当前checkout尚未装位”的过期状态。
