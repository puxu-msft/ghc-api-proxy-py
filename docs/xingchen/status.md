...
当前 checkout 已包含 Xingchen 实现：`0cd1641` 的 feature commit 可从当前 `main` 达到，随后由 `5e73417` 与 CodeBuddy provider 对账合并；`src/app/model_provider/xingchen/` 与对应 unit tests 均被当前索引跟踪。此前“只在另一 source clone、当前未装位”的导入注记已被这些可观察事实推翻。

更新时间：2026-09-08

行为规格：[`spec.md`](spec.md)。Provider 主体已实现；当前实现仍由 Xingchen client 注入 streaming 字段，provider capability 与 pipeline Chat payload/mode 的边界尚未迁移。跨 provider 路由与模型目录见 [`../multi-provider-routing/`](../multi-provider-routing/README.md)。
...
以下是 2026-09-06 provider 交付时的历史验证快照，不代表本次文档整理重新执行了全仓验证。
...
协议评审与修复后代码复评均为 0 blocker/major。原始转录位于 [`reports/`](reports/)；这些报告保留为历史证据，不覆盖当前 checkout 的代码状态。
...
真实 canary 会使用有效 credential 并消耗额度，需要新的明确指令。本主题没有把真实 Xingchen 网关 canary 当作当前状态证据。
