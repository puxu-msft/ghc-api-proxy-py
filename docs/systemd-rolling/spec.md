...
- **状态**：**活文档，不冻结**（2026-08-24 用户裁定全面废除 spec 冻结规则；此前本行写的是 `FINALIZED`）。独立终审输入快照SHA-256为`485163f8a7eabba007c7b5464d69d4ee35717adcbdf2993fb0e05fae7fc5ea0f`，结论为`0 blocker／0 major`——那是**当时那份评审输入**的身份记录，是历史事实，不是本文此后不可修订的声明。本文完整记录用户于2026-08-08已裁决的产品保证及其必需派生合同；新的用户裁决或实测与本文冲突时当场修订本文。**但「不冻结」不等于实现可以自行改写行为**：实现与已裁决行为不符时，是把偏离交回用户重裁，不是改本文去迁就代码。
