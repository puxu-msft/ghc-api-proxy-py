...
状态：起草 2026-08-21，**第 2 稿，2026-09-08 残余审计同步**。核心依赖迁移、stream cap、WebSocket、OTel 与迁移当时的回归已完成并进入主线；V2 的 current-stack 异常形态已由后续实测回答。步骤 4 经 2026-09-08 逐处审计仍有明确未完成项，继续保留为 living residual。本文件是迁移记录与残余项载体：2026-08-21 的验证数字只记录当时运行，绝不能替代当前主线验证基线；当前命令与结果另以项目根 `CLAUDE.md` 为准。
...
| 步骤 4：散文与注释逐条复核 | **deferred，未完成，继续 living**。2026-09-08 实测活动 `src/` 无旧包 import，但仍有 75 个旧包完整词出现于 53 个位置；逐项结果见本计划的 deferred 与 `reports/260908-residual-audit.md`。其中 `idle_timeout.py:26` 已正确区分旧栈与当前栈，不能用剩余命中数冒充其他项已处置 | 不阻塞已完成的生产迁移；待源码所有者按 deferred 逐项修正并复核 |
...
**步骤 4——deferred，未完成，继续 living**：`idle_timeout.py:26` 已把旧 `httpx 0.28.1` 与当前 `httpx2` 的关闭行为分开记录；但 2026-09-08 对活动 `src/`（排除 `src/.archived/`）逐处复核确认，仍有把当前 `httpx2`／`httpcore2` 行为写成旧包的 live prose，且旧 logger 名仍被实际配置。本项不能以旧清单的 51 处或本轮 75 个完整词的任一计数结案；逐项 deferred 如下。完整命令、版本与边界见 `reports/260908-residual-audit.md`。

### 步骤 4 deferred（living）

1. **当前 transport／pool 叙述的 package label。** 源码所有者须逐项复核并只改当前栈的名称为 `httpx2`／`httpcore2`：`config/schema.py:272,278,280`；`server/composition.py:111,115,159,161,180,184,191,217,230,232,270,283,293,295,311,323,327`；`upstream/stream_cap.py:7,11,20,44,59,103,105,117,124,128,130`。这些位置混有迁移前的版本化观察（尤其 `schema.py:272,278` 与 `stream_cap.py:20,105`）；保留其 `httpx`／`httpcore` 历史名称，不能机械全替换。
2. **当前 request／response／异常链叙述的 package label。** 源码所有者须逐项复核：`model_provider/codebuddy_client/errors.py:42`、`model_provider/ghc_client/client.py:21`、`model_provider/upstream_errors.py:37,44,46,126,139`、`observability/rejection_capture.py:9`、`pipeline/delivery/stream.py:534`、`pipeline/hand_over.py:152,268`、`server/routes/inference.py:805,812,1738`、`observability/request_trace.py:57,99`。当前环境无旧 `httpx`／`httpcore` 可导入，活动代码使用的是 `httpx2`／`httpcore2`；历史归因若指向迁移前栈，须显式标版本而非改写事实。
3. **library logger 名与筛噪行为。** `observability/logging.py:34,198,199,216` 与 `observability/request_trace.py:29` 仍把当前库称作／配置成 `"httpx"`、`"httpcore"`。2026-09-08 实测 `httpx2` 的 logger 是 `httpx2`，`httpcore2` 子模块的 logger 前缀是 `httpcore2`，故 `logging.py:199` 是待源码所有者处理的运行时配置，不只是散文校正。
4. **已验证可保留项。** `streaming/idle_timeout.py:26` 同时明确旧 `httpx 0.28.1` 的历史观察和当前 `httpx2` 行为，准确，不改。其余混合历史／当前语句只有完成上述逐项处置及复核后才可加入此类。
