...
> 状态（2026-09-08）：`LIVING`，Current主树为`main@d0a4fc8b8fcc2306498f35601e73bb3a61f5983a`。S3 graceful timeout（`c53849e…`）和 S4 rootless user installer（`e9fb277…`）均已进入当前 `main`；它们的 reviewed-source archives 保留为 provenance，不是开发 HEAD。**本计划唯一未闭合的运行时门是 S5**：本机 private user-manager／cgroup 诊断在 private control socket 创建前以 `rc=1` 退出，且调用进程位于 root 所有、不可写的 `/init.scope`，所以不能安全取得 delegated cgroup，也未执行真实 activation、effective cgroup、restart 或 manager stop。下一环境必须是具备独立 login session／user manager 与 delegated cgroup v2 的可销毁 VM 或 container。三个 known minor 仍是 non-blocking 后补项，不是 S5 的并列阻塞理由。S7 已移交给 [systemd-rolling](../systemd-rolling/plan.md)；本计划不再保留 rolling 计划内容。`NO_CUTOVER` 保持不变。
> 文档同步基线：本次状态核对在 `main@d0a4fc8b8fcc2306498f35601e73bb3a61f5983a` 完成；读取主树事实的每次 shell 都现场解析 current `main^{commit}`，记录所得完整 SHA，并验证物理主树、`main` 分支与 `HEAD` 三者一致。后续 `main` 前进时更新状态快照，勿将旧 SHA 当成可执行命令的永久门。
> 证据边界：祖先 `main@d903d726…` 的 backup smoke R3 与祖先 `main@c1de6bf…` 的应用 canary 都只证明各自限定应用路径；它们不解除 S5 的 manager／cgroup `BLOCKED`，也不证明 unit 安装、activation 或 effective limits。
...
> 评审输入：两份初始 systemd 报告 `reports/260807-review-code-systemd-runtime.md`、`reports/260807-systemd-socket-feasibility.md`，code R2／R3／R4 `reports/260807-review-code-systemd-runtime-r2.md`、`reports/260807-review-code-systemd-runtime-r3.md`、`reports/260807-review-code-systemd-runtime-r4.md`，Plan R2～R6，旧 systemd-next merged-state review／verify／replay gate，以及重建审计 `reports/260807-audit-systemd-next-rebuild.md`、旧code-only merged-state review `reports/260807-review-systemd-code-only.md`、旧独立验收 `reports/260807-verify-systemd-code-only.md` 均已消费。重建审计否决携带旧 Plan patch 的直接 cherry-pick；旧code-only review精确绑定`2ec0cb8…`，verdict为`0 blocker／0 major／2 non-blocking minor`，旧独立verify对同一exact tip给出`PASS`。该旧 verdict 只证明 `80bc8f2…` base 上的两片顺序与语义，不放行在 `b91e58a29324b11840002efc53ed6f869b800c39` 直接 replay。Current输入另明确消费new-main merged-state独立代码评审`reports/260807-resume-review-systemd-rebuild.md`：它精确绑定`base@b91e58a29324b11840002efc53ed6f869b800c39`与`tip@d3fabfadfba57af6c2d63e543e3198444777df54`，verdict为`0 blocker／0 major`，只授权按`8cae6c2… → d3fabfa…`逐片squash且每片通过identity／preimage与main-side tests gate后才继续；并列消费`reports/260807-verify-systemd-rebuild-resume.md`与`reports/260807-resume-verify-systemd-rebuild.md`两份exact-tip独立verify `PASS`。三份current证据角色不互相替代，也不冒充本轮新Plan bytes已取得独立复评。
...
本计划把已进入main的systemd socket activation、Uvicorn inherited fd与cgroup v2骨架逐步收敛为可评审、可rootless验证、可由用户显式安装的运行方案。M1、S3和S4均已完成并进入main；S5首轮本机隔离诊断的helper／临时apply／unit verify与direct inherited-fd支持路径通过，但独立manager fixture在 private control socket 创建前失败，故真实manager／cgroup验收为`BLOCKED`。这是本计划唯一未闭合的运行时门，不是已经证实的产品代码缺陷；三个 non-blocking minor 继续后补。下一最小动作是在具备独立login session／user manager与delegated cgroup v2的可销毁VM／container继续S5诊断。rolling 已由 `systemd-rolling` 独立维护，不是本计划的后续门。Plan持续living且保持`NO_CUTOVER`。
...
M1后的 runtime 强化范围现已收敛：S3与S4 main checkpoints均已完成，reviewed-source archives均已建立；S5首轮本机隔离诊断因独立manager fixture启动失败而`BLOCKED`，应直接在具备独立login session／user manager与delegated cgroup v2的可销毁VM／container继续。Config precedence、逐文件atomicity与timeout facts重复owner三个 non-blocking minor继续后补，不扩大当前收敛门。S7 rolling 已由 `systemd-rolling` 独立维护。

## 2. 固定事实、已知可行性与能力边界

### 2.1 基线与候选事实

以下 M1 reviewed source 事实锚定到代码 base `ed77c9d191df81c451c25161420515cca52ce6a4` 与 source `49fb1988621bba4356e7a5039a6994c2e6d19604`；current main状态锚定本次修订时的`main@d0a4fc8b8fcc2306498f35601e73bb3a61f5983a`。读取主树时现场解析current `main^{commit}`并验证该完整SHA；S3、S4 rebuilt source链保留为provenance：
...
- S4 rebuilt source identity为`d3fabfa…`，parent为`8cae6c2…`，含3个S4非Plan paths；其delta已作为main commit `e9fb2771d6e040c761bb4074e3fcf2547caece28`进入主树。S4回并时的关键systemd tests、全量pytest 588项、Ruff与Pyright通过；后继`main@d903d72…`的全量pytest 590项、Ruff与Pyright通过。Current `main@d0a4fc…`继续包含S3、S4；`archive/260807-systemd-user-install`指向reviewed source `e16c2a700f23f66535e7347ab7357518eb8e56bd`。逐文件atomicity及timeout facts重复owner minor继续后补。
...
| M1 首次 squash／回放 checkpoint | **已在 main 完成，Plan 继续 living** | `cf53334…` 已是 current `main@d0a4fc8b8fcc2306498f35601e73bb3a61f5983a` 的祖先；M1 main-side 全仓 pytest 375 项、Ruff、Pyright 通过；`archive/260807-systemd-runtime` → `49fb198…` | 不重复回放，不外推为部署或 cutover |
...
| S5 真实 user-manager／cgroup smoke | **唯一未闭合运行时门：本机 private user-manager／cgroup 诊断 `BLOCKED`；转外部环境续诊** | 历史执行记录`reports/260807-systemd-user-manager-smoke.md`绑定`main@e9fb277…`：helper、临时apply、动态unit verify与direct inherited-fd关键路径通过；独立`systemd --user`在 private control socket 创建前`rc=1`，故真实activation／effective cgroup／restart未执行。当前调用进程位于 root 所有、不可写的`/init.scope`，无 delegated cgroup；应用 canary HTTP 200不是manager／cgroup证据。Current主树为`d0a4fc…` | 转可销毁VM／container，提供独立login session／user manager与delegated cgroup v2后继续S5；不退回宿主manager，不触碰生产4141 |
| S6 M2 new-main 组合复核 | **review／verify与main回并均闭合** | source exact链`b91e58a… → 8cae6c2… → d3fabfa…`；474项执行／collect-only、Ruff、Pyright、正控与Plan排除通过；S3、S4分别以`c53849e…`、`e9fb277…`进入main。`d903d72…`历史gate为全量590项tests＋Ruff／Pyright；current main已前进到`d0a4fc…` | 保留provenance与三个non-blocking minor；在受控VM／container继续本机`BLOCKED`的S5诊断 |
| S7 双实例／rolling | **已移交 `systemd-rolling`，不再是本计划保留项** | [rolling Spec](../systemd-rolling/spec.md) 与 [rolling Plan](../systemd-rolling/plan.md) 已拥有拓扑、readiness、状态隔离、drain、回滚与并发合同；移交前摘要见 [history](history/260908-s7-before-systemd-rolling-handoff.md) | 在 `systemd-rolling` 按独立计划实施和验收；不得将其完成度作为 S5 的条件 |
...
**下一动作**：S3、S4 已进入main且其 archives 已建立。S5 是本计划唯一未闭合的运行时门：在可销毁VM／container中，以独立login session／user manager和delegated cgroup v2继续真实 activation／cgroup smoke。三个 non-blocking minor 后补，不升级为当前门；unit 安装与生产 manager 操作继续禁止，保持`NO_CUTOVER`。S7 已移交 `systemd-rolling`。
...
**状态**：**本机private user-manager／cgroup诊断`BLOCKED`，也是本计划唯一未闭合的运行时门；转外部环境续诊。** [S5执行记录](reports/260807-systemd-user-manager-smoke.md)精确绑定历史 `main@e9fb277…`；本次状态核对的主树为`main@d0a4fc…`。private manager诊断（原引 `docs/tmp/260807-systemd-user-manager-diagnosis.md`；该名在任何 ref 与批次清单里都不存在，搬迁前即为断链，未替它猜目标）进一步确认当前VS Code调用进程位于root所有且不可写的`/init.scope`，无法安全获得独立manager所需delegated cgroup。Helper渲染、临时apply、动态loopback unit verify与不经过manager的direct inherited-fd／liveness／readiness／graceful关键路径通过；但独立`systemd --user`在创建临时`XDG_RUNTIME_DIR/systemd/private`前静默以`rc=1`退出。因此没有执行真实`daemon-reload`、socket／service启动、systemd fd inheritance、effective cgroup、restart或manager stop，S5不得判`PASS`。这不是已证实的产品代码缺陷。Backup smoke R3与应用 canary 都只证明祖先候选的限定应用路径，不是manager／cgroup证据。下一诊断环境必须是可销毁VM／container，具备独立login session／user manager与delegated cgroup v2；不得退回当前宿主user manager。本阶段不触碰生产端口或现有服务，旧Bun仍持有双栈4141且位于`/init.scope`。
...
**评审**：`reports/260807-resume-review-systemd-rebuild.md`精确绑定new-main base`b91e58a…`与tip`d3fabfa…`，merged-state verdict为`0 blocker／0 major`，明确允许按`8cae6c2… → d3fabfa…`顺序逐片squash，并要求每片identity／preimage gate与main-side tests gate通过后才进入下一片。`reports/260807-verify-systemd-rebuild-resume.md`与`reports/260807-resume-verify-systemd-rebuild.md`分别对同一exact tip给出独立`PASS`；474项执行／collect-only、Ruff、Pyright、deadline正控、短SIGTERM、installer与Plan排除均通过。旧code-only／systemd-next identity与Plan postimage仍不可作为current payload。M2 review与verify证据门已闭合到code-only范围，不得外推S5或cutover。

**回并边界与下一动作**：S3、S4两片已分别进入main，不再重放。S3 source archive为`archive/260807-systemd-graceful-timeout → 865a5b71210e2436b36786b5de67146939d1e0f5`，S4 source archive为`archive/260807-systemd-user-install → e16c2a700f23f66535e7347ab7357518eb8e56bd`。旧链与rebuilt source identities只保留provenance，不作为S5开发HEAD。S5本机首轮诊断仍`BLOCKED`，下一步在受控VM／container继续定位并完成真实manager／cgroup验收；三个 non-blocking minor 继续登记但不是并列 gate。S7 已移交 `systemd-rolling`。Worktree／branch清理不在本计划自动执行。

### S7：已移交 `systemd-rolling`

S7 不再是本 plan 的未来切片或保留理由。现行 rolling 的行为合同与实施顺序分别由 [systemd-rolling Spec](../systemd-rolling/spec.md) 和 [systemd-rolling Plan](../systemd-rolling/plan.md) 独占；该线的 `4144` 动态 generation 运行面独立于本文所维护的单实例 `4141` runtime／S5 manager smoke。移交前的简略范围保留在 [history/260908-s7-before-systemd-rolling-handoff.md](history/260908-s7-before-systemd-rolling-handoff.md)，仅供追溯，不能作为当前设计输入。
...
| M1 squash／回放 checkpoint | **已在 main 完成，Plan living 继续** | `cf53334…` 已是 current `main@d0a4fc8b8fcc2306498f35601e73bb3a61f5983a` 的祖先；M1 gate 为全仓 pytest 375 项、Ruff、Pyright 通过；archive ref → `49fb198…` | 不重复回放；S3、S4均已main，不外推安装、部署或cutover |
...
| rolling | **已移交 `systemd-rolling`** | 现行 Spec／Plan 是唯一 owner；本 plan 不再重复或门控 rolling，移交前摘要仅存 `history/260908-s7-before-systemd-rolling-handoff.md` |
...
| 旧 integration commits 与 `plan.md` | 高频 living 状态、旧 base 与可复用代码语义耦合，导致代码语义仍可作 oracle，而 old commits 不可安全回放到 new main | 排除Plan的rebuilt source为`8cae6c2… → d3fabfa…`；S3、S4已分别以`c53849e…`、`e9fb277…`进入main。旧两条链仅保留provenance，长期可再拆分volatile execution state与稳定设计，但不删减本Plan的living信息 |
...
从 current `main@d0a4fc8b8fcc2306498f35601e73bb3a61f5983a` 继续本 living Plan。M1已以`cf53334a10a717a3a3d30d6c0e8a297f5000d90c`进入main历史，`archive/260807-systemd-runtime`仍精确指向reviewed source`49fb1988621bba4356e7a5039a6994c2e6d19604`。S3已以main commit`c53849e2b5103c6426a67a8cbab687f2e45c1fa0`进入，`archive/260807-systemd-graceful-timeout`精确指向reviewed source`865a5b71210e2436b36786b5de67146939d1e0f5`。S4已以main commit`e9fb2771d6e040c761bb4074e3fcf2547caece28`进入，`archive/260807-systemd-user-install`精确指向reviewed source`e16c2a700f23f66535e7347ab7357518eb8e56bd`。`d903d72…`的590项tests、Ruff与Pyright只作历史gate；不要把该测试数冒充current main新跑结果。不要重放M1、S3或S4；读取主树时现场解析current `main^{commit}`并固定所得完整SHA。

先消费两份初始 systemd reports、code R2～R4、Plan R2～R6、旧 systemd-next 证据，以及 `reports/260807-audit-systemd-next-rebuild.md`、`reports/260807-review-systemd-code-only.md`、`reports/260807-verify-systemd-code-only.md`。Current new-main证据必须另外并列消费：`reports/260807-resume-review-systemd-rebuild.md`精确绑定`b91e58a… → 8cae6c2… → d3fabfa…`并给出merged-state review `0 blocker／0 major`；`reports/260807-verify-systemd-rebuild-resume.md`与`reports/260807-resume-verify-systemd-rebuild.md`对同一exact tip分别给出独立`PASS`。Code review、两份verify与旧链历史证据角色不同，不得互相替代。M1 已确认 `Type=exec`、`KillMode=control-group`、StateDirectory＋显式 History／tokenization 路径、`--fd >= 1`、`StateDirectoryMode=0700`、`UMask=0077`、覆盖目录文档、真实 writer 权限回归，以及无 HOME 的 inherited-fd／backlog／readiness／真实请求／History＋tokenization smoke。保持 `/health/readiness` 为独立 oracle；不得把 `Type=exec` 写成应用 ready，也不得把仓库 checkpoint 写成 unit 已安装或运行态已切换。

Rebuilt source链精确为`b91e58a29324b11840002efc53ed6f869b800c39 → 8cae6c260c8bc2930be96eaecc7d6d24d470e00a → d3fabfadfba57af6c2d63e543e3198444777df54`，两片均排除Plan，merged-state review为`0 blocker／0 major`且两份独立verify均为`PASS`。两片已经分别以`c53849e…`、`e9fb277…`进入main。Backup smoke R3已在祖先`d903d72…`取得限定范围PASS，不再排队复跑，也不覆盖后继current main。S5本机首轮隔离诊断已执行，但独立manager fixture在private control socket创建前失败，真实manager／cgroup判为`BLOCKED`。真实Copilot app canary的HTTP 200不是manager／cgroup证据。全局下一序列固定为：在受控VM／container继续S5；按真实缺口补retry／quota／partial-write。rolling 的后续实施与验收由 [systemd-rolling](../systemd-rolling/plan.md) 独立管理。三个non-blocking minor——config precedence判别力、逐文件atomicity故障恢复回归、timeout facts重复owner——继续登记但不升级为S5前置门。
