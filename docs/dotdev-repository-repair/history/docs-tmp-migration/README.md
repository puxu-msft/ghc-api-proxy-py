...
**一处后续规整**：`delivery-keepalive` 从 `docs/agents/` 搬来时，17 份评审原件是摊在话题根上的（`review-async-correctness-r2.md` 之类），与其余话题「原件收在 `reports/` 下」的形状不一致——那是它在旧位置就有的组织方式，搬迁按「不改内容不改结构」原样保留。用户随后指示规整，17 份已 `git mv` 进 `reports/`（现共 33 份），话题根只剩 `spec.md` / `status.md` / `deferred.md`。三份活文档里 7 处裸文件名引用已加 `reports/` 前缀并逐条验证可解析；17 份之间的 4 处互引因仍同目录而无需改动。这 17 份里另有 37 处对话题根三份活文档的行内引述（10 处带行号），**没有一处是 markdown 链接**，因此搬深一层不弄断任何东西——它们是「被评审文档在当时的样子」的引述，按快照不改写的原则保持原样。
