...
本文件承接 2026-08-27 从 `deferred.md` 迁出的裁决、完成记录、已否决方案与方法教训。迁移未压缩原文；各段中的提交、日期与点时判断保留其原有含义。当前未闭合项仍以 [`deferred.md`](deferred.md) 为准。
...
用户 2026-08-20 裁决：被取代就删。逐条核对确实被取代——原始意图见[历史设计快照 `2604-streaming-resilience.md`](history/2604-streaming-resilience.md) 的原配置表（迁移前 `:284-288`）：
...
该文档现位于[历史证据 `history/2604-streaming-resilience.md`](history/2604-streaming-resilience.md)，属归档件。其原 `:284-288` 的配置表用旧 `AppSettings` 键名，且 `upstream.keepalive_expiry = 30` 从来不是生效值——但归档件记录的是当时的设计意图，不是当前事实，**不必回头改它**。当前事实以 `schema.py` 的注释与本文件为准。
