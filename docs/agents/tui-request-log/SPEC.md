...
| **下行字节** | 按**显示出来的**数字分档：<10.0KB 灰、<100.0KB 白、≥100.0KB 黄（KB 为 1024 进制） |
| **下行词元** | 同样按显示的数字：<1.0k 灰、<10.0k 白、≥10.0k 黄（k 为 1000 进制） |
...
- **按「显示出来的数字」而不是原始数值分档**。两个 formatter 都只印一位小数，于是原始阈值恰好落在四舍五入带里：10239 与 10240 字节都显示 `10.0KB`，按原始值分档就会让**同一个数字出现两种颜色**——这正是加颜色本要避免的困惑。改按显示值判定，代价是实际阈值前移约 0.5%（10.0KB 从 10240 变成 10189 字节），换来「屏幕上不可能自相矛盾」。`round()` 与 `f"{:.1f}"` 对同一个浮点数舍入方式相同，这是两者不会脱节的依据；`tests/unit/test_request_log.py::test_the_same_number_is_never_shown_in_two_colours` 是守住它的断言。
