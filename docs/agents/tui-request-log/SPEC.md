...
| **下行字节** | <10KB 灰、<100KB 白、≥100KB 黄（1024 进制，与 `format_bytes` 打印的数字同时变色） |
| **下行词元** | <1k 灰、<10k 白、≥10k 黄 |
| 上行字节 | 恒灰。它的大小由客户端发来的请求决定，与回复如何无关 |
| 上行词元 | 本身不着色；其缓存分段为「读」灰、「写」青 |
| 缓存命中率 | 反向：≥80% 灰，越低越红 |
| **结束原因** | `end_turn` / `stop_sequence` 绿；`max_tokens` / `refusal` 黄；`tool_use` / `function_call` 不着色 |
...
- **结束原因分两档而不是一档**。`max_tokens` 与 `refusal` 确实终结了这一轮，但它们不是干净的收尾；染成与 `end_turn` 同样的绿会把「回复被截断」显示成「一切正常」。绿色必须保住「这里没什么可看的」这个含义。`tool_use` 两者都不是——它表示这一轮**还在继续**。
- **`AskUserQuestion` 从灰色列表里挑出来**。列表里其它每一项都会自行了结，只有它意味着这一轮在等一个人。
