...
- **当前总状态**：**`NO_CUTOVER／PARTIAL`**。Reasoning capability、History facts、stream route、S3 graceful、S4 rootless installer、stream request facts与headers-before Responses network retry已依次进入current `main@080105b…`；network retry reviewed source由`archive/260807-responses-network-retry@584e63b…`精确保留，父提交`fb5c027…`的main-side全量599项tests、Ruff与Pyright通过。`080105b…`已补回[current main network retry定向复核](../../tmp/260807-review-main-network-retry.md)识别的exhaustion永久回归门，5个具名component tests经collect与执行交叉核对均通过，唯一major关闭。Stream主路径R3仍只在祖先`d903d72…`取得`PASS_KEY_BACKUP_PORT_SMOKE_R3`并关闭History facts定向缺口，不覆盖retry后current main、request／global quota与resident backpressure、真实socket partial-write／delivery uncertainty、真实credential、真实systemd manager或cutover；完整产品继续`UNVERIFIED`。Resident-byte final candidate为clean `feat/resident-byte-budget@5b744ce81d0b3c8a3684aab12a376aa7b3bd5cad`；旧`63db675…`独立评审的4项major均已修复，final focused re-review为`0 blocker／0 major`且`mergeable`，独立验收`PASS`，exact final HEAD全量611项tests通过，因此可在current living checkpoint后squash，但仍未进入main。Production request／global quota完整接线与resident backpressure仍为`UNVERIFIED`。S5隔离诊断因当前VS Code调用上下文位于不可委派的`/init.scope`、独立`systemd --user`未创建private control socket而把本机真实manager／cgroup／unit运行态判为`BLOCKED`，下一入口为可销毁VM／container。P0仍为`PARTIAL`，P1仍为`FOUNDATIONS_ONLY`，P2与P3均未闭合。
...
- **作者基线**：`/home/xp/src/ghc-api-proxy-py` 的 current `main@080105b54614e1320a5c193d7206dcaa584c9b41`。本轮每次 shell 调用均在同一次调用内现场解析`main^{commit}`，并验证物理仓库根、当前目录、`main` 分支、`HEAD == refs/heads/main`与所得完整SHA。Current main在线性既有foundation、systemd runtime、happy／usage、semantic／route／block之上，又包含reasoning capability `bd86207…`、History facts `38bb06f…`、stream route `ae84aa9…`、S3 graceful `c53849e…`、S4 rootless installer `e9fb277…`、stream request facts `d903d72…`、headers-before Responses network retry `fb5c027…`与exhaustion永久回归测试`080105b…`。这些仓库bytes与范围内绿灯仍不表示完整retry／quota、真实socket partial-write、完整P0 bridge、安装态、真实manager运行态或cutover已完成。
- **当前输入身份与状态**：Current cutover inventory SHA-256为`1f72038de99bbddc4a3c71243d5e20ce8d0db8783afe6d49dcf46f17e63481d9`；Spec为`FINALIZED@5e362822…`，Acceptance为`FINALIZED_ACCEPTANCE_ORACLE@6457b896…`且本轮未改。Current main为`080105b…`；capability、History、stream、S3 graceful、S4 installer、stream request facts、headers-before network retry与其exhaustion永久回归门均已main。父提交的全量599项tests、Ruff与Pyright通过，`080105b…`的5个定向tests通过，network retry reviewed source已归档且唯一major关闭。备用端口happy smoke的既有`PASS`绑定`ae84aa9…`，R2 scoped PASS绑定`e9fb277…`，R3 scoped PASS `PASS_KEY_BACKUP_PORT_SMOKE_R3`绑定祖先`d903d72…`并关闭stream History facts定向缺口，但不得外推到retry后主线。Resident-byte final candidate `5b744ce…`已关闭旧`63db675…`的4项major，取得final review 0 major、verify `PASS`与全量611项tests通过，可squash但尚未main；production request／global quota与resident backpressure继续`UNVERIFIED`。Unit仍未安装；S5尝试了隔离真实user-manager入口，但当前VS Code进程位于root所有且不可写的`/init.scope`，无法安全获得独立manager所需delegated cgroup，故本机真实activation、cgroup与unit lifecycle保持`BLOCKED`。下一序列是checkpoint current Implementation＋Readiness→resident squash，再到VM／container继续真实manager／cgroup验证并逐片补quota／backpressure与partial-write；真实credential与cutover仍未验证。
...
|---|---|---|---|
| P0 | 核心 Anthropic → Responses 正确服务 | `PARTIAL` | Capability、History、stream、stream request facts、headers-before network retry与其exhaustion永久回归门已进入current `main@080105b…`；父提交main-side全量599项tests、Ruff与Pyright通过，current 5个定向tests通过，唯一major关闭。Backup smoke R3绑定祖先`d903d72…`并关闭stream History facts定向缺口。Resident-byte final candidate `5b744ce…`已取得final review 0 major、verify `PASS`与全量611项tests通过，可squash但尚未main；完整retry、production quota／resident backpressure接线、真实socket partial-write／delivery uncertainty、真实credential及完整Acceptance仍`UNVERIFIED`。 |
| P1 | 运行面 | `FOUNDATIONS_ONLY` | Systemd runtime、S3 graceful与S4 rootless installer代码均已进入main，current主树为`080105b…`；unit未安装。S5在当前VS Code／`/init.scope`上下文无法安全启动具备delegated cgroup的独立manager，故本机真实manager／cgroup／unit运行态、双fd／双栈仍`BLOCKED`或未验；下一入口为可销毁VM／container。备用端口scoped PASS不等于restart／systemd／双栈PASS；旧Bun仍持有双栈`4141`。 |
...
|---|---|---|---|---|
| Request conversion 与 route | `FOUNDATIONS_ONLY` | Capability修复已作为`main@bd86207…`进入主树并与History、stream合成；定向复核确认每次attempt的`PRE_SEND`后按resolved-model capability重新转换。R2实证的stream History request-facts缺口已由reviewed source`4fa7a87…`的净语义作为`main@d903d72…`修复并archive；retry后current `main@080105b…`已含永久exhaustion门且5个定向tests通过。R3真实History API只在祖先`d903d72…`观察到final attempt的typed request conversion fact，定向缺口关闭但不外推到retry后主线。 | `bridge/request`＋`pipeline/route` | 按未验证边界补thinking success、unknown capability reject、budget边界与retry后重转换的完整Acceptance正反控制；R3 scoped PASS不得外推到完整request矩阵。 |
| Non-stream response | `FOUNDATIONS_ONLY` | History facts已作为`main@38bb06f…`进入主树并与stream、network retry合成；真实`HistoryConsumer`的completed／failed投影及non-stream失败零成功事实已获定向复核。祖先`d903d72…`的R3 non-stream真实route通过；`080105b…`现已永久固定exhaustion attempt／History／hooks语义并关闭唯一major，完整错误、retry与usage矩阵仍未执行。 | `bridge/nonstream` | 从备用端口按真实缺口补non-stream错误、usage／History exactly-once与local fault矩阵。 |
| Stream conversion 与 strict Anthropic SSE | `PASS_CURRENT_LAYER` | Stream route已作为`main@ae84aa9…`进入主树；绑定`ae84aa9…`的定向复核为0 blocker／0 major，同提交备用端口为`PASS_HAPPY_BACKUP_PORT_SMOKE`。`e9fb277…`关键主路径R2取得`PASS_KEY_BACKUP_PORT_SMOKE_R2_WITH_STREAM_HISTORY_FACT_GAP`并获独立复核`0 blocker／0 major／1 minor`；该request-facts缺口已由`main@d903d72…`修复，R3取得`PASS_KEY_BACKUP_PORT_SMOKE_R3`并由真实History API确认缺口关闭。Retry后current `main@080105b…`的exhaustion永久测试与4个近邻定向tests通过，但R3不得外推到该提交。Semantic reorder、完整terminal／usage、完整retry、quota／backpressure、真实socket partial-write与真实credential仍未验证。 | `bridge/stream` | 按真实缺口执行`STREAM-MERGE-05`、完整usage／terminal／History矩阵及retry／quota／partial-write的持久化正反控制，不得把scoped PASS冒充完整Acceptance。 |
| Block-level buffering／delivery frontier／resident budgets与backpressure | `UNVERIFIED` | Spec／Acceptance已冻结完整Anthropic content block为最小下游提交单元，并由 `REL-06` 冻结普通 per-request aggregate＋global resident reservation、有限queue、charge-before-read、两级可观测计账、capacity／deadline／cancel终态与拒绝新admission。Current main stream已把typed delivery接入真实ASGI route，备用端口证明首个完整block前零success bytes，定向测试覆盖首个ASGI body send uncertainty。Final candidate `5b744ce…`已实现opt-in两级weighted reservation、request account、lease cleanup与ACK bytes retention删除，旧`63db675…`的4项major已修复，final review 0 major、verify `PASS`且全量611项tests通过，但它仍未main，也未接production配置／route／parser drafts；因此真实socket多个offset partial-write／RST、完整delivery-uncertain、请求级／全局配额完整接线、有限queue和真实backpressure继续为`UNVERIFIED`。 | `delivery/block-buffer`＋`pipeline/sink`＋`runtime/memory-quota`＋`pipeline/admission` | 先checkpoint→resident squash；随后以真实并发reader、慢consumer和loopback socket覆盖 `A.start → B.done → A.done`、同item多content parts、open-source terminal、block批提交、response-start／多个body byte offset／terminal短写或RST，以及uncertain后禁止重发／success terminal。另以配置化 `0 < request_budget < global_budget` 聚合同一请求的多个resident owner，覆盖有限queue、charge-before-read、drain后容量恢复、各终态release归零、global压力下拒绝新admission且已接纳请求继续受控；分别注入global-only和single-block／16 MiB分支两种单侧缺陷，禁止spill或live forwarding。 |
...
| Readiness 项 | Current status | Current evidence | Owner | Next smoke |
|---|---|---|---|---|
| 备用端口完整候选 | `PARTIAL` | `main@ae84aa9…`已在隔离`4142` app＋`4143` fake upstream取得`PASS_HAPPY_BACKUP_PORT_SMOKE`。`e9fb277…`关键主路径R2取得`PASS_KEY_BACKUP_PORT_SMOKE_R2_WITH_STREAM_HISTORY_FACT_GAP`；该缺口已由`main@d903d72…`修复并archive，R3取得`PASS_KEY_BACKUP_PORT_SMOKE_R3`。Retry后current `main@080105b…`的5个定向tests通过且唯一major关闭，但尚无该提交的备用端口运行证据。 | `runtime/launcher`＋`service-cutover/canary` | 到VM／container完成真实manager／cgroup验证，随后按实际缺口补quota／backpressure、真实partial-write、真实credential及其余持久化矩阵。 |
| Readiness／liveness语义 | `UNVERIFIED` | Current main `080105b…`包含systemd runtime、S3 graceful与S4 installer代码；祖先上的备用端口R2实测liveness／readiness、真实Anthropic主路径与清理。S5独立manager fixture启动失败，故仍无安装态或真实manager证据，也未覆盖认证失效、模型目录未加载、History不可写或shutdown draining。 | `runtime/health` | 在具备独立user manager与delegated cgroup v2的可销毁容器或虚拟机中覆盖启动、上游认证失效、模型目录未加载、History不可写和shutdown draining，且不操作生产`4141`。 |
| User systemd socket activation | `BLOCKED` | M1 socket activation runtime、S3 graceful与S4 rootless installer代码已进入main，current主树为`080105b…`。S5本机隔离执行中helper、临时apply、动态unit verify与direct inherited-fd支持路径通过，但独立`systemd --user`在创建private control socket前以`rc=1`退出；真实daemon-reload、activation、fd inheritance与restart均未执行。 | `runtime/systemd` | 在可销毁systemd-nspawn容器或虚拟机中提供独立login session／user manager与delegated cgroup v2后复验；不得退回宿主user manager，安装任何真实目标unit仍须另行明确授权。 |
...
| 独立评审与最终授权 | `UNVERIFIED` | Capability、History、stream、S3、S4 installer、stream request facts、headers-before network retry与其exhaustion永久回归门均已main；current为`080105b…`，父提交main-side全量599项tests、Ruff与Pyright通过，current 5个定向tests通过，network retry reviewed source已archive且唯一major关闭。Backup smoke R3仍绑定祖先`d903d72…`。Resident-byte final candidate `5b744ce…`已关闭旧4项major，取得final review 0 major、verify `PASS`与全量611项tests通过，可squash但尚未main；S5本机真实manager／cgroup诊断`BLOCKED`。P0 10＋P1 8＋P2 11＋P3 12＋`cc-daemon` 2＝43行口径保持；完整retry／production quota接线／partial-write、真实credential、真实manager／cgroup／unit运行态、P2、最终unit bytes、runbook、rollback与cutover仍无最终merged-state证据。 | `review/readiness`＋用户 | 本次新bytes必须按新hash重新定向复评，旧hash verdict不能沿用；先checkpoint current Implementation＋Readiness，再执行resident squash。在可销毁VM／container续诊S5，并按真实缺口补quota／partial-write与真实credential；任何局部PASS均不等于完整产品PASS或切换授权。 |
...
1. **P0组合已main但完整Acceptance未闭合**：Capability `bd86207…`、History `38bb06f…`、stream `ae84aa9…`、headers-before network retry `fb5c027…`与exhaustion永久回归测试`080105b…`已依次进入main；5个定向tests通过，retry review唯一major关闭。这只关闭该定向major，不关闭完整产品门。
2. **P0关键主路径R3仍只绑定祖先**：Stream request facts已作为`main@d903d72…`进入主树并由`archive/260807-stream-request-facts@4fa7a877…`归档。Backup smoke R3取得`PASS_KEY_BACKUP_PORT_SMOKE_R3`，真实History API确认stream request conversion facts定向缺口关闭，但该scoped PASS不得外推到retry后`080105b…`。Resident-byte final candidate `5b744ce…`已关闭旧`63db675…`的4项major，取得final review 0 major、verify `PASS`与全量611项tests通过，可squash但尚未main；仍须按真实缺口执行semantic reorder、完整usage／terminal／History、完整retry、production request／global quota与resident backpressure接线、真实socket partial-write／delivery uncertainty及真实credential正反控制；缺任一项都不得升级完整P0或完整产品。
3. **P1 runtime未闭合且S5本机诊断`BLOCKED`**：S3 graceful与S4 installer代码均已进入main，current主树为`080105b…`；unit仍未安装。当前VS Code调用进程位于root所有且不可写的`/init.scope`，无法安全启动具备delegated cgroup的独立user manager；private control socket未出现，真实user manager／cgroup／unit、双fd／双栈activation／restart smoke均未执行。下一入口是可销毁容器或虚拟机，不是宿主user manager。旧Bun继续持有双栈`4141`；禁止stop／signal并保持`cc-daemon`不变。S7 rolling继续后置。
...
| 位置 | 怪味类型 | 处置 |
|---|---|---|
| Bridge开发线与完整产品状态 | Capability／History／stream、stream request facts、headers-before network retry与exhaustion永久回归门已进main，current 5个定向tests通过且唯一major关闭，R3关键主路径仍只绑定祖先`d903d72…`。Resident-byte final candidate `5b744ce…`已取得final review 0 major、verify `PASS`与全量611项tests通过，可squash但尚未main；完整retry、production quota／backpressure接线、真实socket partial-write、真实credential仍未闭合；局部绿灯容易被拼成“服务已完成”的假绿 | Stream主路径精确标`PASS_CURRENT_LAYER`、P0域标`PARTIAL`；先checkpoint→resident squash，再分别补完整容量接线、partial-write与credential入口证据。R3继续固定为祖先上的`PASS_KEY_BACKUP_PORT_SMOKE_R3`，不得外推为current main或完整产品`PASS`。 |
| `src/app/cli.py`、current-main systemd runtime与installer | 应用host／port owner、单fd模板、S3 graceful、S4 rootless render／install和目标双栈socket activation分属不同层 | S3与S4 installer代码均已进入main，current主树为`080105b…`；仍须备用端口真实双具名fd／manager／cgroup／unit smoke，不得把installer代码升级为安装态或生产运行态。 |
...
**实时结论：`NO_CUTOVER／PARTIAL`。Capability、History、stream、S3 graceful、S4 rootless installer、stream request facts、headers-before Responses network retry与exhaustion永久回归门均已进入current `main@080105b…`；network retry reviewed source已由`archive/260807-responses-network-retry@584e63b…`归档，父提交main-side全量599项tests、Ruff与Pyright通过，current 5个定向tests通过，唯一major关闭。Backup smoke R3仍只绑定祖先`d903d72…`并确认stream request conversion facts定向缺口关闭，不外推到retry后主线。Resident-byte final candidate `5b744ce…`已关闭旧`63db675…`的4项major，取得final review `0 blocker／0 major`、verify `PASS`与exact final HEAD全量611项tests通过，可squash但尚未main；完整retry、production request／global quota与resident backpressure接线、真实socket partial-write／delivery uncertainty、真实credential继续`UNVERIFIED`。Unit未安装；S5真实user manager／cgroup／unit在本机因当前VS Code／`/init.scope`上下文无法安全启动具备delegated cgroup的独立manager而`BLOCKED`，下一入口为可销毁VM／container，S7 rolling后置；真实systemd manager与cutover均未验证。P0保持`PARTIAL`，P1保持`FOUNDATIONS_ONLY`；旧Bun incarnation与双栈`4141`边界保持，`cc-daemon`继续禁止触碰。43行口径保持为P0 10＋P1 8＋P2 11＋P3 12＋`cc-daemon` 2。本文继续living、不收口；下一序列严格为current Implementation＋Readiness checkpoint→resident squash→真实manager／cgroup外部环境→按真实缺口补quota／partial-write与credential。本轮新hash须后续重新复评，所有绑定旧hash的current复评均失效。在P0～P3 required行闭合前，不替代当前`copilot-api-js`。**
