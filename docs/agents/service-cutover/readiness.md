...
- **当前总状态**：**`NO_CUTOVER／PARTIAL`**。Reasoning capability、History facts、stream route、S3 graceful与S4 rootless installer已依次进入current `main@e9fb277…`。Stream主路径先凭`ae84aa9…`的定向`0 blocker／0 major`与同提交备用端口`PASS_HAPPY_BACKUP_PORT_SMOKE`提升为`PASS_CURRENT_LAYER`；后继current-main R2又取得`PASS_KEY_BACKUP_PORT_SMOKE_R2_WITH_STREAM_HISTORY_FACT_GAP`并获独立复核`0 blocker／0 major／1 minor`。该scoped PASS覆盖nonstream、stream withholding、SSE、唯一terminal、cancel、SIGTERM cleanup、wait／reap与旧Bun不变，不覆盖完整retry、request／global quota与resident backpressure、真实socket partial-write／delivery uncertainty。唯一minor是stream History request conversion fact缺口，clean candidate `4fa7a87…`已完成`0 blocker／0 major／1 minor` final review并明确可以squash；候选既有6个定向tests、全量590项回归与backup smoke支持该缺口的定向verification闭环，但候选尚未main，也不构成完整产品或完整Acceptance `PASS`。Installer代码已main；S5隔离诊断因当前VS Code调用上下文位于不可委派的`/init.scope`、独立`systemd --user`未创建private control socket而把真实manager／cgroup／unit运行态判为`BLOCKED`。P0仍为`PARTIAL`，P1仍为`FOUNDATIONS_ONLY`，P2与P3均未闭合。
...
- **作者基线**：`/home/xp/src/ghc-api-proxy-py` 的 current `main@e9fb2771d6e040c761bb4074e3fcf2547caece28`。本轮每次 shell 调用均在同一次调用内验证物理仓库根、当前目录、`main` 分支与该精确提交。Current main在线性既有foundation、systemd runtime、happy／usage、semantic／route／block之上，又包含reasoning capability `bd86207…`、History facts `38bb06f…`、stream route `ae84aa9…`、S3 graceful `c53849e…`与S4 rootless installer `e9fb277…`。这些仓库bytes与范围内绿灯仍不表示完整retry／quota／真实socket partial-write、完整P0 bridge、安装态、真实manager运行态或cutover已完成。
- **当前输入身份与状态**：Current cutover inventory SHA-256为`1f72038de99bbddc4a3c71243d5e20ce8d0db8783afe6d49dcf46f17e63481d9`；Spec为`FINALIZED@5e362822…`，Acceptance为`FINALIZED_ACCEPTANCE_ORACLE@6457b896…`且本轮未改。Current main为`e9fb277…`；capability、History、stream、S3 graceful与S4 installer均已main。备用端口happy smoke的既有`PASS`绑定`ae84aa9…`，current-main R2的scoped PASS绑定`e9fb277…`且保留唯一stream History request-facts缺口；二者都不覆盖retry／quota／partial-write矩阵。Unit仍未安装；S5尝试了隔离真实user-manager入口，但当前VS Code进程位于root所有且不可写的`/init.scope`，无法安全获得独立manager所需delegated cgroup，故真实activation、cgroup与unit lifecycle保持`BLOCKED`。`fix/stream-request-facts@4fa7a87728376f14bd84b4b5853f8212d5bc786b`是current main上的唯一major新线；clean candidate已完成0-major final review并可squash，定向verification闭环由候选既有6个定向tests、全量590项回归与backup smoke支持；候选尚未main，下一门为squash／main-side复验。
- **证据时效**：持久inventory仍主要锚定`2026-08-07T07:02:27Z`。后续备用端口happy smoke在`ae84aa9…`上确认旧Bun不变；current-main R2又在`e9fb277…`上确认旧Bun同一incarnation继续持有`127.0.0.1:4141`与`[::1]:4141`、cgroup为`/init.scope`、本轮signal数为零，且收口后`4142`／`4143`均释放。该动态证据只证明各自smoke窗口内事实，未重取完整inventory资产集合。listener、PID、cgroup、unit、open fd、writer、资源与认证状态在任何后续smoke或cutover决策前都必须重取。
...
|---|---|---|---|
| P0 | 核心 Anthropic → Responses 正确服务 | `PARTIAL` | Capability、History与stream已进入current main；`e9fb277…`关键备用端口主路径R2为scoped PASS并保留唯一stream History request-facts缺口。Clean candidate `4fa7a87…`已完成0-major final review并可squash，6个定向tests、全量590项回归与backup smoke支持该缺口的定向verification闭环；尚待squash／main-side复验且未main。完整retry、quota／resident backpressure、真实socket partial-write／delivery uncertainty及完整Acceptance仍`UNVERIFIED`。 |
| P1 | 运行面 | `FOUNDATIONS_ONLY` | Systemd runtime、S3 graceful与S4 rootless installer代码均已进入`main@e9fb277…`；unit未安装。S5在当前VS Code／`/init.scope`上下文无法安全启动具备delegated cgroup的独立manager，故真实manager／cgroup／unit运行态、双fd／双栈仍`BLOCKED`或未验；下一入口为可销毁VM／container。备用端口scoped PASS不等于restart／systemd／双栈PASS；旧Bun仍持有双栈`4141`。 |
...
|---|---|---|---|---|
| Request conversion 与 route | `FOUNDATIONS_ONLY` | Capability修复已作为`main@bd86207…`进入主树并与History、stream合成；定向复核确认每次attempt的`PRE_SEND`后按resolved-model capability重新转换。Current-main备用端口R2覆盖真实non-stream与stream主路径，并实证唯一stream History request-facts缺口；`fix/stream-request-facts@4fa7a87…`clean candidate已完成`0 blocker／0 major／1 minor` final review并可squash，既有6个定向tests、全量590项回归与backup smoke支持该缺口的定向verification闭环，但尚无main-side关闭证据。 | `bridge/request`＋`pipeline/route` | 对exact `4fa7a87…`执行squash／main-side gate，且不得提前写候选已main或完整产品`PASS`；随后补thinking success、unknown capability reject、budget边界与retry后重转换的完整Acceptance正反控制。 |
| Non-stream response | `FOUNDATIONS_ONLY` | History facts已作为`main@38bb06f…`进入主树并与stream合成；真实`HistoryConsumer`的completed／failed投影及non-stream失败零成功事实已获定向复核。备用端口happy smoke的non-stream真实route通过，但完整错误、retry与usage矩阵仍未执行。 | `bridge/nonstream` | 在current `e9fb277…`上从备用端口补齐non-stream错误、retry attempt隔离、usage／History exactly-once与local fault矩阵。 |
| Stream conversion 与 strict Anthropic SSE | `PASS_CURRENT_LAYER` | Stream route已作为`main@ae84aa9…`进入主树；绑定`ae84aa9…`的定向复核为0 blocker／0 major，同提交备用端口为`PASS_HAPPY_BACKUP_PORT_SMOKE`。Current `e9fb277…`关键主路径R2进一步取得`PASS_KEY_BACKUP_PORT_SMOKE_R2_WITH_STREAM_HISTORY_FACT_GAP`并获独立复核`0 blocker／0 major／1 minor`；唯一minor是stream request fact缺失，`4fa7a87…`候选已完成0-major final review并可squash，定向verification闭环由6个定向tests、全量590项回归与backup smoke支持，但尚未main。Semantic reorder、完整terminal／usage、retry、quota／backpressure和真实socket partial-write仍未验证。 | `bridge/stream` | 对`4fa7a87…`执行squash／main-side复验；随后执行`STREAM-MERGE-05`、完整usage／terminal／History矩阵及retry／quota／partial-write的持久化正反控制，不得把scoped PASS冒充完整Acceptance。 |
...
| Readiness 项 | Current status | Current evidence | Owner | Next smoke |
|---|---|---|---|---|
| 备用端口完整候选 | `PARTIAL` | `main@ae84aa9…`已在隔离`4142` app＋`4143` fake upstream取得`PASS_HAPPY_BACKUP_PORT_SMOKE`。Current `e9fb277…`关键主路径R2取得`PASS_KEY_BACKUP_PORT_SMOKE_R2_WITH_STREAM_HISTORY_FACT_GAP`：主路径、wait／reap、cleanup、端口释放与旧Bun不变通过，唯一stream History request-facts缺口由`4fa7a87…`候选修复；该候选已完成0-major final review且定向verification闭环有6个定向tests、全量590项回归与backup smoke支持，完整候选矩阵仍未闭合。 | `runtime/launcher`＋`service-cutover/canary` | 先完成`4fa7a87…` squash／main-side复验；随后补持久化harness与semantic reorder、usage／terminal／History、retry、quota／backpressure、真实partial-write及重复停止／启动矩阵。 |
...
| 独立评审与最终授权 | `UNVERIFIED` | Capability、History、stream、S3与S4 installer代码均已main；current-main备用端口R2关键主路径为scoped PASS并保留唯一stream History request-facts缺口。Clean candidate `4fa7a87…`已完成0-major final review并可squash，6个定向tests、全量590项回归与backup smoke支持该缺口的定向verification闭环，但尚未main；S5真实manager／cgroup诊断`BLOCKED`。P0 10＋P1 8＋P2 11＋P3 12＋`cc-daemon` 2＝43行口径保持；完整retry／quota／partial-write、真实manager／cgroup／unit运行态、P2、最终unit bytes、runbook和rollback仍无最终merged-state证据。 | `review/readiness`＋用户 | 本次新bytes必须按新hash重新定向复评，旧hash verdict不能沿用。先完成`4fa7a87…` squash／main-side复验，在可销毁容器或虚拟机续诊S5，再执行其余未验证P0矩阵；任何局部PASS均不等于切换授权。 |
...
2. **下一P0 smoke仍有明确缺口**：Current `e9fb277…`关键备用端口主路径R2已范围内PASS，但保留唯一stream History request-facts缺口；clean candidate `4fa7a87…`已完成0-major final review并可squash，6个定向tests、全量590项回归与backup smoke支持该缺口的定向verification闭环，但尚待squash／main-side复验且未main。该候选收敛后，仍须执行semantic reorder、完整usage／terminal／History、retry、request／global quota与resident backpressure、真实socket partial-write／delivery uncertainty正反控制；缺任一项都不得升级完整P0。
3. **P1 runtime未闭合且S5诊断`BLOCKED`**：S3 graceful与S4 installer代码均已进入current `main@e9fb277…`；unit仍未安装。当前VS Code调用进程位于root所有且不可写的`/init.scope`，无法安全启动具备delegated cgroup的独立user manager；private control socket未出现，真实user manager／cgroup／unit、双fd／双栈activation／restart smoke均未执行。下一入口是可销毁容器或虚拟机，不是宿主user manager。旧Bun继续持有双栈`4141`；禁止stop／signal并保持`cc-daemon`不变。S7 rolling继续后置。
...
| 位置 | 怪味类型 | 处置 |
|---|---|---|
| Bridge开发线与完整产品状态 | Capability／History／stream已进main且stream主路径的范围内评审与current-main R2 smoke为绿，但唯一stream History request-facts缺口及retry、quota／backpressure、真实socket partial-write未闭合；current-layer绿灯容易被拼成“服务已完成”的假绿 | Stream主路径精确标`PASS_CURRENT_LAYER`、P0域标`PARTIAL`；`4fa7a87…`保持candidate，已完成0-major final review且定向verification闭环有6个定向tests、全量590项回归与backup smoke支持，尚待squash／main-side复验且未main，再补完整retry、容量与partial-write真实入口证据；不得外推候选或完整产品`PASS`。 |
| `src/app/cli.py`、current-main systemd runtime与installer | 应用host／port owner、单fd模板、S3 graceful、S4 rootless render／install和目标双栈socket activation分属不同层 | S3与S4 installer代码均已进入`main@e9fb277…`；仍须备用端口真实双具名fd／manager／cgroup／unit smoke，不得把installer代码升级为安装态或生产运行态。 |
...
**实时结论：`NO_CUTOVER／PARTIAL`。Capability、History、stream、S3 graceful与S4 rootless installer代码均已进入current `main@e9fb277…`。Current-main关键备用端口R2取得`PASS_KEY_BACKUP_PORT_SMOKE_R2_WITH_STREAM_HISTORY_FACT_GAP`并获独立复核`0 blocker／0 major／1 minor`；唯一stream History request-facts缺口正由clean candidate `4fa7a87…`修复，该候选已完成`0 blocker／0 major／1 minor` final review并明确可以squash，6个定向tests、全量590项回归与backup smoke支持该缺口的定向verification闭环，但尚待squash／main-side复验且未main。完整retry、request／global quota与resident backpressure、真实socket partial-write／delivery uncertainty继续`UNVERIFIED`。Unit未安装；S5真实user manager／cgroup／unit诊断因当前VS Code／`/init.scope`上下文无法安全启动具备delegated cgroup的独立manager而`BLOCKED`，下一入口为可销毁容器或虚拟机，S7 rolling后置。P0保持`PARTIAL`，P1保持`FOUNDATIONS_ONLY`；旧Bun incarnation与双栈`4141`边界保持，`cc-daemon`继续禁止触碰。43行口径保持为P0 10＋P1 8＋P2 11＋P3 12＋`cc-daemon` 2。本文继续living、不收口；本轮新hash须后续重新复评，所有绑定旧hash的current复评均失效。在P0～P3 required行闭合前，不替代当前`copilot-api-js`。**
