...
- **作者基线**：`/home/xp/src/ghc-api-proxy-py` 的 current `main@cf53334a10a717a3a3d30d6c0e8a297f5000d90c`。本轮每次 shell 调用均在同一次调用内验证物理仓库根、当前目录、`main` 分支与 `HEAD == refs/heads/main`。Current main 已线性包含 reasoning cardinality `d274f584219f8ae32f59d15d08ac007c45058c8d`、session liveness `798ba3e7653b513c3c9c732019e793f828ae0890`、request conversion `1c13fda4f5eac5e42ca0025d503f91eb0563f0e7` 与 systemd runtime `cf53334a10a717a3a3d30d6c0e8a297f5000d90c`；这只证明相应仓库 foundations／runtime bytes 已进入 `main`，不表示完整 P0 bridge、安装态、真实 manager 运行态或 cutover 已完成。
- **当前输入身份与状态**：current cutover inventory SHA-256 `1f72038de99bbddc4a3c71243d5e20ce8d0db8783afe6d49dcf46f17e63481d9`；service cutover living Plan 为本轮current bytes，其上一稳定 bytes `ab840f2a…` 的 Plan R2 为 `0 blocker／0 major／0 minor`，current新bytes仍待联合复评；bridge Spec 为 `FINALIZED@5e3628226238a2c271824bc47d0f2fd67db9a6eb36224ee088984c96eb62a5f1`；bridge Acceptance 为 `FINALIZED_ACCEPTANCE_ORACLE@224b020d30059b899bbdc2571af0ebd199f061df2288e5c202f8cd264e9c76f4`；bridge Implementation 是持续更新而不收口的 living document，current SHA-256 `4ace302283e2b4b539c8195e55a2a23020f692d40fe0df868546dd58312036e2`。Implementation current bytes已消费上一checkpoint评审点名的状态漂移并继续等待自身定向复评，不能把Implementation当成finalized产品证据。Foundations 三片现由 `d274f58… → 798ba3e… → 1c13fda…` 承载于 current main，但完整 P0 bridge 仍缺 non-stream response、stream parser、route wiring、block sink／delivery frontier与 bridge-aware retry 接缝。Systemd source `49fb1988621bba4356e7a5039a6994c2e6d19604`的code R4为`0 blocker／0 major`，其仓库 runtime现由`main@cf53334…`承载；尚未安装 unit，也未执行真实 user manager／cgroup验收。Inventory、Plan与Implementation是各自current工作树内容身份；不得用current HEAD冒充它们的blob身份，也不得把Spec／Acceptance定稿、Implementation局部状态、foundations进入main、systemd R4或`cf53334…`外推为候选产品`PASS`、已安装、已部署或cutover获授权。
...
|---|---|---|---|
| P0 | 核心 Anthropic → Responses 正确服务 | `FOUNDATIONS_ONLY` | Foundations 三片已由`d274f58… → 798ba3e… → 1c13fda…`进入main；完整产品仍缺non-stream response、stream parser、route wiring、block sink／delivery frontier与bridge-aware retry，继续为`UNVERIFIED`。 |
| P1 | 运行面 | `FOUNDATIONS_ONLY` | Systemd runtime已由`cf53334…`进入main；unit尚未安装，真实manager／cgroup、目标user unit、双fd／双栈、备用端口、真实graceful与前门兼容仍未通过，current inventory中的双栈`4141`仍由旧Bun持有。 |
...
|---|---|---|---|---|
| Request conversion 与 route | `FOUNDATIONS_ONLY` | Reviewed request converter现由`main@1c13fda…`承载，reasoning cardinality与session liveness前置也已进入main；尚未完成真实route wiring，且未证明approval后prepare、每attempt转换或capability真值表的完整入口行为。 | `bridge/request`＋`pipeline/route` | 在current main上先补route wiring并绑定唯一pipeline owner，再从备用端口真实`/v1/messages`覆盖Responses-only、Messages-only、双支持默认Messages、显式Responses override、unknown fail-closed；捕获最后一跳Responses wire，并执行错误route单缺陷控制。 |
...
| Reasoning／carrier／multi-item／echo | `FOUNDATIONS_ONLY` | Reasoning baseline、cardinality correction与request decoder现已由`ed77c9d…`、`d274f58…`与`1c13fda…`进入main。Carrier v2定向复评R2绑定`feat/reasoning-carrier-v2@8301ee938601ad86c7f72d313abc6c976a74b2a9`，确认direct Messages final-wire strip major关闭且Responses leg未误剥，结论为0 blocker／0 major、可squash；该候选仍未进入`main`，完整non-stream／stream一item一block、encrypted-only no-loss和真实echo未验收。 | `bridge/reasoning` | 在含完整non-stream／parser／route wiring的同一候选上执行项目主v1 producer exact vector、项目／upstream v1 consumer vectors、encrypted-only、多item、authoritative`.done`、strip、unknown／foreign／malformed最小止血与client echo；producer-only／consumer-only变异必须分别有效。 |
...
| 独立评审与最终授权 | `UNVERIFIED` | Service cutover Plan上一稳定bytes的R2为`0 blocker／0 major`；联合文档R2随后对Plan／Readiness状态链给出唯一major，本次readiness新bytes已同步foundations三片与systemd runtime进入current main，同时保留完整P0、安装态与真实manager缺口；关闭情况仍须current-byte复评。`REL-06`行级门与43行口径保持；P0完整candidate、P1 runtime、P2 disposition、unit bytes、runbook和rollback evidence均无最终merged-state评审。 | `review/readiness`＋用户 | 先定向复评本次Readiness新bytes；随后每个阶段证据更新后继续复评，P0～P3合并态再做独立对账。0 blocker／0 major只表示技术门可继续，不等于生产切换授权；实际切换必须由用户对当次动作明确授权。 |
...
1. **P0组合候选缺失**：Foundations已由`d274f58… → 798ba3e… → 1c13fda…`进入current main；下一步只在该基座上收敛non-stream response、stream parser、route wiring、block sink／delivery frontier与bridge-aware retry，再组合carrier及其余lifecycle接缝形成一个完整候选。在这些接缝闭合前P0保持`FOUNDATIONS_ONLY`。
2. **P0真实证据缺失**：完整候选必须先在隔离数据与备用端口执行Acceptance required gates、真实入口、正反控制、live／capture／fault；不以局部测试替代。
3. **P1 runtime未闭合**：Systemd runtime已由`cf53334…`进入main，但unit尚未安装，真实manager／cgroup、user unit适配、双fd／双栈、readiness与graceful仍未验收。下一步先做current-main unit安装前检查，再在隔离真实user manager与备用端口执行activation／restart／cgroup smoke；始终保持旧`4141`与`cc-daemon`不变，不把隔离验收外推为生产安装。
...
| `src/app/cli.py`与current-main systemd socket runtime | 应用host／port owner、单fd模板与目标双栈socket activation合同错位 | P1要求备用端口真实双具名fd smoke；不得以代码进入main、unit静态检查或单IPv4 listener升级。 |
