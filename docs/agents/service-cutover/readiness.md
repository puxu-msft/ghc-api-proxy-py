...
- **当前总状态**：**`NO_CUTOVER／FOUNDATIONS_ONLY`**。Current main 已包含 foundations、happy pure-path primitives、non-stream usage与systemd runtime。Bridge successor `integrate/260807-bridge-successor@c43db35a7a5851225b55ce31b8edbec2cf90917f` 已绑定 semantic、route、block 三个线性 integration commits，取得 merged-state code review `0 blocker／0 major／0 minor`与独立 scoped verification `PASS`；形成本文 checkpoint 后，可按 semantic `04bdfcb…` → route `088d66d…` → block `c43db35…` 逐片回放 main并逐片执行 main-side gate。旧 bridge-next `a23081c…` 仍不得回放。Successor 的真实 non-stream ASGI路径与 typed stream reject已验，但 Responses parser／delivery 尚未接入真实ASGI streaming transport，完整 stream、bridge-aware retry及完整 P0 正确服务证据仍未闭合；P1运行面、P2数据／认证 disposition与P3 cutover／rollback／observation也未完成。
...
- **作者基线**：`/home/xp/src/ghc-api-proxy-py` 的 current `main@80bc8f252b46c511f428af1d97159a5980ee9dc9`。本轮每次 shell 调用均在同一次调用内验证物理仓库根、当前目录、`main` 分支、`HEAD == refs/heads/main` 与该精确提交。Current main 已线性包含 foundations `d274f584219f8ae32f59d15d08ac007c45058c8d → 798ba3e7653b513c3c9c732019e793f828ae0890 → 1c13fda4f5eac5e42ca0025d503f91eb0563f0e7`、systemd runtime `cf53334a10a717a3a3d30d6c0e8a297f5000d90c`、happy pure-path `a0d807fe807629b739ab16c5463f99bc27bc7aac → cdc080e1795ee1ac63d589ee00a10acd581b460e → a815948ef1b8e739e4bd49e31894be4dffc06950 → d913a033252693022f0871f1e92c1b996d05eb71` 与 non-stream usage `80bc8f252b46c511f428af1d97159a5980ee9dc9`；这些仓库 bytes 进入 `main` 仍不表示完整 P0 bridge、安装态、真实 manager 运行态或 cutover 已完成。
- **当前输入身份与状态**：current cutover inventory SHA-256 `1f72038de99bbddc4a3c71243d5e20ce8d0db8783afe6d49dcf46f17e63481d9`；service cutover Plan 与 bridge Implementation 均以读取时的 current living 内容为准；bridge Spec 为 `FINALIZED@5e3628226238a2c271824bc47d0f2fd67db9a6eb36224ee088984c96eb62a5f1`。Bridge Acceptance 的 empty-reasoning 修订候选 `a4b9e31fd1d237ca8038573320809305e0ac567eb2d56d5c967716cc8cdbfac8` 已取得 `0 blocker／0 major／0 minor` 定向复评；仅同步状态／评审 provenance 后，current Acceptance 已恢复为 `FINALIZED_ACCEPTANCE_ORACLE@6457b896ff8ae2f865e7d92443cfe893504b5757b482b4fbe61174072ff3f001`。该 oracle checkpoint 不改变产品 `UNVERIFIED`。Foundations、happy四片、usage与systemd runtime均已进入`main@80bc8f2…`，但完整产品继续为`UNVERIFIED`。P0 successor `integrate/260807-bridge-successor@c43db35a7a5851225b55ce31b8edbec2cf90917f` 由 semantic `04bdfcbf75bfa7e9709d55869c70106c49146db6`、route `088d66d3f12bd39be7ce7f61877336f490e7dbdb`、block `c43db35a7a5851225b55ce31b8edbec2cf90917f` 三个线性 commits组成；其 source分别精确对应 `f5bca39ac582911b61d278fd678ec9298ad0c08e`、`dd376d6f1e9dc2997bc2f95d03a352fed4df1412`、`e506bf87318424e4075b6422772ee0c7e9b8694a` 的完整 reviewed范围。Current successor 已取得 merged-state code review `0 blocker／0 major／0 minor`与独立 scoped verification `PASS`，可在本文 checkpoint 后按 semantic → route → block逐片回放main并逐片执行main-side gate；旧 `integrate/260807-bridge-next@a23081c5d5f48143bf3015182d8f00e1f6297755` 仍不得回放。Scoped `PASS`仅覆盖真实ASGI non-stream、typed stream reject与parser→delivery typed core；Responses parser／delivery尚未接入真实ASGI streaming transport，完整stream继续为`UNVERIFIED`。P1 rebuilt code-only integration `integrate/260807-systemd-code-only@2ec0cb81832691685bfe8d98ad03071d2d5e5316` 由 `862f4cfa55b124ef9ad21ff2ded2b944ee3307bc → 2ec0cb81832691685bfe8d98ad03071d2d5e5316` 两个线性 commits组成，明确排除 `docs/agents/systemd-runtime/plan.md`；其merged-state review为`0 blocker／0 major`、独立验收`PASS`。Acceptance／Implementation／Readiness／systemd Plan四文档形成checkpoint后，可按该顺序逐片回放main并逐片执行main-side gate；每片后fresh更新并checkpoint living Plan，代码提交不得覆盖Plan。成功后reviewed source archive targets固定为graceful `865a5b71210e2436b36786b5de67146939d1e0f5`与installer `e16c2a700f23f66535e7347ab7357518eb8e56bd`。该integration尚未回放，systemd unit仍未安装，也未执行真实user manager／cgroup smoke。各局部verdict均不证明完整route、stream、真实manager或安装态。Inventory、Plan、Implementation与候选提交是不同身份层；不得把Spec／Acceptance定稿、living状态、局部`PASS`、候选提交存在、局部review或systemd runtime进入main外推为完整产品`PASS`、已安装、已部署或cutover获授权。
...
|---|---|---|---|
| P0 | 核心 Anthropic → Responses 正确服务 | `FOUNDATIONS_ONLY` | Successor `c43db35…` 已把 reviewed semantic／route／block完整范围组成三个线性 commits，取得 merged-state review `0 blocker／0 major／0 minor`与独立 scoped verification `PASS`；可在本文 checkpoint 后按 semantic `04bdfcb…` → route `088d66d…` → block `c43db35…`逐片回放main。完整stream生产接缝与bridge-aware retry仍未接入，scoped `PASS`不得外推，故完整产品继续为`UNVERIFIED`。 |
| P1 | 运行面 | `FOUNDATIONS_ONLY` | Systemd runtime已由`cf53334…`进入current main但unit未安装；rebuilt code-only integration `862f4cf… → 2ec0cb8…`已获merged-state review `0 blocker／0 major`与独立验收`PASS`，四文档checkpoint后可逐片回放，但尚未回放。Plan不在代码提交内，每片gate后须fresh更新并checkpoint。真实manager／cgroup、双fd／双栈、备用端口与前门兼容仍未通过，current inventory中的双栈`4141`仍由旧Bun持有。 |
...
|---|---|---|---|---|
| Request conversion 与 route | `FOUNDATIONS_ONLY` | Reviewed request converter与typed route policy现分别由`main@1c13fda…`与`main@d913a03…`承载，happy pure-path范围已验证。Successor `c43db35…` 已把 semantic／route／block reviewed范围组成同一 merged state，取得 code review `0 blocker／0 major／0 minor`与 scoped verification `PASS`；真实ASGI non-stream success／429、pre-attempt typed rejects、hooks `ERROR → FINALIZE`与header过滤已验。完整stream converter与ASGI block sink尚未接入，故仍不能声称完整route完成。 | `bridge/request`＋`pipeline/route` | 形成本文checkpoint后，按 semantic `04bdfcb…` → route `088d66d…` → block `c43db35…`逐片回放main，每片先重验preimage并执行对应main-side gate；三片后重跑merged-state regression。随后从备用端口真实`/v1/messages`补齐Responses-only、Messages-only、双支持默认Messages、显式Responses override、unknown fail-closed与完整stream路径，捕获最后一跳Responses wire并执行错误route单缺陷控制。 |
| Non-stream response | `FOUNDATIONS_ONLY` | Reviewed non-stream converter已作为`main@cdc080e…`进入current main，usage details也由`main@80bc8f2…`承载。Successor `c43db35…` 的scoped verification已从真实ASGI入口验证Responses non-stream hook顺序、Anthropic最终body与header过滤，merged-state review为`0 blocker／0 major／0 minor`；该结果仍不覆盖完整stream、route-level refusal、unknown item、failed／incomplete、History、close与完整error lifecycle。 | `bridge/nonstream` | checkpoint后按 semantic → route → block逐片回放并完成main-side gate；随后从真实HTTP入口补齐refusal、unknown item、failed／incomplete、History、close与完整error lifecycle，由官方SDK／独立schema消费，并注入identity、unknown-item和close缺陷。 |
| Stream conversion 与 strict Anthropic SSE | `FOUNDATIONS_ONLY` | Successor `c43db35…` 已组合 reviewed semantic parser、route与typed block delivery，merged-state review为`0 blocker／0 major／0 minor`，scoped verification覆盖typed stream pre-attempt reject、parser unknown／error typed facts、delivery连续block顺序与terminal gate。真实`/v1/messages` Responses stream仍明确typed reject；parser／delivery尚无route／pipeline production caller，尚未接入真实ASGI streaming transport，故完整stream继续为`UNVERIFIED`。 | `bridge/stream` | checkpoint后按 semantic `04bdfcb…` → route `088d66d…` → block `c43db35…`逐片回放并完成main-side gate；随后把parser、sequencer、renderer、block sink与route接成同一真实stream生产路径。从真实备用端口消费随机rechunk的Responses SSE／WS，按 `CAL-04-GRAMMAR-v1` 和固定Anthropic SDK双oracle覆盖thinking、tool、text、usage与terminal，并执行missing start、index gap、duplicate terminal、error后content等单缺陷控制。 |
| Block-level buffering／delivery frontier／resident budgets与backpressure | `UNVERIFIED` | Spec／Acceptance已冻结完整Anthropic content block为最小下游提交单元，并由 `REL-06` 冻结普通 per-request aggregate＋global resident reservation、有限queue、charge-before-read、两级可观测计账、capacity／deadline／cancel终态与拒绝新admission；这些容量合同当前均为 `UNVERIFIED`。Successor `c43db35…` 已纳入 block reviewed范围，merged-state review为`0 blocker／0 major／0 minor`，scoped verification通过parser→delivery、multi-part／zero-block／later-item／typed terminal与single-writer矩阵；但typed core尚未接入真实ASGI sink，也仍未证明delayed-start、sink partial-write、delivery-uncertain、请求级／全局配额或真实backpressure。 | `delivery/block-buffer`＋`pipeline/sink`＋`runtime/memory-quota`＋`pipeline/admission` | checkpoint后按 semantic → route → block逐片回放并完成main-side gate，再接入真实route／ASGI sink。以真实并发reader、慢consumer和loopback socket证明首个完整block前零success headers／body；覆盖 `A.start → B.done → A.done`、同item多content parts、open-source terminal、block批提交、多个byte offset短写／RST及uncertain后禁止重发／success terminal。另以配置化 `0 < request_budget < global_budget` 聚合同一请求的draft、completed queue、预渲染envelope、carrier与History移交等多个resident owner，覆盖有限queue、charge-before-read、drain后容量恢复、各终态release归零、global压力下拒绝新admission且已接纳请求继续受控；分别注入global-only和single-block／16 MiB分支两种单侧缺陷，禁止spill或live forwarding。 |
| Retry ownership 与 pre／post-commit frontier | `UNVERIFIED` | 现有pipeline有retry基础，Acceptance已冻结唯一owner、pre-commit可重试、post-commit禁full replay。Successor `c43db35…` 已把semantic／route／block组成同一reviewed候选并取得scoped `PASS`，关闭旧bridge-next的pre-attempt hooks终结缺陷；但当前stream仍typed reject，尚无完整bridge route-level attempt reset、真实exchange计数和post-commit partial failure证据。 | `pipeline/retry`＋`delivery/frontier` | checkpoint后按 semantic → route → block逐片回放并完成main-side gate；接入真实stream生产路径后，以Fake upstream＋真实socket分别在headers前、首block未完成、首block已提交后注入reset／clean EOF／converter error，断言真实exchange数＝attempts、失败代全reset、已提交prefix不重复且post-commit不透明重放。 |
| Usage 与 stream／non-stream等价 | `FOUNDATIONS_ONLY` | Non-stream usage details已作为`main@80bc8f2…`进入current main，happy pure-path对固定算式与future details保留取得范围内`PASS`；尚无同一完整候选上的stream terminal、HTTP／WS parity、失败attempt隔离和History／token observer证据，不得把pure-path结果外推为route-level等价。 | `bridge/usage`＋`lifecycle/tokenization` | 同一语义fixture分别走non-stream、HTTP SSE与WS，比较归一化usage；注入重复terminal usage、失败attempt计费、cache算术与reasoning归类缺陷，并核对History和calibration只采纳最终成功attempt。 |
...
| User systemd socket activation | `FOUNDATIONS_ONLY` | Reviewed systemd runtime现由`main@cf53334…`承载，仍是system-level单socket模板；unit尚未安装，也未执行真实user activation。Rebuilt code-only integration `integrate/260807-systemd-code-only@2ec0cb81832691685bfe8d98ad03071d2d5e5316` 已获merged-state review `0 blocker／0 major`与独立验收`PASS`；其dry-run、apply、幂等、零`systemctl`、systemd parser与timeout合同已验证，Plan bytes相对base保持不变，但真实manager activation、effective cgroup、双fd／双栈和生产运行态仍未验证。 | `runtime/systemd` | Acceptance／Implementation／Readiness／systemd Plan四文档形成checkpoint后，按`862f4cfa… → 2ec0cb8…`逐片回放main并执行main-side gate；每片后fresh更新并checkpoint Plan，代码提交不得覆盖Plan。两片成功后将reviewed source归档为graceful `865a5b7…`与installer `e16c2a7…`。随后才在隔离真实user manager和备用端口验证user`.socket`／`.service`／`.slice`、`Accept=no`、具名双inherited fds、activation、restart、真实manager／cgroup与错误fd拒绝。记录unit bytes与manager身份，不安装或启动production unit，不操作`4141`。 |
...
| Cgroup／resource limits | `FOUNDATIONS_ONLY` | `main@cf53334…`已包含`.slice`声明与静态回归；current Bun owner在inventory时位于`/init.scope`。尚无真实user manager中同一候选的effective limits、MemoryHigh、OOM、TasksMax和cleanup证据，模板进入main不得冒充内核已采用。 | `runtime/cgroup` | 在隔离真实user manager的备用user slice运行current-main实例，交叉核对unit声明、systemd effective属性与`/proc/<pid>/cgroup`；只制造受控memory／task压力，证明限制仅作用于代理service、退出后cgroup归零且不传播到其他unit或`cc-daemon`。 |
| Graceful shutdown | `FOUNDATIONS_ONLY` | `src/app/server.py`已有lifespan cleanup，局部liveness覆盖一部分cancel cleanup；rebuilt code-only integration `862f4cf… → 2ec0cb8…`已获merged-state review `0 blocker／0 major`与独立验收`PASS`，固定application `300s`／manager `330s`合同并验证短timeout SIGTERM后lifespan cleanup。该integration尚未回放，且局部PASS不证明真实user-manager SIGTERM、长SSE／WS、pending approval、tokenization flush、History close或force timeout。 | `runtime/shutdown`＋`pipeline/lifecycle` | 四文档checkpoint后按`862f4cfa… → 2ec0cb8…`逐片回放并执行main-side gate，每片后fresh更新并checkpoint Plan且不得由代码提交覆盖；随后对隔离备用user service执行真实manager SIGTERM：停止application admission，短请求drain，长SSE／WS按合同终止，approval拒绝，History／tokenization／upstream／background task归零；再验证`TimeoutStopSec`与`KillMode=control-group`强制路径。 |
...
| 独立评审与最终授权 | `UNVERIFIED` | Deployment living docs既有评审只绑定各自稳定bytes；本次readiness新bytes同步current `main@80bc8f2…`与latest successor证据，同时保留完整P0、安装态与真实manager缺口。Bridge successor `c43db35…` 已取得merged-state code review `0 blocker／0 major／0 minor`与独立scoped verification `PASS`，明确允许按 semantic `04bdfcb…` → route `088d66d…` → block `c43db35…`逐片回放main；旧bridge-next `a23081c…`仍不可回放。Scoped `PASS`不覆盖真实ASGI streaming transport，完整stream继续为`UNVERIFIED`。Systemd rebuilt code-only `862f4cf… → 2ec0cb8…`为merged-state review `0 blocker／0 major`、独立验收`PASS`，Plan未被提交覆盖；四文档checkpoint后可逐片回放，每片后须fresh更新并checkpoint Plan，成功后archive targets为`865a5b7…`／`e16c2a7…`。Acceptance empty修订候选`a4b9e31f…`已获0 blocker／0 major定向复评，current状态／provenance已恢复为`FINALIZED_ACCEPTANCE_ORACLE@6457b896…`；产品仍为`UNVERIFIED`。`REL-06`行级门与P0 10＋P1 8＋P2 11＋P3 12＋`cc-daemon` 2＝43行口径保持；P0完整stream、P1真实runtime、P2 disposition、最终unit bytes、runbook和rollback evidence均无最终merged-state评审。 | `review/readiness`＋用户 | 先定向复评本次Readiness新bytes；四文档形成checkpoint后，bridge按 semantic → route → block逐片回放并执行main-side gate；systemd按`862f4cfa… → 2ec0cb8…`逐片回放并执行main-side gate，每片后fresh更新并checkpoint Plan且不得由代码提交覆盖。Bridge successor与systemd code-only的局部放行只覆盖各自声明范围，均不得外推为完整stream产品、安装态、真实manager或P1通过。随后每个阶段证据更新后继续复评，P0～P3合并态再做独立对账。0 blocker／0 major只表示技术门可继续，不等于生产切换授权；实际切换必须由用户对当次动作明确授权。 |
...
1. **P0 successor可回放但完整stream仍缺失**：Bridge successor `c43db35…` 已把semantic／route／block reviewed范围组成三个线性commits，取得merged-state code review `0 blocker／0 major／0 minor`与独立scoped verification `PASS`。形成本文checkpoint后，按 semantic `04bdfcb…` → route `088d66d…` → block `c43db35…`逐片回放main，每片先重验preimage并执行main-side gate，三片后重跑merged-state regression；旧`a23081c…`不得回放。Scoped `PASS`只覆盖真实ASGI non-stream、typed stream reject与parser→delivery typed core；完整Responses stream生产接缝、block sink／delivery frontier、bridge-aware retry及其余lifecycle接缝仍未接入，P0继续保持`FOUNDATIONS_ONLY`。
2. **P0真实证据缺失**：完整候选必须先在隔离数据与备用端口执行Acceptance required gates、真实入口、正反控制、live／capture／fault；不以局部测试替代。
3. **P1 runtime未闭合**：Systemd runtime已由`cf53334…`进入current main但unit未安装；rebuilt code-only integration `862f4cfa… → 2ec0cb8…`已获merged-state review `0 blocker／0 major`与独立验收`PASS`，Plan未被提交覆盖，但两片尚未回放。真实manager／cgroup、双fd／双栈、readiness与graceful仍未验收。下一步先形成Acceptance／Implementation／Readiness／systemd Plan四文档checkpoint，再按`862f4cfa… → 2ec0cb8…`逐片回放并执行main-side gate；每片后fresh更新并checkpoint Plan，代码提交不得覆盖Plan。两片成功后将reviewed source归档到`865a5b7…`／`e16c2a7…`，再做current-main安装前检查，最后仅在隔离真实user manager与备用端口执行activation／restart／cgroup smoke；始终保持旧`4141`与`cc-daemon`不变，不把隔离验收外推为生产安装。
...
| 位置 | 怪味类型 | 处置 |
|---|---|---|
| Bridge开发线与完整产品状态 | Successor `c43db35…` 已获merged-state 0 major与scoped `PASS`，但完整stream仍未生产接线；局部绿灯容易被拼成“服务已完成”的假绿 | 本矩阵统一标`FOUNDATIONS_ONLY`；checkpoint后按 semantic → route → block逐片回放并执行main-side gate，随后把stream、retry与其余lifecycle接缝纳入同一真实入口证据；不得把scoped `PASS`外推为完整stream或完整产品`PASS`。 |
| `src/app/cli.py`、current-main systemd runtime与installer候选 | 应用host／port owner、单fd模板、rootless render／install和目标双栈socket activation分属不同层；rebuilt code-only `862f4cf… → 2ec0cb8…` review／verify PASS仍可能被误作真实manager生效 | P1在四文档checkpoint后逐片回放code-only commits并执行main-side gate，每片后fresh更新且不以提交覆盖Plan；随后再做备用端口真实双具名fd／manager smoke。不得以代码进入main、helper评审、unit静态检查或单IPv4 listener升级。 |
| Lifespan cleanup、liveness helper与graceful候选 | 清理与deadline能力分散；rebuilt code-only `862f4cf… → 2ec0cb8…`虽已获merged-state review `0 blocker／0 major`与独立验收`PASS`，仍不能证明真实user-manager SIGTERM、长连接和force timeout接缝 | P0 session liveness与P1 graceful仍分别要求真实客户端取消和真实manager SIGTERM，再由组合态验证统一owner。 |
...
**实时结论：`NO_CUTOVER／FOUNDATIONS_ONLY`。Bridge successor `c43db35…` 可在checkpoint后按 semantic → route → block逐片回放，但完整stream仍未接入；下一证据目标是在回放后接通并验证同一身份的完整备用端口候选。在P0～P3 required行按各自门闭合前，不替代当前`copilot-api-js`。**
