...
- **事实快照**：2026-08-08。主仓为 `/home/xp/src/ghc-api-proxy-py`，current `main` HEAD 为 `e9fb2771d6e040c761bb4074e3fcf2547caece28`。本轮每次 shell 调用都在同一调用内验证物理 repo root、分支 `main`、`HEAD == refs/heads/main` 与该完整 SHA；未来 `main` 前进后，living document 必须先记录新锚点并重建 gate，不得把本快照冒充未来 HEAD。
...
- **当前 foundations、happy、usage 与 bridge 主树状态**：foundations 三片、happy 四片、non-stream usage details、semantic parity、route happy、block delivery、reasoning capability、History facts 与 stream route 均已进入 `main`。在旧 `main@b91e58a…` 之后，living checkpoint `fa3fb7c…`、capability `bd86207b4fdb55b7c10c795118f61ba693192003`、History `38bb06ff0eefef69fd4fdab830e67ff549563a20` 与 stream `ae84aa9d4330e56b83aefdad977e7d93190ff0d4` 线性进入主树；三个产品切片均已完成 main-side gate 与 reviewed-source archive。Systemd S3 graceful timeout `c53849e2b5103c6426a67a8cbab687f2e45c1fa0`、post-S3 living checkpoint `3a55c091ef000d867b2f58f3afeacf60b6c1c64d` 与 S4 rootless user installer `e9fb2771d6e040c761bb4074e3fcf2547caece28` 随后进入主树，S3／S4 reviewed sources均已归档；完整产品仍为 `UNVERIFIED`，任何局部测试、评审或 smoke 不外推为完整 Acceptance `PASS`。
...
- **备用端口 happy smoke 与运行态边界**：[备用端口 smoke 执行记录](../../tmp/260807-resume-backup-port-smoke-execution.md)精确绑定`main@ae84aa9d4330e56b83aefdad977e7d93190ff0d4`，在隔离`127.0.0.1:4142` app＋`127.0.0.1:4143` fake上给出`PASS_HAPPY_BACKUP_PORT_SMOKE`。后继[current-main备用端口关键主路径smoke R2](../../tmp/260807-final-backup-port-smoke-r2.md)精确绑定`main@e9fb2771d6e040c761bb4074e3fcf2547caece28`，给出`PASS_KEY_BACKUP_PORT_SMOKE_R2_WITH_STREAM_HISTORY_FACT_GAP`；[独立快速复核](../../tmp/260807-review-final-backup-port-smoke-r2.md)确认该报告可按`0 blocker／0 major／1 minor`归纳。Non-stream、stream withholding、Responses SSE→Anthropic SSE、唯一success terminal、cancel cleanup、SIGTERM lifespan cleanup、wait／reap、备用端口释放与旧Bun不变均取得运行证据；唯一facts缺口是stream History未承载final selected attempt的typed request conversion fact，正由`4fa7a87…`候选修复。该scoped PASS不覆盖`STREAM-MERGE-05` semantic reorder、完整usage parity、failed／error／EOF与History enabled矩阵、retry、quota／resident backpressure、真实socket partial-write／RST、完整shutdown竞态、R3持久化harness／safety tests或完整Acceptance。部署保持`NO_CUTOVER`。
- **当前唯一 major 新线**：`fix/stream-request-facts`／`/home/xp/src/ghc-api-proxy-py-stream-facts`已从current `main@e9fb277…`形成clean candidate `4fa7a87728376f14bd84b4b5853f8212d5bc786b`，用于收敛R2唯一stream History request-facts缺口。只读预审提出的唯一争议已由[Spec窄裁决](../../tmp/260807-arbitrate-stream-request-facts.md)判为保持候选：completed、post-commit partial failure与delivery-uncertain均保留最终selected attempt的request conversion observations；[独立定向终审R2](../../tmp/260807-review-stream-request-facts-r2.md)已对exact HEAD给出`0 blocker／0 major／1 minor`并明确可以squash。候选既有6个定向tests、全量590项回归与current-main备用端口smoke共同支持该request-facts缺口的定向verification闭环，但不构成候选完整产品或完整Acceptance `PASS`。候选尚未进入main；下一动作是先形成living checkpoint，再执行squash／main-side gate。其余capability、History、stream happy、S3与S4旧major均已main并归档，不重复开线。
- **当前 merged-state 结论**：Capability `bd86207…`、History `38bb06f…`、stream `ae84aa9…`、S3 `c53849e…`与S4 `e9fb277…`均已依次进入main并归档reviewed source。上述证据只覆盖各自声明范围；完整产品继续`UNVERIFIED`，部署继续`NO_CUTOVER`，Implementation继续`LIVING`且不收口。
...
| Post-S3 checkpoint后主线收敛 | **已完成。** Capability、History、stream、S3与S4分别作为`bd86207…`、`38bb06f…`、`ae84aa9…`、`c53849e…`与`e9fb277…`进入main；五片reviewed source均已归档，post-S3 Implementation bytes以`3a55c09…`形成living checkpoint | Current不再重复上述squash或S4 gate；备用端口R2关键主路径范围内PASS，S5真实manager／cgroup诊断`BLOCKED`，stream request facts clean candidate `4fa7a87…`已完成0-major final review并可squash，下一动作是living checkpoint后执行squash／main-side gate。完整产品仍`UNVERIFIED`，部署仍`NO_CUTOVER` |
| Backup happy smoke执行 | **范围内通过。** `main@ae84aa9…`取得`PASS_HAPPY_BACKUP_PORT_SMOKE`；后继`main@e9fb277…`取得`PASS_KEY_BACKUP_PORT_SMOKE_R2_WITH_STREAM_HISTORY_FACT_GAP`并获独立复核`0 blocker／0 major／1 minor` | 唯一facts缺口正由`4fa7a87…`候选修复；不升级为R3全量入口或完整产品PASS，semantic reorder、完整usage／terminal／History矩阵、retry、quota／backpressure、真实partial-write、in-flight shutdown、持久化harness／变异控制与完整Acceptance继续未验证 |
...
| 21 | Stream request facts major | `fix/stream-request-facts@4fa7a87728376f14bd84b4b5853f8212d5bc786b`；worktree `/home/xp/src/ghc-api-proxy-py-stream-facts`；base `e9fb2771d6e040c761bb4074e3fcf2547caece28` | **Clean candidate已完成exact-HEAD final review：`0 blocker／0 major／1 minor`，明确可以squash；既有6个定向tests、全量590项回归与backup smoke支持request-facts缺口的定向verification闭环；尚未main** | 先形成living checkpoint，再执行squash／main-side identity、preimage、声明范围与回归gate；不得外推为完整产品或完整Acceptance `PASS` |

Foundations、happy、usage、semantic、route、block、capability、History、stream、systemd S3与S4均已进入current `main@e9fb2771d6e040c761bb4074e3fcf2547caece28`，对应reviewed sources均已归档。Current-main备用端口R2关键主路径已取得带唯一stream History request-facts缺口的scoped PASS；S5真实manager／effective cgroup诊断已明确`BLOCKED`并须换到受控容器或虚拟机继续；stream request facts是当前唯一major新线，clean candidate `4fa7a87…`已完成`0 blocker／0 major／1 minor` final review并可squash，既有6个定向tests、全量590项回归与backup smoke支持该缺口的定向verification闭环。下一动作是living checkpoint后执行squash／main-side gate；以上进展都不能外推为部署、候选已main或完整产品`PASS`。
...
| Rootless user install（已收敛） | reviewed source `e16c2a700f23f66535e7347ab7357518eb8e56bd`；历史 rebuilt candidate `d3fabfadfba57af6c2d63e543e3198444777df54` | 历史 candidate parent `8cae6c260c8bc2930be96eaecc7d6d24d470e00a` | main `e9fb2771d6e040c761bb4074e3fcf2547caece28`；main-side gate通过；archive `archive/260807-systemd-user-install@e16c2a7…` | 保持archive immutable；不重复S4回放。S5首轮诊断已`BLOCKED`，下一systemd动作是在受控容器或虚拟机续诊 |
| S5隔离user-manager／effective cgroup验证 | [执行记录](../../tmp/260807-systemd-user-manager-smoke.md)绑定current `main@e9fb277…` | current `main@e9fb2771d6e040c761bb4074e3fcf2547caece28` | **`BLOCKED`；独立manager未创建private control socket，真实activation／effective cgroup未执行；未安装、未部署、未cutover** | 在可销毁systemd-nspawn容器或虚拟机中继续诊断，要求独立login session／user manager与delegated cgroup v2；不得退回宿主user manager |
| Stream request facts major（当前唯一major新线） | `/home/xp/src/ghc-api-proxy-py-stream-facts`；`fix/stream-request-facts@4fa7a87728376f14bd84b4b5853f8212d5bc786b` | current `main@e9fb2771d6e040c761bb4074e3fcf2547caece28` | **Clean candidate已完成exact-HEAD final review：`0 blocker／0 major／1 minor`，明确可以squash；6个定向tests、全量590项回归与backup smoke支持request-facts缺口的定向verification闭环；未进入main** | 先形成living checkpoint，再执行squash／main-side gate；不重复已main的stream happy source，不把定向闭环外推为完整产品`PASS` |
...
| Systemd next integration（历史） | `/home/xp/src/ghc-api-proxy-py-integrate-systemd-next`；`integrate/260807-systemd-next` | `80bc8f252b46c511f428af1d97159a5980ee9dc9` | 旧 HEAD `0a93e7f18f197bf8a2395eaaf20afda446f92d6b` 与 `91f95f7… → 0a93e7f…` 保留 merged review／verify／adaptation provenance；其 Plan patch 已被 current living bytes 超越 | 不回放、不续写、不cherry-pick，不以old Plan postimage解冲突；S5只从current `main@e9fb277…`执行 |

Capability `bd86207…`、History `38bb06f…`、stream `ae84aa9…`、S3 `c53849e…`与S4 `e9fb277…`均已进入current main并归档reviewed source。S5真实user-manager／effective cgroup、完整stream未验证矩阵、部署与cutover均未完成。任何代码收敛都不授权停止现服、安装生产unit或抢占`4141`。

### 增量开发风格

当前开发节奏明确采用：**先建立结构骨架，完成 happy path 与 smoke，形成单一可评审提交；独立 review 通过后尽快 squash／归档；随后按 Spec／Acceptance 继续补齐边界、失败路径、正反控制与组合态接缝。** Capability、History、stream、S3与S4已完成该轮收敛。Current并行事实是：current-main备用端口R2关键主路径已范围内PASS且唯一facts缺口正由clean candidate `4fa7a87…`修复；该候选已完成`0 blocker／0 major／1 minor` final review并可squash，既有6个定向tests、全量590项回归与backup smoke支持该缺口的定向verification闭环，下一动作是living checkpoint后执行squash／main-side gate；S5真实manager／cgroup诊断`BLOCKED`并转入受控容器或虚拟机续诊。产品在完整required gates通过前保持`UNVERIFIED`。
...
| 本文 | [Implementation post-S3 定向复评](../../tmp/260807-resume-review-implementation-post-s3.md)精确绑定 SHA-256 `0a99620dc352b1cdec788c0a67d1b476df67926ae598970994649a69617e043d` 与`main@c53849e…`并给出0 blocker／0 major／0 minor；对应bytes已作为`3a55c09…`checkpoint | 此后S4 `e9fb277…`进入main并归档，current-main备用端口R2取得带唯一stream History request-facts缺口的scoped PASS；`4fa7a87…`已完成0-major final review并可squash，定向verification闭环由候选既有6个定向tests、全量590项回归与backup smoke支持。本轮同步这些状态，并保持完整产品`UNVERIFIED`、部署`NO_CUTOVER`、本文`LIVING` | 本轮新bytes须按新hash重新定向复评并形成living checkpoint；该复评不表示文档收口、`4fa7a87…`已main、S5已完成、完整产品`PASS`或部署获授权 |
...
3. Spec current hash仍为`5e362822…`；Acceptance current hash为`6457b896…`、状态为`FINALIZED_ACCEPTANCE_ORACLE`且R2定向复评为0／0／0。Foundations、happy、usage、bridge后继与systemd S3／S4回放已经完成；current-main备用端口R2已范围内通过，`4fa7a87…`已完成0-major final review并由6个定向tests、全量590项回归与backup smoke支持request-facts缺口的定向verification闭环，当前下一门是living checkpoint后的squash／main-side gate、其余stream未验证矩阵与S5隔离manager／cgroup验证。旧living-index阻断、integration audit与已完成候选回放清单只作历史证据。
...
5. Graceful-timeout S3已作为`c53849e…`进入main并归档，user-install S4已作为`e9fb277…`进入main并由`archive/260807-systemd-user-install@e16c2a7…`归档；不得再次回放`8cae6c2… → d3fabfa…`、old-base链或source S4 patch。S5必须从current main在隔离manager边界执行，不得把candidate-side或仓库gate冒充真实运行态证据。
6. Current产品执行面包含三条未闭合线：current-main备用端口R2关键主路径已范围内PASS，但唯一stream History request-facts缺口尚待候选进入main且其余完整矩阵未闭合；S5真实manager／effective cgroup诊断`BLOCKED`并须转到受控容器或虚拟机；stream request facts是唯一major新线，clean candidate `4fa7a87…`已完成0-major final review并由6个定向tests、全量590项回归与backup smoke支持该缺口的定向verification闭环，下一动作是living checkpoint后的squash／main-side gate。Semantic reorder、完整usage／terminal／History矩阵、retry frontier、quota／resident backpressure、真实socket partial-write／RST、in-flight shutdown、持久化harness／变异控制及完整Acceptance仍未验证。任何局部review、裁决、阶段`PASS`、smoke或归档都不改变产品`UNVERIFIED`或部署`NO_CUTOVER`；只有完整bridge按current Acceptance required gates取得实证，产品verdict才可升级。

### 保留评审证据与归档

- Squash 前不强制改写候选分支。若复评要求继续修复，在活动分支追加提交并对新的完整 HEAD 重新评审；旧报告永远只绑定其原 HEAD。
- 每片回放并完成 main侧验证后，为**最终独立评审通过的 pre-squash HEAD**创建 immutable archive ref。现有 reasoning archive保持指向 `d90c90d…`；foundations refs精确为 `archive/260807-anthropic-responses-reasoning-cardinality@b876e62…`、`archive/260807-anthropic-responses-liveness@f27a8c0…` 与 `archive/260807-anthropic-responses-request@fdd2f75…`；happy／usage refs精确为 `archive/260807-reasoning-carrier-v2@8301ee9…`、`archive/260807-responses-anthropic-nonstream@7ddf173…`、`archive/260807-responses-stream-parser@73a6aa1…`、`archive/260807-anthropic-responses-route-policy@84a22c0…` 与 `archive/260807-nonstream-usage-details@aca3ced…`；后继refs精确为`archive/260807-responses-semantic-parity@f5bca39…`、`archive/260807-anthropic-responses-route-happy@dd376d6…`、`archive/260807-anthropic-block-delivery@e506bf8…`、`archive/260807-responses-reasoning-capability@8bff1c3…`、`archive/260807-responses-history-facts@b1df8f9…`与`archive/260807-anthropic-responses-stream-route@f3922a9…`；systemd refs精确为`archive/260807-systemd-runtime@49fb198…`、`archive/260807-systemd-graceful-timeout@865a5b7…`与`archive/260807-systemd-user-install@e16c2a7…`。不得用integration commit替代reviewed feature HEAD。对应feature worktree／branch clean且archive ref精确后，可随该片逐片清理。
...
Current main上的foundations、systemd runtime、happy、usage、semantic、route、block、capability、History、stream、S3与S4均保持独立语义提交；出现组合回归时先定位到引入缺陷的最低共同层，再按提交依赖逆序形成显式revert，不把“最新提交”自动当作根因，也不整文件恢复相邻切片。原reviewed HEAD仍由archive ref保留；回滚只改变main上的集成提交，不移动archive ref，也不覆盖评审证据。Current锚点为`main@e9fb2771d6e040c761bb4074e3fcf2547caece28`；旧successor、旧bridge-next与旧systemd integration只保留provenance。S5运行态发现若要求代码修复，必须从current main建立新切片，不复用旧S4 candidate或integration identity。
...
1. **Living checkpoint → stream request facts squash／main-side gate**：clean candidate `4fa7a87728376f14bd84b4b5853f8212d5bc786b`的[独立定向终审R2](../../tmp/260807-review-stream-request-facts-r2.md)已给出`0 blocker／0 major／1 minor`并明确可以squash；候选既有6个定向tests、全量590项回归与current-main备用端口smoke支持request-facts缺口的定向verification闭环。下一最小动作是先让本轮Implementation bytes取得0-major定向复评并形成living checkpoint，再按current main执行candidate identity／preimage／声明范围 gate、squash与main-side回归／组合复核。该定向闭环不构成候选完整产品或完整Acceptance `PASS`，候选当前仍未进入main。
2. **继续S5隔离user-manager／effective cgroup诊断**：[本轮S5执行记录](../../tmp/260807-systemd-user-manager-smoke.md)已把真实manager／cgroup判为`BLOCKED`，因为独立`systemd --user`未创建private control socket。下一次在可销毁systemd-nspawn容器或虚拟机中提供独立login session／user manager与delegated cgroup v2后复验activation、实际unit、effective limits、restart与cleanup；不得改用宿主user manager，也不得接管生产`4141`。S7 rolling继续后置。
3. **Main-side gate后按真实缺口补其余stream边界**：只有`4fa7a87…`完成squash／main-side gate，才可写成已进入main；当前仍不得提前写已main或完整产品`PASS`。随后以[祖先main备用端口 smoke 执行记录](../../tmp/260807-resume-backup-port-smoke-execution.md)及[current-main R2](../../tmp/260807-final-backup-port-smoke-r2.md)为输入，补`STREAM-MERGE-05` semantic reorder、完整usage parity、failed／error／EOF与History enabled矩阵、retry frontier、quota／resident backpressure、slow consumer、真实socket partial-write／RST、in-flight shutdown、持久化harness／safety tests与变异控制。完整产品继续保持`UNVERIFIED`，直到current Acceptance required gates全部取得实证。
...
| `docs/agents/anthropic-responses-bridge/implementation.md` 的current identity与动作 | Route、semantic、block、capability、History、stream、systemd与Acceptance状态在顶部、处置表、进度表、开发线、收敛、下一步和结尾多点复述，main前进时容易出现弱一致性副本 | 本轮同步main`bd86207… → 38bb06f… → ae84aa9… → c53849e… → 3a55c09… → e9fb277…`、backup current-main R2 scoped PASS＋唯一facts缺口、S5真实manager／cgroup`BLOCKED`，以及stream request facts clean candidate `4fa7a87…`已完成0-major final review、定向verification闭环有6个定向tests／全量590项回归／backup smoke支持、下一动作是living checkpoint→squash／main-side gate；不得提前写候选已main或完整产品`PASS`。后续优先把顶部current-state入口作为身份真相源 |
...
| 历史 source refs与current main commits | Reviewed source identity与main squash identity不同，后续若从旧source重建会制造第二条实现链 | 最新archives保持immutable且只作provenance；current主线锚点为`e9fb277…`。S4已收敛，不再从冻结candidate、reviewed source或历史integration重建；S5发现从current main另建修复切片 |
...
Spec `FINALIZED@5e362822…`继续提供current行为oracle；Acceptance current `6457b896…`保持`FINALIZED_ACCEPTANCE_ORACLE`，本轮未改Acceptance。Capability `bd86207…`、History `38bb06f…`、stream `ae84aa9…`、S3 `c53849e…`与S4 `e9fb277…`均已进入main并归档；main stream定向review为`0 blocker／0 major`，祖先main上的backup smoke为`PASS_HAPPY_BACKUP_PORT_SMOKE`。Current-main R2关键主路径取得`PASS_KEY_BACKUP_PORT_SMOKE_R2_WITH_STREAM_HISTORY_FACT_GAP`并获独立复核`0 blocker／0 major／1 minor`；唯一facts缺口正由clean candidate `4fa7a87…`修复，该候选已完成`0 blocker／0 major／1 minor` final review并明确可以squash，6个定向tests、全量590项回归与backup smoke支持该缺口的定向verification闭环。下一动作是本轮living checkpoint后执行squash／main-side gate；候选尚未main，定向闭环不构成完整产品或完整Acceptance `PASS`。S5真实manager／effective cgroup诊断为`BLOCKED`，下一最小动作是转到受控可销毁容器或虚拟机。Stream semantic reorder、完整usage／terminal／History矩阵、retry、quota／backpressure、真实partial-write、持久化harness／变异控制及完整Acceptance仍未验证。整体保持`NO_CUTOVER`与产品`UNVERIFIED`，本文保持`LIVING`、不收口；不得提前写候选已main或完整产品`PASS`。本轮新bytes必须按新hash重新定向复评，任何绑定修订前旧hash的current复评均不得沿用。