...
| 位置 | 怪味类型 | 处置 |
|---|---|---|
| `src/app/pipeline/delivery/stream.py` 的 terminal frames 与 legacy `src/app/delivery/responses_anthropic_stream.py` 的 `not frontier.terminal_accepted` 分支 | 同一条冻结规则在两条链路上一实现一缺失。legacy 对无合法 terminal event 的 EOF 抛 `incomplete_responses_stream` 并 render SSE error；新 pipeline 链路仍 flush `message_delta{stop_reason:"end_turn"}`＋`message_stop`，把被截断的 turn 伪装成干净结束并存进客户端历史。这是**回归**，不是未开工功能，STR-04 的验收判据早已冻结 | 2026-08-20 已在 `stream.py` 就地标注 KNOWN SPEC VIOLATION 并指向 legacy 参考实现；`tests/unit/test_stream_delivery.py::test_a_truncated_stream_is_still_flushed_as_a_clean_ending_downstream` 钉住现状且在 docstring 声明「本测试的目的是被反转、不是被保留」。操作者一侧已先行止血：请求行改报 `[FAIL]` 与具体成因，见 `.dev/docs/tui/archive-truncated-stream/`（主仓提交 `15abef4`）。落地 STR-04 时从 legacy 移植而非重新设计 |
