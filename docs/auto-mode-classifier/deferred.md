...
>
> D5（配置压成标量后、两个判据字面量都无处可配的代价）已于 2026-08-23 关闭：用户裁定改回结构化配置，那个整体性的能力损失随之消失。**注意关闭理由的边界**——M2 其后按另一次独立裁定被钉死为常量，它单独的代价没有消失，见下面的 D6。结论归入 `spec.md` §5。
>
> 快照日期：2026-08-23。

## D6 — M2 没有单独关闭的开关

**状态**：已知代价，今天不需要，记下来以免日后重新发现。

`match_system_prompt_prefix` 是配置项，把它设成空字符串就能单独关掉 M1 那一条标记（`_matches_system_prompt` 的空值守卫）。M2（转录包裹）2026-08-23 降为模块常量后没有对应的开关——它要么生效，要么改代码。

**什么时候会需要它**：某个合法客户端开始把 `<transcript>\n` 用作最后一条 user 消息的首块，并且同一条消息里还有闭合块，且该请求还满足结构门槛（无 tools、非流式、无 assistant 轮）。今天没有观察到这样的客户端，且结构门槛已经把绝大多数「总结下面的转录」形态挡在外面。

**真发生了怎么办**：短期把 `_TRANSCRIPT_OPEN` 改成不可能匹配的值即可（一行），长期再考虑要不要还它一个键。

出处：`reports/260823-review-2-claude.md` minor-3。原始裁定见 `spec.md` §5「M2 是常量，不是配置项」。
