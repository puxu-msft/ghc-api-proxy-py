...
## D5 — 判据字面量随「配置项改为标量」变成了模块常量

**状态**：有意为之，记录代价。

用户 2026-08-23 在 `docs/.human-controlled/config.example.yaml` 亲笔定下这个配置项的形状：一个标量 `intercept_auto_mode_classifier: false | allow | block`，其下没有子键。实现按这份权威改写，原先的三个子键因此变成 `app/pipeline/auto_mode_classifier.py` 里的模块常量：

- `_MONITOR_PROMPT`（M1 标记）
- `_TRANSCRIPT_OPEN`（M2 标记）
- `_BLOCK_REASON`

**代价**：这两个标记是**别人程序里的字符串字面量**。Claude Code 改写分类器 prompt 的措辞时，识别会静默失效（方向安全——认不出就照常转发，只是不再省那 710 KB），而修复它现在要改代码并发一次版本，而不是改一行配置。

**为什么仍然按这个形状做**：`docs/.human-controlled/` 是用户亲笔的权威，压过我推导的设计；而且用户是本项目唯一的操作者兼开发者，「改常量」与「改配置」的差别在这个部署里比在一般产品里小得多。

**若要加回旋钮**，那是用户的裁决，不是实现可以自己决定的——本条记在这里，是为了让那一天有人能找到「当初为什么去掉」。届时 §4.1 第 1 条的大小写过滤会重新变成一条有真实输入能触达的守卫。

出处：用户对 `config.example.yaml` 的编辑（2026-08-23），以及 `spec.md` §5「判据字面量不可配置」。
