...
1. `acceptance.md` 的 `CAL-04-GRAMMAR-v1` 需升版：`ping` 转移行、冻结 fixture 集合里「不得把 `ping` 放在首批之前」那句，以及三条已闭评审行 R3-M1／R4-M1／R5-M1（R4-M1 曾采纳过与本次同向的修订、后被 R5 推翻，重做时要把这段历史读完）。该语法**目前只存在于文档，主仓测试没有实现**，不阻塞运行时。
