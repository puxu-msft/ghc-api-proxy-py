...
不在请求粒度为无痕重试设冷却间隔。在模型提供方粒度，当上游连续失败时，由该模型提供方共享的限速器做退让（`reactive_rate_limiter.failure_backoff_base_sec` 起，逐次连续失败翻倍，至 `failure_backoff_max_sec` 封顶，一次成功即清零）。驱动层重试与交付层无痕重放都经它拉开间隔，避免上游挣扎时短时间内加剧其负载。
