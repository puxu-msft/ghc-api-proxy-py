...
`restart-handover/` 末尾有一节「遗留」。其中配置示例把 pidfile 文件名写成请求端口，而实现按 bind 后的最终生效端口命名；该 human-controlled 文本须由用户更新。另有两条既有缺陷（信号处理器安装窗口、并发回滚覆盖后来者）建议各自单独立项。此前的同端口覆盖与 `--fd` 静默冲突均已实现修复，不再列为遗留。
