...
历史注记：`9b8114a` 原为 `1a7353e`，被并行会话重写（src/tests 逐字节相同）。**当时几条提交的 `Docs:` 尾注指向 `docs/tmp/260820-*.md`，那些文件现已搬到本目录 `reports/`**，对照关系见下。

## 遗留张力，交给用户裁决

- **`docs/2604-rewrite/shutdown.md` 第 17 行写「停止监听新连接（已建立的连接保留，不受影响）」，而第一档现在会主动关掉空闲的池化连接。** 那份文档描述的是**计划中的 4 阶段设计**、不是已发布的阶梯（`app.state.draining`、`drain_queue`、`ws_manager` 三个符号在 `src/` 中均不存在，2026-08-20 核过），所以它今天不是任何东西的活载体，本次也没有改它。但**将来照它实现时会与本次行为冲突**，届时要么改那一行、要么改这一档。没有替用户裁决。
