...
权威规范仍是主仓库 `docs/.human-controlled/lifecycle.md` 与 `config.example.yaml`，本目录是围绕它们的过程记录。`config.example.yaml` 的默认**目录** `$XDG_DATA_HOME/ghc-api-proxy` 已与实现一致；但其两句文件名示例仍写成 `standalone-${GHC_API_PROXY_PORT}.pid`。实现由 bind 后取得的**最终生效端口**命名（包括直接调用 `StandaloneOptions(port=0)` 时内核分配的端口），因此该示例不是精确的默认规则，仍需用户亲自改为 `standalone-<最终生效端口>.pid`——见下文「遗留」。
...
完整取证：[260822-pidfile-missing-forensics.md](history/260822-pidfile-missing-forensics.md)（含时间线、每条证据的取得方式、7 条已排除假设）。关键一环有直接一手观测：13:17:36 另一个 subagent 的脚本因为假设 pidfile 只有一行 PID 而在 `int()` 上崩了，**正因为崩了才把完整两行原样打印出来**。
...
目录语义已同步；`config.example.yaml` 仍把文件名中的端口写成配置／环境请求值，和第 3 条的“最终生效端口”不一致。这是 human-controlled 配置文档中的单独待改文字，不影响已经落地的代码路径。
...
1. **human-controlled 配置示例的端口来源**。`docs/.human-controlled/config.example.yaml:226,229` 的目录默认值正确，但 `standalone-${GHC_API_PROXY_PORT}.pid` 把文件名绑定到请求端口。当前实现 `src/app/config/paths.py:30-43` 与 `src/app/lifecycle/entry.py:101-102` 则在 bind 后用实际地址的端口。该文件不在本子话题的可改范围；待用户把中英文两句改为 `standalone-<最终生效端口>.pid`。
...
- [260822-pidfile-missing-forensics.md](history/260822-pidfile-missing-forensics.md) —— 取证
- [260822-review-pidfile-port-scoping-opus.md](history/260822-review-pidfile-port-scoping-opus.md) —— 评审（Opus）
- [260822-review-pidfile-port-scoping-gpt.md](history/260822-review-pidfile-port-scoping-gpt.md) —— 评审（GPT）

三份已归入本子话题的 `history/`；迁移只改写了本 README 的入站引用，不改写原件内容。报告原件仍是时间点记录，其中的路径与行号是当时的快照，不得把其结论外推为 current code／config／运行态事实。
