...
> **⚠️ 状态：本文的修复方案已被取代（2026-08-20 同日）。**
>
> 本文交付的是**剥离**声明：400 消失了，但 web search 也随之不可用。**用户裁决这不符合初衷**，随后改为**映射**成 `{"type":"web_search"}` 并在响应侧渲染搜索行，让搜索真正执行。现行方案见 [`260820-websearch-responses-leg-mapping.md`](260820-websearch-responses-leg-mapping.md)。
>
> 本文**保留不删**，因为其中三部分仍然成立且是现行方案的基础：§2 的根因分析（`_function_tool` 的 `input_schema` 捷径）、§6 对 spec 的两处事实更正、§7 的两轮评审处置。§3 的行为表与 §4 的范围裁决**已过时**，不得引用。
>
> 也保留一条教训：本文 §4 的首要理由（「响应侧接不住 `web_search_call`」）是**错的**，把 legacy 链路当成了在产链路——而我在同一份文档的 §6.1 刚给 spec 指出过同一个错误。正是这条错误理由让我把选项收窄到了剥离。
