...
四条请求起始时刻不同（散布约 10 秒），结束秒相同，异常形状相同。**这强烈支持一次相关的连接层终止，而不是四次独立失败。** 但当时没有 connection id，**不能确认它们来自同一条连接、或同一帧 GOAWAY**——单连接与多连接被同步回收，两者与现有证据同等相容（见下表「未决」列）。

**这类中断在姊妹服务上有多常见**：现网 `copilot-api-js` 的 history，2026-07-17..08-19、21 个日历日、93125 条请求，**广义传输中断的代理分类**命中 594 条 = **0.64%**，繁忙日 178–235 次/天。另有约 0.2% 的重试记录，但它与传输中断的对应口径**未核实**。

**这个数不能拿来估计本次事故那一类事件的频率**——它数的是若干错误字符串的并集，不是 GOAWAY 帧计数（`copilot-api-js` 的 GOAWAY ledger 在压缩 blob 里，未解码）。取证报告自己也是这么写的，详见 `archive-260820/260820-goaway-frequency-forensics.md`。

**时长是历史样本里的主导变量，而本次不在它的高危区**：危险率从 <5 秒的 0.011% 涨到 160–320 秒的 24.9%；本次四条是 5.3–15.9 秒，落在最低区间。所以本次**不像**历史样本里以长流为主的那种典型形态——**但低概率不等于机制不同，据此排除同一底层机制是无效推理**（本文初稿这么写过，已更正）。

按时长分层后，请求大小与模型选择的总体表观效应明显减弱，但长时长桶仍有未解释的残余差异（80 秒以上 22.9% vs 5.9%），取证报告明确表示不据此下结论——所以这两项是**未证成独立驱动因素，也未排除其作用，仍未决**。「每天固定的部署窗口」则是**在有覆盖的数据中未获支持**，而数据本身有 13 天空洞。

三者都不是「已证伪」，但它们的证据强度也**互不相同**：本文初稿写成「均被证伪」，第一次更正又把三者一并压成「均未获支持」——那是过度修正，同样不准。
...
| 同连接多条并发流一起死（双流实测 3/3） | 「上游响应被提前关闭」的频率——**目前已知的、能区分 h2+cap 与 HTTP/1.1 的量**；结构化日志使其可估，但 `status=gone` 是代理指标而非精确计数（见 `deferred.md`） |
...
| 姊妹服务上广义传输中断的**代理分类**命中 0.64%，时长是其主导变量（不是本次事故那一类的频率） | |
...
**2026-08-27 关闭一条旧问题**：`HistoryConsumer` 没接活链路不是待确认的疏漏。当前 `src/` 与 `tests/` 均无 `HistoryConsumer` 命中，整条旧 history 链已归档到 `src/.archived/app/history/`；新的取证记录能力由当前 structured request log／JSONL 实现承接（`src/app/observability/request_log_file.py`，实现状态见 [`../retry-and-continuation/status.md`](../retry-and-continuation/status.md)）。这个结论只关闭「是否有意为之」的问句，不替本项目补出生产数据，也不改变本主题仍无自身中断频率基线的事实。
...
| 动作 | 状态 |
|---|---|
| **每连接流数上限的取值** | **机制已落地**（`42738c9`）：`upstream_transport.max_streams_per_connection`，默认 0 = 关闭。待裁决的是**取什么值 / 是否开启**，不是要不要实现。PoC 数据：不设 cap 时 6 条流挤 1 条连接，cap=2 → 3 条连接，cap=1 → 6 条连接；打掉一条连接后 cap=2 只死 2 条、cap=1 只死 1 条，其余存活。详见 `archive-260820/260820-h2-stream-cap-poc.md` 与 `deferred.md` |
...
所以两者是**两个不同的产品选择**，不是同一件事的两种写法。`http2: false` 与每连接流数上限**都已落地**（`5a366a8` / `42738c9`）；剩下的是选哪一条、以及 cap 取什么值。
...
本次诊断经**四轮**独立评审（三轮诊断 + 一轮收尾）、34 条发现，**每一轮都查出「把未观测的说成已确认」**，第二轮那条还是我在修第一轮问题的过程中新引入的，第三轮推翻了一条我写死的结论（「没有第三条路」）——推翻它的是**实测**，纯文本评审没查出来；第四轮（收尾）又在这份文档里查出同一个毛病：把「同秒同形」写成了「同一帧 GOAWAY／一个共同连接事件」，而同一份文档下面就写着「是否共用同一条连接：未决」。

本次可支持的结论只有这么窄：**这一条运行时行为主张上，增加纯文本评审轮次没有发现反例，而 PoC 发现了。** 今后同类的运行时行为主张应先做有分辨力的 PoC 再评审——但这不等于「所有过度断言都只能靠 PoC 治」。 同时，五份文档 128KB 对应两页可行动内容，这个产出比本身就是问题；后续同类工作应当短诊断 + PoC + 一轮评审 + 立刻落地已裁决部分。
