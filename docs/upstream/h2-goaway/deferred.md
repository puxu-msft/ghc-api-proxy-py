...
`HistoryConsumer` 没接活链路是否有意为之已于 2026-08-27 核清并移入 [`findings.md`](findings.md)：旧 history 链已整体归档，新的取证记录能力由当前 structured request log／JSONL 实现承接（见同主题 `findings.md` 的「结构化请求日志」）。它不再构成本条的未闭合部分。
