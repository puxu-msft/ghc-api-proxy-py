...
2026-08-20 15:01:59，四条流式请求在同一秒集体失败，异常形状相同：一帧 `NO_ERROR` + `last_stream_id=2^31-1` 的 GOAWAY，即 RFC 9113 §6.8 的优雅关闭首帧。**它们是否来自同一条连接、同一帧，当时没有 connection id 可证——这一点至今未决。**
...
| 路径 | 是什么 |
|---|---|
| `findings.md` | **活文档**。现象、机理、确凿与未决、已落地的六处修复、待裁决项 |
...
| `260820-closeout-review.md` | 收尾产物的独立评审，11 条，全部采纳 |

四轮独立评审（三轮诊断 + 一轮收尾）共 34 条发现，全部采纳。**每一轮都查出「把未观测的说成已确认」**，其中第二轮那条是在修第一轮问题的过程中新引入的，第三轮推翻的那条是纯文本评审三轮都没查出、只有实测才推翻的；第四轮又在收尾文档里查出同型问题。这个模式记在 `260820-h2-goaway-review-round2.md` 末尾。

### `evidence/` 里的三个脚本

`history-frequency-analyze-{1,2,3}.py`——`260820-goaway-frequency-forensics.md` 全部数字的**唯一产出者**。报告里引的是**关键节选**（完整 stdout 未进本目录），脚本本身留下来，因为数据源还在原地、随时可重跑出完整输出：
...
| 提交 | 做了什么 |
|---|---|
| `16dd68c` | STR-04 的 SSE 信封一半：**已发出 `message_start`** 却无终止事件的 EOF 改发 Anthropic SSE `error`，不再伪装成干净结束；未开始的空流行为未改 |
