# 裁决记录

**权威**：`docs/.human-controlled/upstream-retry-and-continuation.md`。本文件不与它竞争——它是用户亲笔的产品裁决，本文件只是**把裁决发生的时间、场合与理由记下来**，供后来者判断某句话的出处与保质期。凡两者不一致，以人写文档为准，并来改本文件。

**为什么需要这份文件**：本主题的裁决大多在一次交互式讨论中做出（2026-08-21），有些已写进人写文档，有些只以口头形式存在于那次对话。独立评审在 2026-08-21 指出，`.dev` 里查不到后者的任何记录，于是把已裁决的内容误判为「照未经裁决的建议施工」。那是记录的缺陷，不是裁决的缺陷。

## 〇、权威文档有两份

`docs/.human-controlled/upstream-retry-and-continuation.md` 之外，2026-08-21 新增 **`docs/.human-controlled/client-side-block-delivery.md`**，它是「客户端块级交付」的权威，本主题的几处判据直接落在它上面：

| 它裁定的 | 对本主题的影响 |
|---|---|
| 完整块的定义：一个 `_start` 与一个 `_end` 事件之间的全部内容；每形成一个完整块就交付并从缓存移除 | 「已交付过完整块」这个判据的定义来源 |
| **响应头在第一次 HTTP 200 尝试时就转发**，不等完整块 | 所以「响应头已提交」**不等于**「已交付完整块」。重放的合法性钉在后者 |
| 后续重试若 HTTP 报错**只能转化为 SSE error**，`Retry-After` 找不到载体转发，接受该限制 | 界定了 D 组那条「429 + `Retry-After`」的适用面：**只在从未拿到 200 时**成立 |
| **响应头提交之后**按 `client_delivery.sse_ping_interval` 发 SSE ping | 直接确认了 ping 不应被「有没有交付过块」拦住 |
| 非流式天然满足块级交付（等价于一次性交付） | 与「非流式只可能无痕重试」一致 |

## 一、已写进人写文档的（以那份为准，此处只列索引）

**2026-08-22：本表的「落点」列原本是行号，全部作废并改为引句。** 用户当日多次重写该文档，五行里没有一行的行号还是对的（21/23/25/27/31 实际已成 20/22/24/30/…），而一个错的行号比没有行号更糟——它让读者以为自己找错了地方。引句不随文档挪动失效；措辞若被改写，找不到反而是正确的信号，说明该复核这一行。

| 裁决 | 人写文档中的原句（片段） |
|---|---|
| 无法继续 / 一般可以继续 两张分类表 | 「这些情况下无法继续：」与「这些情况下一般可以继续：」两节 |
| 未交付完整块 → 代理端无痕重试 | 「如果还没交付过完整块，直接在代理端无痕重试。无痕重试不设冷却间隔。」 |
| `max_tokens` 不应无痕重试 | 「## 输出超长」一节：「……不应无痕重试。要么在能续写的情况下，丢弃未完成的块……」 |
| 优雅关闭不无痕重试 | 「特别地，优雅关闭时报错不再考虑无痕重试，**可以**走下文合成续写机制。」——注意是「可以走」而非「走」，措辞已于 2026-08-22 由命令式改为许可式，本表原先转述的「改走」对应的是旧版本 |
| 429 走反应式限流器；预算耗尽后返回 429 + `Retry-After`；删除 `client_delivery.synthesized_response_headers_after_sec` | 「特别地，HTTP 429 会启用“反应式限流器”……」及紧随其后的引用块 |
| 已交付过完整块 → 合成 `tool_use` 调用 `turn_interrupted(num_messages, category, message)` | 「## MCP-driven 合成续写」一节首段 |
| 工具全名可由 `upstream_request_retry.auto_retry_tool_call_full_name` 覆盖 | 「该工具调用全名可通过配置项……覆盖。」 |
| 工具定义缺失时打 warning 但照发 | 「……如果没有，在本代理日志中打印警告日志，但仍然依样返回给客户端。」 |
| MCP-driven 续写不设次数上限 | 「MCP-driven 续写是否需要次数上限？不需要……」 |
| 仅 anthropic-messages 客户端请求适用 | 「我接受目前只给 anthropic-messages 客户端请求时使用该机制……」 |
| 非流式两条机制都支持 | 「## 非流式请求」一节：「非流式请求也支持无痕重试、合成续写机制。」（2026-08-22 新增，反转了更早那句「不支持合成续写」） |
| 代理内续写放弃 | 「## 代理内续写（已放弃）」一节 |

## 二、只存在于 2026-08-21 那次讨论中的（本文件是它们目前唯一的记录）

下面每条都是用户的明确裁决，**不是本项目的建议**。措辞为转述，括号内是当时给出的理由。

1. **截断块的丢弃规则**：`max_tokens` 时，**只有未完成块就保留它；有任何完整块就丢弃未完成块**。（保留半截内容优于给客户端一个空回答。）
2. **`max_tokens` 一律走合成**，不回落到无痕重试——即使按第 1 条丢弃后一块不剩。
3. **reasoning item 无 `status` 信号时不特殊处理**。（「历史里没有就保持悬念，暂不特殊处理」。）
4. ~~**非流式路径只可能无痕重试，不可能续写。**~~ —— **2026-08-22 用户自行推翻**：非流式应当找到最后一个 incomplete 块的边界，并同样交接。理由是缓冲交付不改变「这一轮没写完」这个事实，客户端仍是唯一能续下去的一侧。已落地（主仓 `af84097`）。
5. **无痕重试不设间隔**，反应式限流器给的间隔除外。（「程序正常运行时多次超长时间的无痕重试是值得的，不必过多防御」。）
6. **`num_messages` 取客户端请求的 `messages` 长度**，不取上游请求。
7. **观测面**：合成之后**客户端请求算成功、本次上游尝试算失败**，请求行用 `[RETY]` 前缀 + 黄色。（「我们有独特的 MCP tool use，所以不难判断」。）`[RETY]` 而非 `[RETRY]`：保持四字符宽。
8. **`stop_reason` 不得被改写成 `end_turn`**，原样透出，已知值着色。（「绝不能」。）
9. **`refusal` 归「无法继续」。**
10. **区分请求断开原因要增强**，理由是有助于调试。
11. **归档去向**：新开主题 `upstream/retry-and-continuation/` 承载归档与后续实现，而非原地归档在 `h2-goaway/` 下。
12. **删码范围**：只删代理内续写机制本身，**其他未接线的功能不要动**。（**2026-08-22 被第 15之三 条局部推翻**：`max_tokens_as_retryable` 已按用户口述删除。`streamReplay` 亦已由第 15 条推翻。其余仍然有效。）
13. **不向上游发真实请求补证**：只查历史，历史没有就保持悬念。
14. **实施顺序**：按 A（文档归档）→ B（删码）→ C（观测面）→ D（无痕重试）→ E（MCP 续写）逐阶段完成，每阶段提交一部分。

## 二之二、2026-08-22 讨论中的裁决

15. **删除 `streamReplay`**：断流重开走 `network` 的普通预算，不设专属计数器。（用户质疑「一条客户端请求最多重开多少次上游流」为什么需要专门配置；本项目答不出好理由——那是代理内续写方案的遗留。）
15之二. **非流式也要交接**，并在翻译时按同一条规则找到被截断 item 的边界（有完整块在前才丢，只有它时保留）。推翻上面第 4 条。
15之三. **配置项 `auto_retry_tool_call_full_name` 从 `client_delivery` 移到 `upstream_request_retry`；`max_tokens_as_retryable` 删除。**（2026-08-22 用户口述，未写进人写文档。**这条局部推翻了第二节第 12 条「其他未接线的功能不要动」**——`max_tokens_as_retryable` 正是那批之一，独立评审据此判为与已记录裁决冲突，因为当时 `.dev` 里查不到这次推翻。此处补记，那条冲突随之解除。）
16. **客户端时限在 body 阶段触发时，发一个 SSE error 帧再收尾**，不直接截断，也不先 flush 缓冲块。（响应头早已发出，状态码改不了；`error_frame` 通道现成、只是没接上。）
17. **D 组照做在 main 上，与同伴的 `fix/upstream-error-events` 分支的冲突留给该分支自己解。**（G1 已停 8 小时、落后 19 个提交、且其独立评审结论为 needs-fix。）
18. **`client_request_deadline` 保护的是「这一轮总耗时」**，不是「客户端还愿不愿意等」。（用户 2026-08-22，回应 `deferred.md` 第 11 条：「这是个小概率问题，时限保护的是这一轮总耗时」。）

    直接后果是**客户端时限先答、`terminal.seen` 后答**：时限一响这一轮就到此为止，上游是不是刚好写完最后一个字节不改变结论。`stream.py` 当前次序已经是这个，本条是确认现状而非待办。

    **上游时限（`StreamDeadlineError`）的次序相反且必须相反**：它只结束*这次尝试*，所以「上游说完了没有」必须先答，否则一个已完成的回合会被判成可重试的失败（那正是 `1743a0b` 修的东西）。两条守卫排序相反，是因为它们终止的对象不同。

## 三、本项目推论，**不是裁决**（前提变了即失效）

1. **HTTP 状态码类的失败只可能出现在「未交付」位置**，`stop_reason` 类只可能出现在「已交付」位置。前提是「已交付分支不发起新的上游 attempt」。见 `status.md`。
2. **删掉 `synthesized_response_headers_after_sec` 后，`message_start` 只能与第一个完整块同批发出**，半开状态不再可达。前提是该配置项确实被删——**2026-08-22 已随 D 阶段删除**（主仓 `0b57645`），所以这条推论现在成立；它若回来，推论随之失效。
3. **合成时两条上游腿都适用**。由「判据是客户端请求格式」推出；人写文档只写了客户端侧的判据，没有逐腿列举。

## 四、尚待裁决

1. ~~**`max_tokens` 触发合成时 `category` 传什么值**~~ —— **这不是待裁决项，是待对齐项**，2026-08-22 更正。没有分歧：`"max_tokens"` 是上游自己的词，也是唯一候选，实现里用的就是它。需要的是**把这个值告诉改 MCP 端的同伴**，让两边一致。把协调写成裁决是升级过头。

    **2026-08-22 补**：MCP server 在另一个仓，本会话联系不到那位同伴，所以改为把发出方的一手契约写进 `README.md`「本仓实际发出的 MCP 工具调用」一节——`category`、`num_messages`、`message` 的全部取值都在那里，从代码直读。**发出方的实际字节即权威**，那一侧照着对即可。
2. ~~**断流重开要不要独立的重试预算？**~~ —— **已裁决，见第二之二节第 15 条：删除 `streamReplay`。** 本条留作理由记录：`streamReplay` 是代理内续写方案的遗留（当年 replay 与 continuation 并列、各自记账），continuation 删除后配对已不存在。读 body 时撕裂与响应头之前撕裂是同一件事的两个时刻，都是 network 失败；「客户端已经看到什么」决定恢复**合不合法**，不该决定由谁付账。

3. **`num_messages` 的判法**：本项目建议按「同一数值重复出现」而非「数值有没有增长」（并行子智能体与主会话共享同一个 MCP server 进程，调用会交错）。**待与 MCP 端对齐**，两边不一致该参数即失效。

## 五、明确不做（2026-08-22 自 `deferred.md` 迁入）

这四条都是**裁决**——「不做」和「做」一样是决定，理由必须留着，否则下一轮会有人重新提议同一件事并觉得自己想到了新点子。原先躺在 `deferred.md` 的「明确不做」栏，但那个文件按其宪章只收未闭合与待裁项；已经裁掉的属于这里。

**新开一节而不是并入前面几节**，是刻意的：`src/app/pipeline/hand_over.py` 按 `decisions.md` 的**小节号**引用本文件（引的是 `4.1`），往既有节里插条目会改动编号并使那个引用静默失效。同样的理由见 `deferred.md` 抬头。

1. **发真实请求向上游补证。** 用户 2026-08-21 明确禁止：只查历史，历史没有就保持悬念。调查报告里那条「最低成本补证是发个超长 prompt 触发 400」的建议**不采纳**。
2. **代理内续写。** 已裁决放弃，材料归档在 `archive-proxy-side-continuation/`。
3. **MCP-driven 续写的次数上限。** 已裁决不设。理由：会触发这条路就说明事情有进展；门本身（已交付过至少一个完整块）保证零进展的一轮到不了这里。详见 `status.md`。
4. **为非 anthropic-messages 客户端合成工具调用。** 用户接受当前只支持这一种；将来用上别的 harness 再补。**这是范围边界，不是遗漏**——把它读成待办会让人去实现一个用户没有要的东西。

## 六、2026-08-27 从台账迁入的裁决与教训

以下内容的裁决权威仍是本文件开头指向的人写文档与用户后续明确裁决。本节只承接已闭合记录，使 `deferred.md` 继续只放未闭合项。

### 7. 孤儿件与死配置项的处置

`decide_stream_ending()` 之外，还有 5 组零生产调用点的件与 4 个无人读取的配置项：`RetryBudget`、`buffered_retry.py`、`delayed_commit.py`、`continuation.*`、`streamReplay.max_retries`、`max_tokens_as_retryable`、`hedge`。

**用户 2026-08-21 裁决：只删代理内续写机制，其他未接线的功能不要动。** 所以除 `continuation.*` 外一律保留。其中 `delayed_commit.py` 的形状恰好对得上第 2 条将来可能需要的延迟提交，`streamReplay.max_retries`（默认 100）在 D 组接线后会生效。

**2026-08-22 补一条同族的新情况**：`decide_stream_ending()` 本身已接线（`8f654b4`），但 `c86712d` 之后它的 **`COMPLETE` 那一格从唯一生产调用点不可达**——`_deliver` 必须在问它之前先答完「上游说完了没有」，否则一个 `normalize_upstream_error` 不认识的异常（裸 `h2.ProtocolError`）会让完整回复照样被丢。异源评审实测：把该分支改坏，unit+int 1589 条里只有它自己的单测变红。

按上述裁决**不删**，`1479025` 已在该分支加了回指注释。待裁的是形状：要么让这个纯函数只裁「未完成流」（去掉 `terminal_seen` 参数与 `COMPLETE`），要么重塑参数使调用者能在异常分类之前问出完整 verdict。**不要**改成在 verdict switch 里处理 `COMPLETE`——那条路对上述异常根本到不了。

### 22. ~~`internal` 同时盖着「上游协议故障」与「本仓 bug」两件相反的事~~ —— 已裁决并落地（2026-08-23，主仓 `0ca87b9`）

**用户裁决：采用方案 (a)**，把 `h2.exceptions.H2Error` 加进 `errors.py` 的 `_CONNECTION_ERRORS`。理由是这不是新立规矩，而是把代码拉回用户亲笔 `docs/.human-controlled/upstream-retry-and-continuation.md` 已有的「网络中断一般可以继续」。

调查与影响面全文在 [`reports/260823-h2-protocolerror-category.md`](reports/260823-h2-protocolerror-category.md)。留下三条值得记的：

1. **决定命运的是内核那一次 `read()` 的分包。** GOAWAY 与其后的帧分开到达 → `httpx2.RemoteProtocolError` → `network` 且可重放；落进同一次 `read()` → 裸 `h2.ProtocolError` → 当时是 `internal` 且不可重放。同一个上游事件，两种结局，而分岔点在操作系统。这是本条从「标签不好看」升格为缺陷的原因。
2. **`internal` 的「结构上不可达」是错的，而且写下时就错了。** 前提被 `78be0d4`（2026-08-22 18:57）解耦，那句话是次日 `a8862e6`（06:26）写的。形态记在 [`22 之四`](deferred.md#22-之四-一条状态断言在写下时就已经过期--已移入教训文档2026-08-27) 。
3. **(b)「只在 `replay_reason` 里特判」的「影响面更小」是错觉**，已记进报告 §4.2：它改的行为与 (a) 一样多，只是当时的测试打不到——因为那条测试用的是自建的 stand-in `eligible`，不是生产接线 `replay_reason`。

**未随本条解决的**：`httpx2.DecodingError`（上游把 body 压坏）现在归因正确（`upstream`）但**仍不可重放**，因为 `normalize_upstream_error` 仍叫不出它的名字。「叫不出名字的失败该不该可重试」是第 20 条的产品问题，本次没有替它作答。它同时是 `test_a_finished_turn_survives_a_failure_nothing_recognises` 的载具——那条测试的前提断言会在它哪天被命名时出声。

### 优雅关闭排空期间拒绝重开后，零提交不为交接开口

#### 原问题与已经量化的取舍

**2026-08-22 补第三个触发条件，且它比另外两个常见得多：排空主动拒绝重开**（主仓 `db49581`）。每一次带在途流式请求的优雅重启都可能撞上，而不是「预算恰好耗尽」这种偶发。

   **同时更正一条本项目自己写错的机制陈述。** 落地 `db49581` 时，提交信息与两处 docstring 都称「交接需要已交付的内容，所以这一格结构上不可能交接」——**这句话是假的**，异源评审实测证伪（`reports/260822-review-drain-suppression.md` major-2）：`_hand_over` 在 `if not started:` 时会自己补 `framer.preamble()`，只把 `committed_count == 0` 那道门短路掉，排空被拒的那条流立刻产出一次干净的交接（日志 `upstream_replay_refused_while_draining` → `turn handed back to the client to continue`，`status=retry`）。挡住它的是**一道可配置的闸**，不是机制的固有属性。已改正代码注释与 `status.md`；此处登记的是那个假理由掩盖掉的真问题。

   **真问题：排空这一格要不要为交接开口？** 人写文档「特别地，优雅关闭时报错不再考虑无痕重试，**可以**走下文合成续写机制」这句，承接的正是它上一句「如果还没交付过完整块」——**说的就是这扇门**。措辞是许可式（「可以走」）而非命令式，所以现状（不开口）并不违反文档；但也谈不上被文档裁定过。

   **不开口的代价已量化，是真的丢东西**（评审 major-3，代码事实，推理链每环已核）：`client_delivery.buffering_policy` 取 `full` 或 `until-tool-use` 时，缓冲策略把整轮的完整块全压在 `BlockBuffer` 里不释放（`stream.py` 自己的注释写明这两种策略下「首块前的窗口就是整个回合」），于是 `committed_count` 恒为 0、`client_has_bytes` 未 set。此时关机 + 撕流 → `decide_stream_ending` 返回 REPLAY → 排空闸拒绝 → `_hand_over` 因 `committed_count == 0` 返回 `None` → 裸抛，**缓冲区里那一整份已经算完的回答被整批丢弃**。`db49581` **之前**这一格会重放并大概率成功（排空本来就在等这个请求）。即：本项目的改动让一条既有的丢失式结局在非默认配置下变得常态可达。

   开口的做法是现成的：`_hand_over` 一旦被允许在 `committed_count == 0` 时动作，它做的第一件事就是 `session.finish()` 把缓冲块冲出去再附上 `tool_use`，这一格自动补上。**证据等级：机制与代价均为确凿（一手实测 + 代码事实）；开不开口是产品裁决，需用户定。**

#### 用户裁决（2026-08-27）

**维持现状，整批丢弃，不为交接开口。** 用户原话：「根据你的描述，这种情况就应该整批丢弃」。

`committed_count == 0` 意味着客户端一个字节都没收到，这一轮在客户端看来从未开始。hand-over 机制（`turn_interrupted`）的意义是「你已经收到一半了，这是继续的信号」；零提交时没有「一半」这回事，硬造一个交接等于协调一个不存在的部分状态。丢弃后由客户端重发，语义更干净。

**这是知情取舍，不是遗漏。** 那一轮的上游 token 已经计费，丢弃意味着客户端重发时再花一次；`client_delivery.buffering_policy` 取 `full` 时，代价是一整份完整回复的费用。允许交接本可保住这笔费用，但会引入上述「协调不存在的部分状态」的复杂语义；用户在知晓两边代价后选择不开口。

现行代码已经是该行为，不需为本裁决改代码。该格于 2026-08-27 从 `deferred.md` §5 移出；§5 的其它格继续按各自状态留在台账。
