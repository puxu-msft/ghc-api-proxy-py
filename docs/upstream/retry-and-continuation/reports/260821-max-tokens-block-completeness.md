...
证据目录：`.dev/docs/upstream/retry-and-continuation/evidence/max-tokens-block-completeness/`（探针脚本 + 原始输出）
...
原始逐例表格见 `evidence/max-tokens-block-completeness/inc_table.txt`。
...
uv run python .dev/docs/upstream/retry-and-continuation/evidence/max-tokens-block-completeness/probe_hist_3.py > /tmp/scan_hits.txt

# 逐例结构化表格（依赖上一步产出的 /tmp/scan_hits.txt）
uv run python .dev/docs/upstream/retry-and-continuation/evidence/max-tokens-block-completeness/probe_hist_5.py
```
