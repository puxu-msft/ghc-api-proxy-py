...
> **编号是公共接口，永不回收，也不重新编号。** 清点实测：**6 个生产源文件 + 1 个集成测试 + 3 处活文档**按号引用本文件（`stream.py` 引 §12／§19／§20／8d，`openai_responses.py` 与 `responses.py` 引 §2，`inference.py` 与 `test_pipeline_app.py` 引 §5，`base.py` 引 8a）。重新编号**不会报错，只会静默指错人**。所以条目移出后原位留墓碑，空号（13、14）不补——用户 2026-08-22 第 2 条裁决：不必强求序号连续。清点全文见 [`history/260822-deferred-md-inventory.md`](history/260822-deferred-md-inventory.md)。
...
- **本条原标题「主路径抽不出数字」已被用户质疑并撤回**，理由（那个数上游模型目录就公布着；其消费端在不被服务进程构建的 legacy 链路上）与教训（*继承一个缺口的描述时，先找它的消费端*）→ 记在 [`history/260822-deferred-md-inventory.md`](history/260822-deferred-md-inventory.md) 与本条的历史版本里，不再占本文件篇幅。
...
- **为什么这个次序是载重的**，以及两轮受控变异的证据 → [`history/260822-review-complete-fix-opus.md`](history/260822-review-complete-fix-opus.md) 问题 2 与 `reports/260822-review-mcp-contract-and-deadline-order.md` F12／F13。
...
来源：[`history/260822-review-session-closeout.md`](history/260822-review-session-closeout.md) 整改复核 N1（异源评审，配负样本对照）。
...
来源：[`history/260822-review-never-silent-failure-events.md`](history/260822-review-never-silent-failure-events.md) §2（异源评审，探针带正样本对照）。探针原件与边界说明在 `../../../exp/260822-silent-drop-probes/`。
...
### 24. 字节保真是否扩到 5xx / 超时 / 流截断 —— 需用户裁决

**当前范围不是永久否决**：`rejection_capture.capture_rejection()` 只接收 `UpstreamRejected`，即上游已读 body 后给出的 4xx 非限流拒绝；5xx 是上游自身故障，超时／连接失败没有上游 verdict，流截断发生在响应 body 阶段。因此目前只保留 4xx 非限流的出站实际字节，并非已裁决“这些结局永不保留”。

**暂缓建议与理由**：暂不把这些结局并入现有 `rejected/`。该目录的 `KEEP_NEWEST=50` 按份数封顶；扩大结局集会让“说明不了请求为何被拒”的失败驱逐唯一值得读的拒绝样本。超时类 SDK 异常也没有 `.response`，需要另一条取值路径。若用户需要扩展，建议作为独立机制设计为 `rejected/` 与 `failed/` 分目录、各自独立保留上限，而不是扩大既有触发集；具体采集点、保留对象与容量取舍仍待裁决。

**迁入后复核（2026-09-08）**：当前源码仍符合该范围：`src/app/observability/rejection_capture.py` 以 `isinstance(error, UpstreamRejected)` 为唯一入口、`KEEP_NEWEST = 50`，并写入 `error.sent` 的实际 bytes；`src/app/server/routes/inference.py` 在请求发出后才以 `response.request.content` 记录／观察该 bytes。

**来源**：原始产生语境、完整建议与当时实现验证见 [`history/decisions-20260821.md`](history/decisions-20260821.md) §6.1；迁入前来源为已计划退役的 history 主题之 `decisions.md` §6.1。

### 25. `upstream_max_gap_s` 是否上终端完成行 —— 需用户裁决

**当前范围不是永久否决**：`upstream_max_gap_s` 当前进入 request record／JSONL 等记录面，但不渲染到终端完成行；这保留了事实，却没有替未来的显示策略作永久裁决。

**暂缓建议与理由**：暂不上行。相同性质的 `first_upstream_byte_s` 一直只在记录中；卡住的一轮已经由会自行升级颜色的大 `duration_s` 在终端行上提示。若要显示 gap，必须先裁定阈值与配色；`upstream_request_timeouts.stream_idle` 默认 `0`（未启用终止器），不能为阈值提供现成依据。需要显示时，应先以真实配置和观测基线定义“何种沉默值得占用每一行的一列”，而非仅把已记录字段拼到行上。

**迁入后复核（2026-09-08）**：当前源码仍保持此状态：`src/app/server/routes/inference.py` 计算首字节后的最大到达间隔；`src/app/observability/request_completion.py` 将其投影到完成记录；`src/app/observability/request_log.py` 明确说明该字段不渲染到 console line、原因与 `first_upstream_byte_s` 相同。

**来源**：原始产生语境、完整建议与当时实现验证见 [`history/decisions-20260821.md`](history/decisions-20260821.md) §6.2；迁入前来源为已计划退役的 history 主题之 `decisions.md` §6.2。

### 26. 原“完整 HTTP 支持”前提消失后，是否仍需查询面 —— 需重新用户确认

**为什么不是已裁决**：用户 2026-08-20 对“完整 HTTP 支持”的裁决，所指对象是当时计划中的完整取证库及其 HTTP 查询面（L1 `entries/list`／`entries/get` 等）。2026-08-21 重定范围后，完成记录已由 `requests-*.jsonl` 承担，因而不再建设那套库；原裁决的对象与前提随之消失。这不是用户撤回 HTTP 支持，也不是“永不提供查询”的裁决，而是必须针对当前对象重新确认的需求。

**若仍需，查询什么**：查询面只能以已有的 `requests-*.jsonl` 与 `rejected/*` 为对象；它不自动恢复 L1 独立落盘、会话聚合、归档、`entries/*`／`sessions/*`／`archives/*`、`export-cassette`、replay、WS 推送或 pin/unpin。是否需要该查询面、它应提供哪些查询能力与 HTTP 契约，均是新的范围决定，不在本次实施内。

**来源**：原始重定范围的必要上下文与原文见 [`history/decisions-20260821-query-surface.md`](history/decisions-20260821-query-surface.md)；迁入前来源为已计划退役的 history 主题之 `decisions.md` 第五节“重定范围”。
