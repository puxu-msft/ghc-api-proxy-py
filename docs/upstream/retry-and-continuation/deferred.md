...
| **上游终止事件已见之后再撕裂** | `stream.py` 直接 `break`，无痕 | **仍然无痕，未修** |

最后一格是评审补上的反例，它证伪了我写过的「交接是唯一会吞掉异常的结局」。本项目此前已把它登记为 post-terminal failure 无痕，本次没有连带处理。

要不要为消除**误归因**放弃族级映射，仍是产品裁决；但前三格已经不再是看不见的问题。
