...
| 2026-08-24 | **上游干净 EOF 而无终结事件也交接**，与撕裂同形：发 SSE error 之前先咨询 `_hand_over`。此前只有撕裂路径咨询，于是被本侧 idle 守卫掐死的结局反而优于上游干净关闭。触发是生产事故 req=`75ccdf6f`。权威是人写文档第 30 行，非新裁决；`anthropic-responses-bridge/spec.md` 第 7 条已同步。**停在块边界那一格不变**——它不报错，第 30 行的触发条件不成立 |
