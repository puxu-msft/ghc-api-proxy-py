...
**建立日期**：2026-08-21。**最近同步**：2026-09-06。**权威**：`docs/.human-controlled/upstream-retry-and-continuation.md`（下称「人写文档」）。本文只记实现状态与路线，不复述它的裁决。裁决的时间、场合与理由记在 [`decisions.md`](decisions.md)。

## HTTP 499 已进入当前 requirement 与实现

人写文档当前把 `499 Client Closed Request`列入“一般可以继续”；当前 `src/app/model_provider/upstream_errors.py::RETRYABLE_STATUSES` 也包含499，实现提交 `39274d7` 可从当前 `main` 达到。[`http-499-retry.md`](http-499-retry.md)、[`review-disposition.md`](review-disposition.md) 与对应报告因此是当前功能的开发记录，不再只是另一份source clone的导入快照。

这项结论只覆盖HTTP 499 status进入既有retry taxonomy，不把所有名为“client closed”的本地cancellation都变成可重试：本地client disconnect／cancel仍按人写文档取消upstream。HTTP status与本地连接状态是两个来源不同的事实。

用户选择专用 `origin/dotdev` 作为 `.dev` 的远端持久源这一**存储裁决**独立成立，由本仓根 [`../../../README.md`](../../../README.md) 维护；它不参与499行为判断。
...
| 人写文档要求 | live 链路现状 | 出处 |
|---|---|---|
| 网络中断、5xx 可续 | **只在上游响应头到达之前**重试（循环在 `base.py:126-176`；`_send` 止于响应头见 `:228`），~~无退避、无 jitter、无间隔，连打~~ **2026-09-08 起有退让**（见下） | `direct_driver/base.py:126-176`、`:228`；`config/schema.py:163-175,184` |

> **2026-09-08 退让机制**（用户指令「上游连续失败时需要简单的退让机制，避免短时间内加剧问题」）：共享的每提供方 `RateLimiter` 新增连续失败指数退让（`failure_backoff_base_sec` 0.5s 起、逐次翻倍、`failure_backoff_max_sec` 30s 封顶、一次成功清零；`config/schema.py` `ReactiveRateLimiterConfig`）。失败经两处上报：驱动层 `_handle_failure` 对 classify 为 RETRY 的失败 `note_failure()`（`PipelineRetry` 除外）；交付层无痕重放在 `_reopen` 里对被替换的撕流 `note_failure()`（该失败发生在 driver 已交付响应体之后，`_handle_failure` 看不见）。退让在 `note_failure` 时推进 `next_allowed`，用 `max()` 保证不缩短 429/502 的 `retry-after`/`retry_interval` 等待；`acquire` 唤醒后的调度写入同样取 `max`，不抹掉睡眠期间其他请求推上的等待。边界：驱动层 pre-header 重试逐次翻倍；交付层重放路径每次重开经一次「响应头成功」清零 streak，等待恒为 base 不翻倍。人写文档「无痕重试不设冷却间隔」的修订候选见 `.dev/human-controlled-docs-candidates/260908-upstream-retry-backoff.md`，待用户追认。
...
> **2026-08-24 更正**：上表「已交付之后的失败要能进裁决」那一行**只对本节锚定的 `8a36fe3` 成立**，它后来被改了两次——`78be0d4` 让停在块边界的干净 EOF 不再报错而是合成 stop reason 收尾，`a7a0e05` 又让**仍要报错的那些结局先咨询合成续写**。当前行为以下方 E 组表最后一行为准。本节其余各行未随之复核，读时一律先看节标题的锚点。
...
### D. 无痕重试 —— **块级路径已落地；direct Chat terminal-only 路径待实施**
...
**2026-09-06新增、尚未实施的Chat范围**：direct `stream:true /chat/completions` 当前走 `one_shot_delivery()`，headers后的body tear没有接上述ReplaySupport；resolved model endpoint不支持non-stream response时，streaming→buffered JSON adaptation也必须在shared attempt loop内完成，不能留在provider client。用户裁定两个故障面都修，SSE-buffered success必须见 `[DONE]`，已知瞬时流内error进入现有taxonomy，TUI／durable observation同批接通。Provider只声明per-model endpoint capability，pipeline解释并执行；当前CodeBuddy streaming-only capability在P6未运行时保守保留并标为未实测。权威合同见 [`../../direct-passthrough/spec.md`](../../direct-passthrough/spec.md) §5.4，设计与决策来源见 [`../../direct-buffered-chat-completions/`](../../direct-buffered-chat-completions/README.md)。本段是当前状态更新，不把“已落地”从既有block-aware路径外推到Chat one-shot。
...
| `a7a0e05` | **上游干净 EOF 而无终结事件也交接**（2026-08-24），与撕裂同形：发 SSE error 之前先咨询 `_hand_over`。此前只有撕裂路径咨询，于是被本侧 idle 守卫掐死的结局反而优于上游干净关闭。触发是生产事故 req=`75ccdf6f`。权威是人写文档第 30 行，非新裁决；`anthropic-responses-bridge/spec.md` 第 7 条已同步。**停在块边界那一格不变**——它不报错，第 30 行的触发条件不成立。代理自我保护等「无法继续」各格同样不咨询 |
...
2026-08-22 那次生产事故正是 Anthropic **上游**腿（判据：日志行上是 `think` 而非 `reason`，`REASONING_WORD` + `dialect_for` + `assembler_for` 三处共同决定；见 [`history/260822-h2-streamreset-cancel-diagnosis.md`](history/260822-h2-streamreset-cancel-diagnosis.md) §1.2）。

**代价比初稿估的高，这一段也已更正**：

- `code` 不能动——被 2 处测试断言，并被 `../../delivery-keepalive/spec.md` 逐字复述（[`history/260821-plan-g1-upstream-error-events.md`](history/260821-plan-g1-upstream-error-events.md) 的 G4 已查清）。
