...
下面每条都来自真实录制或代码直读，不是推断。完整报告在 `reports/`。`max_output_tokens` 截断这一组的 current evidence（`scan_hits.txt`、`inc_table.txt` 与只读 probe）以 `evidence/max-tokens-block-completeness/` 为 canonical 目录；`probe-reasoning-item-control.py` 从该目录读取 `scan_hits.txt`。
...
| `category` | 上游把这一轮截短、并非错误时：**就是那个 `stop_reason`**，默认配置下唯一可能的值是 **`max_tokens`**。是错误时：`network` / `upstream` / `auth` **三者之一**——**`internal` 已发不出来**，兜底从它改成了 `upstream`（2026-08-23，主仓 `0ca87b9`）。这一格当天变了两次，见下 |
...
- **`internal` 这一格当天变了两次，现在它发不出来。** 值得整段留着，因为它是这个主题最贵的一课。
