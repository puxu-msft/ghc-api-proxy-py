...
本文只针对 `docs/.human-controlled/config.example.yaml:350-352` 那三行。提案部分可整段丢弃。

## 现状

用户文档当前这样写（`config.example.yaml:350-352`）：
...
- 键名与层级与实现一致——`src/app/config/schema.py:301` 挂载 `proactive_rate_limiter`，`schema.py:150` 定义 `max_inflight: int = Field(default=50, ge=0)`。**结构是对的。**
- 该配置**已经接线并生效**，不是规划项：`src/app/server/pipeline_app.py` 把 `InFlightLimit` 挂在整个 ASGI app 的最外层（commit `f5589ec`）。
- 超过上限的请求在 `asyncio.Semaphore` 上**按到达顺序等待**，不被拒绝、不返回 429、连接不关闭（`src/app/server/admission.py`）。这是 2026-08-19 用户裁决「更多请求卡在接受连接而不要断开」的直接落地。
- `0` 表示关闭该门，不表示「不准入任何请求」。
- `/health`、`/health/liveness`、`/health/readiness`、`/metrics` 不占用名额（commit `7e9b62d`），否则饱和时 supervisor 拿不到应答，会把「正忙」误判成「已死」。

## 提案

三点建议，各自独立，可分别取舍：

**一、解注释。** 当前是 `# #` 双井号整段注释，读起来像「尚未实现的规划项」，而它是已生效配置。相邻的 `reactive_rate_limiter` 是未注释的，两者并列时这个反差会让读者以为主动式那套还没做。

**二、示例值与默认值分开写。** 示例写的是 `5`，代码默认是 `50`，而文档没有任何一处说明默认值，读者会把 `5` 当默认。建议要么把示例值改成 `50`，要么在注释里点明默认值。

**三、写明超限行为是等待。** 这是最容易被误解的一点——同一文件 354 行紧接着就在谈 429（那处指的是**上游**返回的 429，与本机制无关），两段相邻很容易被读成「超过 `max_inflight` 就给客户端 429」。
...
等待没有时限。`client_delivery.client_request_deadline`（默认 3600 秒）是在**请求进入处理之后**才起算的（`src/app/server/handler.py:190` 的 `handle_bounded`），而 `InFlightLimit` 在它外面，所以**排队时间不计入任何 deadline**。
