...
1. `/v1/models` 会列出非 default provider 的 `A/claude-opus-5` 这种形式；它与请求侧 `provider/model` 语法共用同一条可路由名称。客户端因此可以从模型列表发现 secondary provider 的模型，但这不改变 Gemini 与 Azure URL 路径端点不支持该前缀的边界。
...
1. **候选集 = 所有 provider 的裸 `available_ids` 并集 ∪ 所有非 default provider 的 `provider/model` 限定名 ∪ 所有 `model_mappings` 的键**。裸 secondary provider 模型保留在候选计算中，但它通常解析到 default provider 而变成 `absent`；限定名才是无需 mapping 即可到达 secondary provider 的公开名称。`/api/status.routes` 与模型列表使用同一候选生成规则。
2. 去重按 `canonical()`，**保留先出现的拼写**，顺序是「裸目录名（排序后）→ secondary provider 限定目录名（排序后）→ mapping 键（排序后）」。目录名优先，因为那是上游的权威拼写；同一 canonical 名只出现一次。
...
5. `owned_by` 填第 3 步算出来的 provider 名。它与 `provider` 查询参数使用同一事实，不按 provider 类型或配置顺序猜测。
6. 只有该 provider **确实服务**解析后的那个模型时才列出。「确实服务」的权威定义在 §4.2 的 `serviceable`，此处引用而非另行推导（规则评审 S-2：两个端点各判一次，迟早会各自演化出一套）。
7. `provider` 是可选的精确查询参数。提供后只保留 `owned_by` 等于该值的 serviceable 条目；它不改变路由解析、不把裸 secondary ID 临时改写成限定名，也不影响 `format=openai|pi` 的后续投影。未知 provider、空值或没有匹配条目时返回 `{"object":"list","data":[]}`。

被否决的两个替代：维持现状会**少报**；简单取并集而不跑路由会**多报**（只在 A 目录里、却没有任何条目指向 A 的模型，实际走 default 被拒，列出来等于承诺了一个会拒绝服务的模型）。

这个 handler 的 docstring 今天写着「The catalog routing consults」——它本来就该和路由看同一份答案。secondary provider 的限定目录名与 `provider` 筛选都建立在这条共享路由表之上，不另维护一份 provider 目录投影。

`owned_by` 由此第一次有真实含义。旧实现曾把 default provider 名填到每一行，等于一个常量；当前实现填入实际路由选择出的 provider。
...
收 **「所有 mapping 键 ∪ 所有 provider 的裸 available_ids ∪ 所有非 default provider 的限定 available_ids」**。候选生成集合与 §4.1 相同；`/api/status.routes` 保留所有 serviceability 状态用于诊断，而三个模型列表入口只返回 `serviceable == "yes"` 的条目。

去重与拼写取舍**与 §4.1 第 2 条相同**：按 `canonical()` 去重，保留先出现的拼写，顺序是裸目录名、限定目录名、mapping 键。首版只写了「去重，别名优先」，规则评审 F-11 指出那句话既没定义等价类（按字面还是按 `canonical`），在新规则下也没有可观察效果——路由结果只由 §2 决定，与名字来自哪个集合无关。所以「别名优先」这个说法撤销，改为上面这条明确的拼写规则。

收全集的理由：省下的裸目录名恰恰是「我没配它、所以它走 default」这一类——而「走 default」正是 §6.1 那个已知缺口最容易出错的一格。限定目录名则让没有 mapping 的 secondary provider 模型可被客户端发现；两者都不能从候选集中静默消失。
...
- `/models`、`/v1/models`、`/openai/v1/models`（含裸目录、secondary provider 限定目录、mapping 键、去重规则与 `provider` 筛选）、`/api/status`（含 `serviceable` 五个值）、`/health/readiness`：unit
...
本节列的是**本次推导中仍越出了用户原裁决边界的四处**。它们都有明确的理由，实现按它们进行，但用户可以推翻其中任何一条——记在这里是为了让推翻这件事不需要先把整份 Spec 读一遍。secondary provider 的限定目录名已由 `52d57a09` 落地并在 §4.1、§4.2.1 中成为现行行为；mapping 键进入候选集仍是另一条待确认事项。

| # | 条款 | 推导了什么 | 越出在哪 | 若被推翻 |
|---|---|---|---|---|
| 1 | §4.1 | `/v1/models` 的候选集加入所有 mapping 键 | 用户在「路由可达集合」上裁决过，但没见过目录会多出 `fable`、`opus`、`sonnet` 等别名的后果 | 退路是只收「解析结果与自身不同」的那些 mapping 键，代价是自映射式的限定条目又看不见 |
...
| 4 | §3.3 | `provider/` 前缀只在模型名位于请求体的端点上生效 | 用户裁的是「支持请求侧指定」，没被问过作用面。Gemini 与 Azure 把模型名放在 URL 路径段上，写前缀会打断路由匹配 | 若要覆盖那两个面，需要另一种表达（查询参数或请求头） |
