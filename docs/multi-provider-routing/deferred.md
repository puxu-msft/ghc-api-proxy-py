...
**为什么没有顺手补**：这一个孔可以两行代码补上，但它属于「mapping 表的静态卫生」这个更大的题目——同族的还有键含 `@format`（同理，已由 MPR-11 在报告侧对齐但配置侧仍无告警）、键是纯空白。只补空键会留下一组形状相同、告警覆盖不一致的孔。

键含 `/` 不再属于这组：2026-09-08 起，首段精确命中已配置 provider 的完整键可在请求侧前缀之前命中并重定向，例如 `ttthree/model: ttthree-a/model`；裸 `model` 还会在其自身未命中后，以默认 provider 合成 `ttthree/model` 再尝试；带 provider 的请求则在原始名未命中后，以 fallback provider 替换前缀再尝试。见 Spec §3.1。
...
secondary provider 的 `provider/model` 限定目录名已在 `52d57a09` 中落地，并在当前 Spec 中记录为现行行为。仍待确认的四项是：mapping 键进入模型列表候选集；启动 WARN 从一类扩到三类；mapping 值不支持 `@format`（是纠正用户亲笔文档还是实现它）；`provider/model` 前缀只在请求体端点生效。
