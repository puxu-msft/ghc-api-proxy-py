...
2026-09-08 用户已单独裁定：非默认 provider 的 `provider/model` 限定名自动出现在模型目录和状态路由表中。这一项已从待确认范围移出，但 §12 中 mapping 键、启动 WARN、mapping 值 `@format` 和路径参数端点作用面仍未获用户裁定。

不在此重复，读 `spec.md` §12 的表。剩余四条都能被单独推翻，每条都写了推翻之后的退路。
