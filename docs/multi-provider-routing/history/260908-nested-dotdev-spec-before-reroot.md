...
| 2026-09-08 | **§3.3、§4.1、§4.2.1** | 用户裁决：公开模型目录与状态路由表除保留裸模型名外，还自动公开非默认 provider 的 `provider/model` 限定名，使已加载的 ttthree 等 secondary provider 模型可被客户端发现；请求侧限定语法不再只是人工调试手段 | 用户 2026-09-08 直接裁决 |
...
不为这两个面另造一种表达（查询参数、请求头）。限定语法在请求体端点上既服务于手工调试，也服务于模型目录对 secondary provider 的自动发现；为路径参数端点再发明一套写法，等于为同一能力维护两种语法。

**目录行为变化与已知代价**：

1. `/v1/models` 与 `/api/status` 会列出非默认 provider 的 `A/claude-opus-5` 这种形式；它既是请求侧的限定语法，也是客户端自动发现 secondary provider 模型的目录 id。默认 provider 的模型仍保留裸名。
...
1. **候选集 = 所有 provider 的裸 `available_ids` 并集 ∪ 所有非默认 provider 的 `provider/model` 限定 ids ∪ 所有 `model_mappings` 的键**，与 §4.2 的 `routes` 同一个口径。首版让两个端点用不同集合回答「有哪些名字可用」，没有理由。
2. 限定 ids 与裸 ids 一并按 `canonical()` 去重，**保留先出现的拼写**，顺序是「目录名（排序后）→ provider/model 限定名（排序后）→ mapping 键（排序后）」。目录名优先，因为那是上游的权威拼写；`claude-opus-4.5` 同时是目录 id 和 mapping 键时只出现一次。
...
4. **列出的 `id` 是候选名本身**，不是解析后的模型名。裸候选使用上游模型名，非默认 provider 的附加候选使用 `provider/model`；这个端点回答的是「你可以发什么」，而客户端能发的正是候选名。
...
收 **「所有 mapping 键 ∪ 所有 provider 的裸 available_ids ∪ 所有非默认 provider 的 provider/model 限定 ids」**。**用户裁决**，选的是全集而非「只收 mapping 键」或更小的集合。

去重与拼写取舍**与 §4.1 第 2 条相同**：按 `canonical()` 去重，保留先出现的拼写，顺序是目录名先、provider/model 限定名后、mapping 键最后。首版只写了「去重，别名优先」，规则评审 F-11 指出那句话既没定义等价类（按字面还是按 `canonical`），在新规则下也没有可观察效果——路由结果只由 §2 决定，与名字来自哪个集合无关。所以「别名优先」这个说法撤销，改为上面这条明确的拼写规则。
...
- `/v1/models`（含 §4.1 的候选集、provider/model 限定名与去重规则）、`/api/status`（含 `serviceable` 五个值与限定路由）、`/health/readiness`：unit
...
本节列的是**本次推导中仍待用户确认的四处**。它们都有明确的理由，实现按它们进行，但用户可以推翻其中任何一条——记在这里是为了让推翻这件事不需要先把整份 Spec 读一遍。
