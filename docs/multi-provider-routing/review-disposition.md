...
## 2026-09-08 后续记录：自动发现 secondary provider 模型

此前 Spec §3.3 将 `provider/model` 限定名限定为请求侧人工调试语法，导致非默认 provider 已成功加载的模型不会出现在 `/v1/models` 和 `/api/status.routes` 中。2026-09-08 的开发记录将当前行为记为：默认 provider 保留裸模型名，非默认 provider 额外公开 `provider/model` 限定名，使 ttthree 等 provider 的模型可被客户端自动发现。本目录未保留该记录的一手逐字锚，因此这段只能说明当时的记录和实现结论，不能单独证明用户裁决的来源或覆盖范围。

该行为已同步到 Spec §3.3、§4.1、§4.2.1，并由 `52d57a0` 实现；`tests/int/test_pipeline_ops_routes.py` 覆盖目录生成、provider 过滤和 `owned_by`。原 D-3 只关闭了其中关于目录公开限定名的这一项，其他待确认推导仍保留在 `deferred.md` 和 Spec §12。
