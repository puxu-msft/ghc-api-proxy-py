...
- 本文只维护现有 `contrib/systemd/` 单实例 `4141` 模板的部署、安装和运行说明；它不拥有 rolling 产品合同或运行时验收状态。
- [systemd-runtime](../systemd-runtime/plan.md) 保留 S3／S4 已进入 `main` 的 provenance，并以 S5 真实 user-manager／cgroup smoke 为该线唯一未闭合的运行时门。
- [systemd-rolling/spec.md](../systemd-rolling/spec.md) 与 [systemd-rolling/plan.md](../systemd-rolling/plan.md) 是独立 `4144` 动态 generation rolling 的行为合同和实施计划；不要把本文的单实例 restart 叙述合并为 rolling 计划，也不要用 rolling 的 `Type=notify` 合同回写本文的 `Type=exec` 模板。
