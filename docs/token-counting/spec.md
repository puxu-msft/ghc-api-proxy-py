...
本 Spec 约束入站 Anthropic `POST /v1/messages/count_tokens`，以及为该端点服务的 direct Anthropic counter、Anthropic Messages→OpenAI Responses/Command Code local prediction、历史学习、持久化和观测。它不把 OpenAI Responses 的 standalone-field `PromptTokenAdmission` 扩张为 whole-request counter，也不允许用 local estimate 填充 Chat Completions 缺席的 upstream usage。
...
Count请求必须经过与实际inference target相同的model resolution、协议翻译和token-relevant request reshape，生成一次frozen target。Direct Anthropic count以有效的Anthropic request为目标；Anthropic Messages→OpenAI Responses count以最终Responses payload为目标；Command Code count以最终Command Code envelope为目标。Local与所有remote counter legs都读取这同一个frozen target，不继承上一条leg的临时状态，也不因计数目的再次进行model mapping。

Command Code当前没有native count endpoint，因此它的local counter必须分析转换后的Command Code envelope（包括其`params.system`、`params.messages`、`params.tools`及其它token-relevant字段），不得回退到入站Anthropic body。Command Code客户端在发送边界追加的固定控制字段（例如强制`stream=true`）若不携带prompt内容，不进入ordinary-text token term；会改变可见prompt的发送边界（例如配置启用的空system placeholder）必须由同一capability snapshot表达并计入。
...
Existing store只超出当前sample／actual／evaluation／event caps时（caps下调后的首次启动即是如此），read-only inspector照常完成真伪校验而不以这些caps拒绝；writer在migration `BEGIN IMMEDIATE`内按§8.5 victim order与§8.4 revision纪律prune到caps，剪过才以caps自检后commit。除这些caps之外的任何invalidity仍按上文返回typed no-write startup error；prefix checkpoint caps不属于可prune的caps，超出仍是invalid state。
...
Limits为：256 samples per identity、512 samples global、5 actuals per full fingerprint、128 persisted diagnostic evaluation rows per candidate key／`ProfileKey`／identity／epoch、4,096 learning events per identity、32,768 events global。Sample caps按whole-state read定尺寸：startup、reader refresh与每个writer transaction都要读取并validate全部retained rows，成本随retained sample JSON总字节线性增长——2026-09-24在4,469 samples／233.5 MiB的production副本上实测一次full read约22 s（SQL fetch 1.8 s，其余为decode），startup约68 s。256 per identity仍覆盖§9 drift所需32＋128＝160条evidence；512 global让两个活跃identity都保有完整窗口。要再放宽sample caps，须先让读取成本不再与retained总量成正比。默认值由`tests/component/tokenization/test_learning_store.py::test_fresh_schema_manifest_pragmas_path_and_lifecycle`转录。每个durable JSON字段（包括compact prefix digest array）上限为1,048,576 bytes；该上限是应用层资源边界，不是SQLite硬限制。Diagnostic rows为每个candidate key分别保留canonical newest 128 matching samples；一个prefix method的三个variants可各自保留至多128 rows，不能共享method-level 128-row budget。Newest order按`observed_at_us`降序，再按`process_boot_id` BINARY、`request_id` BINARY、`attempt_index` numeric降序打破同timestamp tie。Older matching sample的row可prune，但sample／record／actual保留，因此prequential error仍可按candidate key重建。Profile drift的recent 32＋紧邻reference 128共160 method-champion evidence必须从retained `PredictionRecord` candidate＋sample actual重建，绝不查询evaluation table作为完整分母。Rejection-only events计入event caps；sample prune级联records／anchors及其evaluation rows。

若sample的任一durable JSON字段超过V1的1,048,576-byte单字段上限，store必须在commit前回滚该sample，不得截断prefix chain或写入部分sample。该情况属于policy entry后的`analysis-failed`／`persistence` typed failure，required checkpoint outcome为`NotCommitted`；SQLite可写时必须作为bounded `learning_events` row持久化，sample、prediction record、evaluation、anchor和checkpoint均不得留下。学习服务应将此容量边界作为已分类的best-effort学习失败处理，不影响已完成的前台响应。
...
| A23 | 超过sample／event上限后，在一个transaction结果中断言samples为256 per identity／512 global、actuals为5 per fingerprint、persisted diagnostic evaluations为每candidate key／`ProfileKey`／identity／epoch最近128 matching samples、events为4,096 per identity／32,768 global；同method多个variants各有独立128 budget，rejection-only events计数，pruned sample的anchors／evaluations级联消失，空identity metadata消失。以更大caps写入后以当前caps重启的store在startup时prune到caps并正常启动；旧release按更宽caps写超后，当前reader照常refresh，下一个sample transaction把它剪回caps。 | 分别把evaluation cap错误共享到method、禁用anchor cascade、从event cap排除rejection-only、遗漏global cap或保留empty identity metadata；row-set与reference-integrity断言必须红。Startup仍以caps拒绝over-cap store，或reader／writer开头的read以caps拒绝旧release写超的state时，restart／handover断言必须红。 | Local SQLite test证明bounded relational state，不证明production traffic distribution或filesystem performance。 |
...
| A40 | Independent feature oracle逐shape断言fixed context＋items exact conservation，四种visual suffix transitions均按appended item slice得到literal值；compact 788-item prefix digest与contribution JSON各低于1,048,576 bytes并SQLite／worker pickle round-trip。 | 分别重复／删除nested framing、把item token算入context、恢复verbose prefix objects、用whole visual差分或交换tuple位置；exact totals、suffix value、byte bound或round-trip必须红。 | Synthetic features证明本方decomposition，不证明production visual formula。 |
...
| 2026-09-15 | §3.1、§4、§12～§13 | Command Code不再使用原始Anthropic body作为count fallback；count统一以最终Command Code envelope为frozen target，新增Command Code专用local tokenizer与空system placeholder capability边界。 | 用户直接裁决：“command code 分支也应该是按转换后的 command code 格式做 tokenizer”；本次实现同步更新tokenizer、worker、driver和transcription tests。 |
| 2026-09-16 | §8.5、§8.6、§12～§13 | 明确V1 durable JSON字段超出65,536-byte单字段上限时的原子回滚与typed `analysis-failed`／`persistence` durable event处理；禁止截断prefix、放大上限或保留部分sample，且不影响前台响应。 | 生产日志暴露约979个以上prefix digest导致compact数组超限并被包装为未分类`LearningStoreTransitionError`；本地1,000-item真实codec回归确认该边界及SQLite event持久化。 |
| 2026-09-16 | §8.5、§8.6、§12～§13 | 按用户指示打破原64KiB实现边界，将durable JSON单字段上限提高为1,048,576 bytes；1,000-item prefix chain必须正常持久化，仍保留更大样本的有界 persistence failure。 | 原64KiB是实现者在V1持久化设计中设定的应用层边界，并非SQLite或upstream要求；生产日志证明该边界过窄，阻断了可学习样本。 |
| 2026-09-24 | §8.2、§8.3、§8.5、§12 A23 | Sample caps由4,096 per identity／32,768 global降为256／512，并写明按whole-state read成本定尺寸；只超出sample／actual／evaluation／event caps的state不再是invalid state：reads照常接受，writer在migration、sample与prune transaction中剪回caps并只在剪完后自检；逐行decode可按完整stored values复用上次结果，validation仍覆盖全部rows；A23同步新caps并增加over-cap restart与rolling-handover controls。Prune planner改为一次性计算victim keys（与逐个取最小等价），使bulk prune不再随retained samples立方增长。1 MiB单字段上限（用户指示）不变。 | Production副本实测：4,469 samples／233.5 MiB（其中prefix digest arrays 182 MiB）时一次full read约22 s、startup 67.9 s，每个learning transaction读3～4次；2026-09-22 count_tokens p50 18.8 s／max 50.9 s，4141进程RSS 1.95 GB。State bounds与persistence属R4 delegated scope的implementation decisions；修复后同一副本：startup 3.3 s（首次以新caps启动需剪去3,957个samples，一次性53 s），full read 0.35 s。这是止血，根治（不随retained总量线性增长的读取）另行排期。 |
