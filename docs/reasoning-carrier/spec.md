...
- `degrade`对assistant历史中每个不可移植的thinking块删除**整个块**（包括其可见summary与加密state），保留同轮其它块及顺序；非assistant轮中的thinking不是合法历史reasoning，不以有损名义消解其错误。若因此留下空assistant `content: []`，保留该轮，不发明文本或签名。**损失以整个客户端请求为最小记录单元**：无论删除多少个块、经过多少个attempt，同一请求在`conversion_losses`与安全请求日志／History投影中至多有**一条**`LossCode.REASONING_STATE_NOT_PORTABLE`；detail只描述该请求发生不可移植reasoning丢失，不逐块列路径或签名，也绝不声称`lossless`。严格拒绝的错误仍指出首次触发的字段路径。**用户裁定损失仅走现有日志／History安全投影，不在成功响应头、body或SSE事件里新增信号**；这是对运维显式，不承诺CLI用户界面显示损失。沿用当前跨格式writer对不可移植reasoning块的既有处置，不把内部summary伪装成普通assistant text。`refuse`则在发送前返回`reasoning_carrier_not_unwrapped`，原请求不被改写。不能因有损处置跳过后续的普通tool-pair、trailing-assistant及能力校验。
- 同一`REASONING_STATE_NOT_PORTABLE`也用于其它目标协议的请求writer；请求级记录单元不随目标格式改变。这不是把其它不同`LossCode`合并，也不撤销响应侧独立的有损诊断；该code在同一请求的request侧只出现一次，detail使用不含块路径的同一请求级说明。
...
- 2026-09-24，§7.3：用户纠正“逐块记`reasoning-state-not-portable`”为“以请求为最小单元”。取消上一版为每块写独立Loss及路径的规则；同一请求最多一条该code，跨attempt和direct／translated路径共享这一口径，严格档的单点字段路径不受影响。
- 2026-09-24，§7.3：用户针对有损“显式”的可见面选择`log_only`，即现有结构化损失与安全日志／History投影足够，不加成功响应头或SSE/body信号；历史选项原件见`history/260924-loss-visibility-proposal.md`。
