...
顺带两条同类，同样只报告：`auth_base_url` 至今未在 `docs/.human-controlled/config.example.yaml` 中文档化（早前登记于 [260822-ghc-api-conformance-summary.md](history/260822-ghc-api-conformance-summary.md) 的 D2）；同一文件推荐的 `github_token_file` 值带 `.txt` 后缀，与 `FileTokenProvider` 的默认路径不同名（§3.5 第 2 条行为变更已让 `auth` 侧不再受它影响，但两者仍不一致）。
