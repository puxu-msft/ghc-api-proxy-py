...
| 日期 | 条款 | 变化 | 依据 |
|---|---|---|---|
| 2026-09-06 | §3.4、§5.1、§8、§10.2 | Direct streaming Chat 的合法error frame仍推迟，但pre-`[DONE]` transparent replay必须先于最终partial+naked-close fallback；post-`[DONE]`语义冻结，tail结束只留operator observation并提交成功body。明确三种Chat error carrier、known transient隐藏规则，以及mode-adapted non-stream的429／502 status、nested／flat／malformed body保留方式；新增 `ResponseModeNotSupported` 400／`unsupported_response_mode`，不复用endpoint集合为空的 `CapabilityMissing` | 用户2026-09-05～06裁决；[`../direct-buffered-chat-completions/decisions.md`](../direct-buffered-chat-completions/decisions.md) D-2／D-3／D-10／D-11；direct-passthrough §5.4；两份独立设计评审 |
...
- 上一条描述的是**最终选择 `EMIT_UPSTREAM_ENDING` 的attempt**，不禁止semantic commit前的transparent replay。Direct Chat的terminal-only transaction可只读取error的 `code`／`type` 判定它是否属于用户已裁的已知瞬时集合；replacement成功时旧attempt连同failure event完全不可见，replacement未发生时仍按上一条原样交付。该限定及闭集见 [`../direct-passthrough/spec.md`](../direct-passthrough/spec.md) §5.4。
...
| `ResponseModeNotSupported` | `CLIENT` | 400 | 模型明确支持所选endpoint，但该endpoint capability不支持客户端要求的streaming／non-stream response mode，且pipeline没有合法的反向adapter。携带provider、model、requested mode与available modes；OpenAI `code` 固定为 `unsupported_response_mode`，不得复用 `CapabilityMissing` 并谎称endpoint集合为空 |
...
## 8. `framer is None` 的 Chat 腿：transparent replay 与最终 carrier 分开

Inbound是Chat Completions且客户端请求streaming时，该腿没有framer，仍写不出本代理自己的合法Chat error frame。§7的同语义不变量对**最终代理错误carrier**继续带例外；该例外不再被解释成“整个body阶段不能retry”。

[`../direct-passthrough/spec.md`](../direct-passthrough/spec.md) §5.4把整轮raw SSE建模为terminal-only buffered transaction。HTTP 200 headers与comment keepalive不构成semantic commit；**在 `[DONE]` 尚未出现时**，transport tear、attempt deadline、idle timeout、clean EOF以及用户裁定的已知瞬时流内error必须先按统一ledger尝试transparent replay。成功replacement完全隐藏旧attempt的bytes与side facts。

只有在pre-`[DONE]` replay不适用、被draining拒绝、预算耗尽或replacement未建立时，本节的carrier例外才生效：客户端得到仍保留的最终候选attempt已到达的partial bytes + 连接裸断，没有本代理合成的error frame。Client cancellation没有读者，写不出任何ending。

第一个合法 `[DONE]` 出现后语义state冻结，tail不再改变carrier选择：clean EOF正常提交；transport tear、idle timeout、attempt deadline、client deadline或tail cap停止继续收集并提交已含 `[DONE]` 的body，request按成功结束，同时在operator observation记录tail ending；后续error／data／第二个 `[DONE]` 只作为raw tail保留。Client cancellation／downstream write failure仍因没有可写通道而不提交。

Streaming-upstream→non-stream Chat mode adaptation发生在client headers提交前，因此不受这项streaming carrier限制。Parsed object含top-level `error` 时，最终body按JSON value保留整个object。其余由SSE event名 `error` 或flat `type:"error"` 确认的parsed object，把**整个object**作为 `error` 值包入标准OpenAI envelope；这包含 `event:error` + `{"code":"server_error","message":"…"}` 这类没有 `type:"error"` 的flat payload，原字段不得被writer补写或覆盖。Rate-limit failure预算耗尽时status为429，其余明确stream error与 `unassemblable` 为502；没有真实 `Retry-After` 时不得合成该header。Non-object或不可解析error payload用502 proxy envelope，并把原始text／JSON值放入 `upstream_error` 扩展。输出 `Content-Type` 为JSON，hop-by-hop／长度／编码头由代理重建；原HTTP 200 response的其它语义头仍按直连response header policy过滤后转发，但不得让原status 200覆盖本段的429／502。已知瞬时error与无 `[DONE]` 截断先进入shared attempt loop，不得合成成功completion。

「真正按Chat block边界增量交付」与「为Chat streaming发明proxy error frame」仍是两项未实现能力；本次只实现terminal-only retry，不借此撤销它们的既有推迟。
...
### 10.2 Chat Completions 流式腿缺少合法的 proxy error carrier —— error frame仍推迟，提交前replay已裁定

**用户 2026-08-23 裁决继续有效的部分**：不给Chat streaming腿发明proxy error frame；最终无法恢复时仍以已到达的最终候选attempt bytes + 连接裸断结束。真正按Chat block边界增量交付与新的error carrier仍推迟。

**2026-09-05～06 新裁决限定了这个fallback的触发时点，而没有改写其形状。** 用户要求direct terminal-only buffered Chat在semantic body提交前透明retry，SSE transaction必须见 `[DONE]` 才算成功；`server_error`与明确rate-limit流内error进入既有taxonomy，其余流内error不retry。因此旧句“守卫触发或上游撕裂时客户端拿到已到达字节 + 裸断”只描述replay不适用、被draining拒绝、预算耗尽或replacement未建立后的最终动作，**不得再作为跳过replay的依据**。

Final candidate的partial bytes可以在上述fallback时交付；已被成功replacement取代的attempt不得泄漏一个字节。第一个合法 `[DONE]` 后语义success冻结：tail阶段的transport tear、idle timeout、attempt deadline、client deadline与cap只终止tail收集并留下operator observation，提交已含 `[DONE]` 的body；后续semantic event不再反转carrier。Clean EOF无 `[DONE]`在budget耗尽时必须以失败结束，不能因没有exception而记作成功。

Streaming-upstream→non-stream mode adaptation尚未提交client headers，故其不可重试error使用§8定义的OpenAI JSON carrier，不落入本节裸断例外。Parsed nested error object、flat error与malformed raw payload各自按§8保留，不得由writer用本代理默认type／code覆盖上游已有字段。

**登记去向**：新的Chat proxy error frame仍留在 `.dev/docs/error-envelope/deferred.md`；terminal-only replay的现行合同归 [`../direct-passthrough/spec.md`](../direct-passthrough/spec.md) §5.4，完整设计见 [`../direct-buffered-chat-completions/design.md`](../direct-buffered-chat-completions/design.md)。
