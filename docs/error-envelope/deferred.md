...
**顺带半条已闭合（2026-08-27）**：原问题是 `inference.py` 的注释声称守卫触发时客户端拿到空 body，**与实测不符**；实测拿到的是已到达的上游字节。计划的 J 片（`c4216f7`）与 F 片（`633b404`）都已完成且没有带上它，因此不再以这两个已完成切片作载体；这半条改由 2026-08-27 的 `inference.py` 注释修正闭合。当前注释明确写出 `one_shot_delivery` 会先送出已经到达的上游字节，而客户端没有 error frame；实施与变异复核见 `/home/xp/.claude/jobs/0e3de57b/tmp/fix-inference-accounting.md` 第 4 条及「主会话复核」。这只闭合顺带的注释半条，E-1 主体仍按用户裁决推迟。
