...
- 本规格**不**在收到上述任何 400 后剥离并重试。这与[不在 400 后剥离并重试的历史设计边界](../hosted-web-search/history/2604-tool-use.md)一致；该历史背景不是 current authority 或一手用户逐字裁决，本规格的本条规则不重开。
...
- 路由到 **Anthropic Messages 腿**时：`builtin:server-tool-capability` **拒绝**声明——`web_search` 族抛 `WebSearchNotExecutable`，由 `handle()` 答以下一条规定的那对失败块；`web_fetch` 族与两族混合的声明抛普通 `TranslationRefused`，走 error envelope（见本节末两条）。历史块摊平不变（§10）。

  > **本条 2026-08-30 重写，原文是错的。** 起草时写的是「继续**剥离**声明、清理悬空 `tool_choice`……行为完全不变」，而用户 2026-08-20 裁决「去除 drop 策略，drop 在这里行为远不如 mock_result」时订阅者就已经从剥离改成了抛异常。本节下一条当时跟着改了，本条与 §10 的表格没有跟——于是这份规格里同时写着「剥离」和「不得剥离」，两条都自称规范。给错句加一个警告框而留着原句，与不改等价：读者仍会读到它，并有一半机会照它实现。
...
> **⚠️ 本节表格与 §8.3 首条里的「剥离（不变）」已就地改写，不是加注保留，2026-08-30。** 用户 2026-08-20 裁决「去除 drop 策略，drop 在这里行为远不如 mock_result」时，订阅者就从「剥离声明」改成了「抛异常、由 `handle()` 合成失败结果」；§8.3 当时跟着改了，本节表格与 §13 没有跟，于是这两处继续按一个已不存在的实现描述系统。现在的分工是：`web_search` → 抛 `WebSearchNotExecutable` → 合成块对；`web_fetch` → 抛 `TranslationRefused` → error envelope（§8.3 末条）。**历史块摊平不受影响，那一半始终是「不变」。**

| | Anthropic Messages 腿 | Responses 腿 |
|---|---|---|
| 声明 | 订阅者**拒绝** + INFO 日志（`web_search` 族合成失败块对，`web_fetch` 族与混合走 error envelope；2026-08-30 更正，原文写「剥离……不变」） | 本规格 §3 映射为 `{"type":"web_search"}`；能力门不通过时按 §8.3 **合成失败块对**（2026-08-30 更正，原文写「剥离」） |
...
- 历史设计背景（不作为 current authority）：[历史 tool-use 边界](../hosted-web-search/history/2604-tool-use.md)、[历史 Anthropic compatibility 背景](../hosted-web-search/history/2604-anthropic-compat.md)、[历史 hooks 设计背景](../hosted-web-search/history/2604-hooks-system.md)
