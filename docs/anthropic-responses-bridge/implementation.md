...
- **2026-09-25 Claude Code `gpt-6-sol` per-turn control repair（当前代码切片）**：Claude Code 2.1.282 在 `modelOverrides` 把 `gpt-6-sol` 映射为 Claude Opus 5.5 能力时，实测发送 `per-turn-control-2026-07-01`，并让非空 mid-conversation system message 同时携带 `output_config.effort`；旧 reader 把所有含 `output_config` 的 message 都当成必须空 content 的纯控制项，因而在 upstream 前返回 400 `effort control content must be empty`。Current reader 现接受 canonical beta 与三个已部署别名，content-bearing control 只消费 `output_config` 并保留 system content，空 content 纯控制继续从 Responses input 移除，消息顺序最后一条合法 effort（包括尾部控制）决定当前 generation；content-bearing `output_config={}` 保留 prompt 且不改档。真实 Claude Code wire 已固化为 unit 与公开 `/v1/messages` 回归；translation driver 178 项、相关 integration 21 项、targeted Ruff 与 Pyright 均通过。当前代码与测试尚未提交，未运行完整 suite。
...
- **已删除机制的统一说明（2026-08-19 裁决）**：resident-byte／quota／全局内存预算这一整套机制**已按用户裁决整体删除**（`546852a`），由等待式在途请求上限 `proactive_rate_limiter.max_inflight` 取代（`f5589ec`、`7e9b62d`）。本文此后仍会出现「resident wiring」「完整 quota 尚未完成」「quota／backpressure 仍未验证」一类表述；只有明确绑定日期／阶段的段落才是历史取证范围声明，不是待办。`main@c1de6bf…` 同理只在逐处标明2026-08-08历史身份后保留；任何未限定的current主线均由`git rev-parse HEAD`读取，不能由本段总括覆盖写反的具体单元格。
- **2026-09-03 已完成 effort translation 切片**：用户已裁定并批准 request-level thinking／effort 双向设计；current Spec 与 Acceptance 已加入 `ThinkingEffortIntent`、thinking只决定启用、effort只取自`output_config.effort`、Anthropic缺省`high`、逐消息effort折叠、反向`none→disabled`／`minimal→low`及ultracode实际wire只按`xhigh`处理的合同。R2又触发用户裁定：Responses→Anthropic的target thinking capability只取配置，bundled默认以resolved-model正则转录官方当前表，用户配置覆盖；unknown／always-on disabled／extended-only缺manual budget／budget与`max_tokens`不相容均fail closed。Production Tasks 1～5与whole-branch唯一fix wave已按新合同在保留的effort worktree完成：profile配置／target接线、统一IR／request-lifetime source headers／nested residual、正向与反向effort翻译、profile facts持久化、recorder修复与真实explicit-high cassette均已实现；final review修复已squash为`worktree-effort-translation@ed6addd017f461c15abc494584e727f1badec633`；[final scoped R2](reports/260903-review-effort-translation-final-r2.md)确认R1七项全部`ADDRESSED`、0 blocker／major／minor／nit、`NEW_BREAKAGE: none`，精确该HEAD的full Ruff通过、full Pyright为0 errors／warnings／informations、full pytest为2183 passed／2 skipped且coverage 91.18%；本切片的文档复核已由[R4](reports/260903-review-effort-translation-spec-r4.md)以0 blocker／0 major／0 minor／0 nit关闭，[处置账](review-disposition-effort-translation.md)无open／disputed；用户已审阅并批准current Spec；[实施计划](plan-effort-translation.md)已起草，实施计划已由[R5](reports/260903-review-effort-translation-plan-r5.md)以0 blocker／0 major／0 minor／0 nit关闭，[计划处置账](review-disposition-effort-translation-plan.md)无open／disputed；Task 1已在`worktree-effort-translation@69b6ac6`实现并经任务review收口：thinking profile schema、bundled官方regex defaults、startup compile、Chain／routing／TranslationTarget及send／count接线已落地；初审I1要求的两条全链路分辨性测试已补，scoped re-review为PASS（[R1](reports/260903-review-effort-translation-task1.md)、[R2](reports/260903-review-effort-translation-task1-r2.md)）。Task 2已在`worktree-effort-translation@82afd89`完成统一IR、source-header reader contract与nested residual基础设施，旧production behavior保持可运行；独立review为Spec PASS／Quality PASS，零finding（[报告](reports/260903-review-effort-translation-task2.md)）。Task 3最终到`worktree-effort-translation@d824e4f`：正向production切换、compatibility loss oracle和两条integration静态expected均经两轮fix关闭；完整integration在Task 3基线上通过，两个test-only minor留给final review（[R1](reports/260903-review-effort-translation-task3.md)、[R2](reports/260903-review-effort-translation-task3-r2.md)、[R3](reports/260903-review-effort-translation-task3-r3.md)）。Task 4已在`worktree-effort-translation@618fc54`实现Responses→Anthropic profile rendering与effort映射，合并态完整integration 174项与unit selector 25项通过；独立review判核心wire／终态PASS、0 major，两个loss诊断／组合oracle minor留给final review（[报告](reports/260903-review-effort-translation-task4.md)）。Task 5最终到`worktree-effort-translation@505d62f`：profile selected／rejected non-loss facts在success／refusal双路持久化到JSONL，recorder让token／models／responses共用一个RecordingTransport并拒绝零interaction覆盖，第二次经用户单独授权录得explicit-high PONG cassette；该HEAD全量2175项通过、2项skip、coverage 91.17%，Task review为Spec／Quality PASS、0 major／minor、1 nit（[实施报告](reports/260903-effort-translation-task5-implementation.md)、[review](reports/260903-review-effort-translation-task5.md)）。[Whole-branch final review](reports/260903-review-effort-translation-final.md)在`b67634d..505d62f`发现2 major／4 minor／1 nit：replay丢source beta header、Spec同pattern override文字漂移、Anthropic-compatible loss detail及四个测试／docstring缺口；全部采纳，代码／测试／docstring唯一fix wave已squash为`ed6addd`（[fix报告](reports/260903-effort-translation-final-fix.md)），Spec revision record／正文与Acceptance也已同步recursive deep merge合同。当前状态为“Tasks 1～5 complete，review与full suite clean，main integrated”：reviewed source由`archive/260903-effort-translation@ed6addd`精确保留，净语义已squash为`main@4b7d74f`；两提交的完整tree OID逐字相同，故source上的final full suite适用于main装位结果，不重复运行相同bytes；任何既有reasoning carrier canary或本轮单一PONG cassette都不证明其它model／effort或完整REQ-05行为。
...
| Systemd next integration（历史） | `/home/xp/src/ghc-api-proxy-py-integrate-systemd-next`；`integrate/260807-systemd-next` | `80bc8f252b46c511f428af1d97159a5980ee9dc9` | 旧 HEAD `0a93e7f18f197bf8a2395eaaf20afda446f92d6b` 与 `91f95f7… → 0a93e7f…` 保留 merged review／verify／adaptation provenance；其 Plan patch 已被 current living bytes 超越 | 不回放、不续写、不cherry-pick，不以old Plan postimage解冲突；S5只从执行时以`git rev-parse HEAD`读取的current main执行 |
...
| 文档 | 最新报告 | 当前剩余项 | 下一报告 |
|---|---|---|---|
| [Spec](spec.md) | 历史carrier评审链；effort amendment见本轮报告／处置账 | 活文档；2026-09-03 effort amendment已由Spec R4与code final R2关闭全部finding，旧`FINALIZED@4c9beed…`只描述历史内容身份 | 以current Spec内容为行为oracle；未来合同变化继续先修Spec、再改实现并重新评审 |
...
| [Acceptance](acceptance.md) | 历史空reasoning评审链；effort amendment见本轮报告／处置账 | 活文档；REQ-05A与CAL-04 v2已同步current Spec，effort code final R2为0 findings；旧`FINALIZED_ACCEPTANCE_ORACLE@f99492a…`只描述历史内容身份；完整产品继续`UNVERIFIED` | 继续只转录current Spec并保持可证伪；不得把effort切片或旧verdict外推为完整产品`PASS` |
...
| 本文 | [Implementation post-S3 定向复评](reports/260807-resume-review-implementation-post-s3.md)只绑定历史内容身份；effort终态见[code final R2](reports/260903-review-effort-translation-final-r2.md)与[代码评审处置](review-disposition-effort-translation-code.md) | Effort Tasks 1～5已review、full verify并进入main；已归档tool／reasoning scoped PASS与本轮effort切片都不覆盖完整产品。完整产品`UNVERIFIED`、部署`NO_CUTOVER`、本文`LIVING` | Effort切片无后续实施动作；其它current优先级分别回到其topic live文档，不由这张历史表重排 |
...
1. 每个新候选 HEAD 只有在独立复评 blocker 0、major 0，且该 HEAD 的声明范围测试、smoke、lint 与类型检查 gate 通过后，才可进入该片的回放／归档。Foundations、happy、usage、systemd runtime、bridge semantic／route／block／capability／History／stream、systemd S3／S4、stream request facts、network retry、resident primitive／wiring及token／response／item identity fixes均已完成其2026-08-08阶段收敛并进入当时的`main@c1de6bf…`；其旧source／integration verdict只作历史provenance。任一新切片的声明范围先达到0／0即可按其剩余gate收敛，不等待完整bridge；独立验收缺口与后补边界继续单独登记，不因回放消失。
...
3. Current行为与验收分别由[Spec](spec.md)和[Acceptance](acceptance.md)的当前内容承担；旧`FINALIZED@4c9beed…`、`FINALIZED_ACCEPTANCE_ORACLE@f99492a…`及Architecture `746adc7…`均只作历史内容身份，不是本轮放行状态。2026-09-03 effort amendment按本轮评审处置账收口；已归档tool／reasoning最小纵向canary只证明其scoped路径，完整request-level effort与其余未验证边界不得由旧证据外推。
4. Bridge successor三片已作为`bfc461f… → 86b6cc3… → b91e58a…`进入main，后继capability、History与stream又作为`bd86207… → 38bb06f… → ae84aa9…`进入main；六片均完成main-side gate／archive。旧source、integration与WIP身份均不得再作为current载荷。该历史main定向stream review为0 major，但完整retry、buffer cap 下的背压、真实socket partial-write与完整Acceptance继续登记为未验证，不得从定向PASS外推。
5. Graceful-timeout S3已作为`c53849e…`进入main并归档，user-install S4已作为`e9fb277…`进入main并由`archive/260807-systemd-user-install@e16c2a7…`归档；当时main已前进到`c1de6bf…`。不得再次回放`8cae6c2… → d3fabfa…`、old-base链或source S4 patch。S5必须从current main在隔离manager边界执行，不得把真实应用readiness或仓库gate冒充manager／cgroup运行态证据。
...
| `implementation.md` 的current identity与动作 | Route、semantic、block、capability、History、stream、systemd与Acceptance状态在顶部、处置表、进度表、开发线、收敛、下一步和结尾多点复述，main前进时容易出现弱一致性副本 | 2026-08-08该轮同步了当时的`main@c1de6bf…`、tool／reasoning archives与scoped PASS；2026-09-03 current effort状态改由本文活动行与处置账承担 |
...
| 历史 source refs与各阶段 main commits | Reviewed source identity与main squash identity不同，后续若从旧source重建会制造第二条实现链 | 最新archives保持immutable且只作provenance；`c1de6bf…`只是2026-08-08历史锚点，current主线由`git rev-parse HEAD`读取。已收敛切片不再从冻结candidate、reviewed source或历史integration重建；后继缺口只从current main建立新切片 |
