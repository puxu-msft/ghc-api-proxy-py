...
`Chain`新增`thinking_profiles: CompiledThinkingProfiles = ()`；`build_chain()`调用`compile_thinking_profiles(config.model_translation.to_anthropic_messages.thinking_profiles)`；`translation_target(descriptor, thinking_profiles)`从 routing 已选定的同一个 `ModelDescriptor` snapshot 读取 model id 与 reasoning efforts，再选择最后命中 profile 并保留 pattern。`handle()`与`handle_count_tokens()`都传 route-bound descriptor 与同一`chain.thinking_profiles`，不得在 translation 阶段重新查询 provider catalog。
...
`RequestTrace`和`RequestLine`新增`facts: tuple[dict[str, str], ...]`；`_translation_facts()`只接收`ConversionFact`并输出code/detail；把`absorb_losses()`重命名为`absorb_conversion()`，同时重算losses与facts，并更新`server/routes/inference.py`的全部调用点。生产持久化由`RequestCompletionCoordinator.publish()`形成不可变`FinalizedRequest`，`_freeze_line()`与`FinalizedRequest.request_line()`必须同时携带facts；legacy helper `write_request_record()`仍按`RequestLine`字段写入。Console line不新增字段。
