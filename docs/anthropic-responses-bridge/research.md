...
6. **[已独立核验] server-tool 不能因 bridge 引入而被“顺手恢复”。** 既有 canonical historical source `/home/xp/src/ghc-api-proxy-py/.dev/docs/hosted-web-search/history/2604-tool-use.md:3-12` 仅保留当时 client-executed tools 范围的 historical provenance，不是 current behavior oracle。当前结论由本节已核验的目标现状与正式 Spec 的 `Server-tool no-revive` 条款（`/home/xp/src/ghc-api-proxy-py/.dev/docs/anthropic-responses-bridge/spec.md:148-149`）支撑：基础 bridge 不执行、合成或把 Anthropic 原生 server／typed tools 转成普通 function tool；不为此类拒绝建立专用降级 retry。direct bridge 可以定义“不伪造、显式降级、可观测”的跨协议语义；任何扩大该产品边界的行为都必须先由独立 Spec 明确。证据锚定目标 HEAD `47d9ef101c4b81ac70d805b1da157b34d021d33d`。
