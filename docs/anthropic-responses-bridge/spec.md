...
- **2026-09-25 修订逐消息 effort control 的合法形状与折叠顺序。** Claude Code 2.1.282 把 `gpt-6-sol` 通过 `modelOverrides` 映射为 Claude Opus 5.5 能力后，实测首轮请求发送 `per-turn-control-2026-07-01`，并在一个带非空 text content 的 `role=system` 消息上同时携带 `output_config.effort=high`；切换到 `xhigh` 后，下一轮又在消息尾部追加空 content 的 `output_config.effort=xhigh`。Anthropic current beta schema 同时把 `content` 与 `output_config`列为 system message 字段，并明确 content-bearing message 可携带空或含 effort 的 `output_config`。因此旧条款“非空content必拒绝”与“末尾无后续user的控制本轮不生效”均已被真实客户端 wire 和当前 schema 证伪：translated path 必须接受 canonical beta 及已部署兼容别名，保留 content-bearing system message 的 prompt content，只剥离其已消费的 per-message output config；按消息顺序最后一条带合法 effort 的 system message覆盖顶层档位，包括位于本次 messages 尾部的控制。空 content 的纯控制项仍不进入 Responses prompt。
...
`model_translation.to_anthropic_messages.thinking_profiles` 是 Responses→Anthropic writer 选择 thinking wire shape 的唯一能力来源。键为对 resolved model id 执行 `fullmatch` 的正则，所有命中中**最后一项生效**：bundled defaults先加载，用户新增pattern随后合并，因而用户覆盖不被默认宽pattern抢走；用户以同一pattern重写时按配置通用recursive deep merge逐子字段覆盖，未点名字段继续继承bundled profile，mapping以外的值仍整体替换。未命中不按model name、vendor、budget limits或`adaptive_thinking`邻近字段猜测，只有request-level intent确实需要render thinking时才稳定拒绝。
...
- **Anthropic effort 来源与优先级**：合法顶层 `output_config.effort` 是 `low／medium／high／xhigh／max`，省略时有效值为 Anthropic 默认 `high`。带逐消息 effort beta 的 `role=system` message 按消息顺序生效；对当前生成最后出现的合法 `output_config.effort` 覆盖顶层值，包括位于本次 messages 尾部的控制。Translator 必须读取路径转发策略清空前的源协议 header，仅用于验证 beta；该 header 仍不得转发给 Responses upstream。
- **逐消息控制的合法形状与能力边界**：translated path 接受 canonical `mid-conversation-output-config-2026-07-01`，并兼容已部署别名 `per-turn-control-2026-07-01`、`mid-conversation-effort-2026-08-01`、`per-message-effort-2026-07-01`。承载 `output_config` 的 message 必须是 `role=system`；`output_config` 只允许 `effort`，其值若存在必须是合法档位。空 content／省略 content 的纯 effort 控制项只改变当前有效档位，不进入 Responses prompt；content-bearing system message 同时承载 prompt 与 per-message control，必须移除已消费的 `output_config` 后把原 content 按普通 system message 保留到 Responses input。Content-bearing message 的空 `output_config={}` 合法且不改变档位；缺beta、错误role、未知message sibling、未知output_config sibling、空content且缺少effort或非法effort均在upstream调用前拒绝。Direct Anthropic leg原样转发并由目标model执行source capability gate；translated path只要求Responses target明确发布可对齐effort，不把这一扩展表述为source Claude model官方支持。普通不含 `output_config` 的system message不因本条被误认成控制。
