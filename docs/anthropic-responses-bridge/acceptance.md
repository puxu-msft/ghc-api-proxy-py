...
- **缺陷注入控制**：每次只改一侧，并先证明同入口正样本绿，再确认下列目标断言按预期变红；不建设常驻mutation framework。
  ...
  - **逐消息与header**：不折叠控制、把future-only尾部控制提前生效、忘记从Responses input移除控制消息、或让count使用path-policy清空后的header时，最终档位／input缺席／original payload／send-count loss parity至少一项红；跳过beta检查时，缺header的400与零upstream断言红。
