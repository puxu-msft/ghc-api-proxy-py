...
- **缺陷注入控制**：每次只改一侧，并先证明同入口正样本绿，再确认下列目标断言按预期变红；不建设常驻mutation framework。
  ...
  - **正向capability**：让disabled服从显式effort或在目标无`none`时降成最低档，`none`／零upstream断言红；让enabled候选保留`none`时，目标只发布`none`的零upstream断言红；用catalog原顺序首项／末项代替exact／downward／floor时，相同集合不同排列和静态expected至少一项红；把missing／empty／unrankable当默认`high`时，not-carried与wire缺席断言红。
