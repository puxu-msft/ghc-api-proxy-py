...
> Current authority：本文件当前状态是 `LIVING_ACCEPTANCE_ORACLE`——它与所验收的 [spec.md](spec.md) 同为活文档，随 Spec 条款修订同步更新，候选产品及完整 bridge 为 `UNVERIFIED`。**判据以 [spec.md](spec.md) 的当前内容为准**；2026-09-03 effort amendment 的 current 复核状态只在[评审报告](reports/260903-review-effort-translation-spec.md)与[处置账](review-disposition-effort-translation.md)维护。本文此前钉的旧内容哈希均只属于历史处置记录，历史事实见 [reports/260807-acceptance-review-disposition.md](reports/260807-acceptance-review-disposition.md)。Current Architecture 已与 Spec 的“双格式 carrier”方向一致，但仍是非规范提案，不产生 Acceptance expected。下文出现的旧 hash、`upstream-only`、`READY_FOR_FINAL_REVIEW`、“待复评”或 R2～R7 verdict均被本段current声明覆盖，不是current gate。
...
### REQ-05A request-level thinking 与 effort 双向转换

- **独立 oracle**：Anthropic source 合法 effort 集、缺省 `high`、thinking／effort 独立性与逐消息 beta 语义取自 current Spec「Request-level `ThinkingEffortIntent`」及其绑定的 Anthropic 官方合同；Responses source 合法 effort 集取自项目锁定 OpenAI SDK 生成类型和 resolved model catalog。Expected 必须写为静态 wire 对象，不得调用产品 reader、resolver 或 writer生成。
- **正确样本**：Anthropic→Responses 覆盖省略 thinking／effort→`high`，显式 `low／medium／high／xhigh／max`，`disabled`优先且目标不发布`none`时零upstream拒绝，官方`adaptive`与manual`enabled＋budget_tokens>=1024`、translated-path兼容`auto`／缺budget enabled／正整数低budget／非interleaved超界正整数budget均只决定启用，两个不同`budget_tokens`在同一显式effort下产生同一档位且各记未携带；direct path不接受proxy extension的提前改写，catalog精确命中／向下对齐／最低档近似／缺失档位not-carried，以及enabled目标集合只剩`none`时拒绝。Bridge catalog 缺少 `reasoning_effort` 时，`model_providers.<name>.reasoning_efforts` 的 resolved-model fallback 可提供能力集合；catalog 显式列表（包括空列表）优先于 fallback，二者都缺失仍 not-carried。顶层`medium`加带canonical或兼容beta的逐消息`xhigh`必须发`xhigh`；空content纯控制不进入Responses input，Claude Code 2.1.282的`per-turn-control-2026-07-01` content-bearing system control必须保留其content并只剥离`output_config`，消息尾部最后一条合法控制本轮立即生效；content-bearing `output_config={}`保留content且不改档。缺beta、未知sibling、非法role、空content且缺effort、非法effort均400且零upstream，original request保持原样。Responses→Anthropic覆盖默认正则profile的adaptive-only／adaptive+extended／always-on／extended-only四族：`none`只在`can_disable=true`时写disabled，always-on拒绝；bundled profile对可adaptive族优先写adaptive，extended-only未配置manual budget拒绝，custom profile按modes顺序取首个可渲染项，用户resolved-model override配置合法budget后写`enabled＋budget_tokens`；`manual_budget_tokens<1024`在配置加载时拒绝；运行时profile的`modes`只有enabled且budget缺失、缺`max_tokens`或`budget>=max_tokens`时拒绝，有后续adaptive时则回退。`minimal`先近似为low，其余五档按effort catalog exact／downward／floor／not-carried；reasoning存在但effort缺席仍按profile启用thinking；profile未命中拒绝。默认config逐行等价于Spec官方表，每条regex都有官方列明正样本与相邻未列family负样本，用户新增正则最后fullmatch覆盖bundled匹配；用户对bundled同pattern只写一个子字段时，其余profile字段按recursive deep merge保留。`modes=[enabled,adaptive]`在budget合法时写manual shape，在budget缺失或与request `max_tokens`不相容时跳到adaptive；`modes=[enabled]`则拒绝。Malformed／unknown仍拒绝。两向都覆盖被认领对象的兄弟字段同格式重建与跨格式精确loss；actual必须从公开`/v1/messages`、`/responses`和`/v1/messages/count_tokens`至少各走一条，不能只调用私有converter。Claude Code ultracode只以实际wire的`xhigh`作为正样本，literal`ultracode`为拒绝样本。
- **缺陷注入控制**：每次只改一侧，并先证明同入口正样本绿，再确认下列目标断言按预期变红；不建设常驻mutation framework。
  - **Anthropic reader与优先级**：忽略`output_config.effort`时，`adaptive＋xhigh`因actual为`high`而红；恢复budget阶梯时，同effort不同budget的wire相等断言红；把省略effort解释为不发送时，缺省`high`断言红；删除translated-path compatibility branch时，`auto`／缺budget enabled正样本红；让该branch进入direct path时，原样请求断言红。
  - **正向capability**：让disabled服从显式effort或在目标无`none`时降成最低档，`none`／零upstream断言红；让enabled候选保留`none`时，目标只发布`none`的零upstream断言红；用catalog原顺序首项／末项代替exact／downward／floor时，相同集合不同排列和静态expected至少一项红；把missing／empty／unrankable当默认`high`时，not-carried与wire缺席断言红；跳过 bridge catalog 的 `reasoning_effort` 投影、错误覆盖 catalog 的 fallback、或不把 send／count 共用的 resolved descriptor 接上时，对应 descriptor／出站 body／count prepare 断言红。
  - **逐消息与header**：不折叠控制、把尾部最后一条控制错误延后到未来请求、丢掉content-bearing control的system content、把空content纯控制留进Responses input、只接受canonical beta而拒绝Claude Code的兼容别名、或让count使用path-policy清空后的header时，最终档位／input形状／original payload／send-count loss parity至少一项红；跳过beta检查时，缺header的400与零upstream断言红。
  - **反向mapping与target profile**：把`none`省略、让always-on接受disabled、让extended-only无manual budget时猜一个budget、忽略`budget<max_tokens`、让首个而非最后一个正则命中、让第六条regex误收`sonnet-4-1／haiku-4-1／haiku-4`、立即拒绝不可渲染首mode而不扫描后续adaptive、或跳过profile未命中拒绝时，对应完整Anthropic对象／profile来源／400／零upstream断言红；删除新增pattern用户override时，默认与override产生不同thinking shape的静态expected红；把同pattern部分override错误改成whole-profile replacement时，未点名字段继承断言红；把`minimal`原样发出、把downward写成upward、把floor写成省略、或给missing／empty／unrankable猜默认档时，对应effort与loss断言红；把effort缺席／`null`当成无intent时，profile选择后的thinking断言红；接受unknown或literal`ultracode`时，400与零upstream断言红。
  - **residual与生产接线**：删除任一侧sibling merge时，同格式完整对象与跨格式精确loss断言红；让generic extension再次包含已转换effort时，重复loss断言红；绕过公开route只测private writer时，HTTP观测点的upstream wire／调用次数断言不成立；让translator运行于direct leg时，byte-equivalent passthrough断言红。
  每轮失败必须确认落在目标wire／loss／调用次数断言，而不是fixture、header parser或另一条无关断言先失败。
- **方式与证据边界**：`AUTO-UNIT`、`AUTO-COMPONENT`、`AUTO-HTTP`。Mock upstream证明本代理产生了正确wire和错误边界，不证明真实上游当前接受某档；录制catalog只证明录制时的能力集合，不冒充本轮实况。只有要求升级“真实上游接受”结论时才另跑`LIVE-CANARY`，本条不把它设为普通实现的阻断前置。
- **通过判据**：两向所有发送档位都属于resolved model公布集合；thinking不再提供档位；显式／缺省／逐消息effort按冻结优先级产生唯一wire；Responses→Anthropic的thinking shape只来自最后命中的配置profile，always-on／extended-only／unknown与manual budget边界均有唯一终态；bundled默认逐行转录Spec官方表且用户override真实胜出；近似和未携带均有精确loss；非法输入零upstream；send与count的translated body和loss一致；直连腿保持原样。

### REQ-05B reasoning item 与 signature 请求往返
...
| `CAL-04-GRAMMAR-v2`状态／对象 | 唯一允许的下一 batch／batch 内语义事件 | 必须满足的字段、batch与累积规则 |
|---|---|---|
| stream初始态 | `ping` batch、首个完整content batch、零content terminal batch或`error` batch | HTTP success headers不属于本body grammar，并可在第一次upstream 200时先提交。初始态可接受零次或多次独立`ping` batch，且保持初始态；除`ping`外的首个body batch必须是已完整materialize的content／zero-content terminal／error。首个content batch按`message_start → index=0的完整block envelope`组成同一sink batch；零content成功按`message_start → message_delta → message_stop`组成同一完整terminal batch。`message_start`恰好一次；SSE `event`名等于JSON `type`；`message.content=[]`、`stop_reason=null`。 |
...
| 合法`ping` batch | 保持`stream初始态`或`one_or_more_completed_blocks`状态 | 可在`message_start`之前的初始batch边界出现，也可位于首block或任一后续`content_block_stop`之后、下一block／terminal batch之前；不得插入`message_started_but_no_completed_block`、`zero_content_terminal_batch`、任一open block、terminal batch内部或`message_stop`／`error`之后。 |
...
REQ-05A configuration controls must reject leading/trailing whitespace in fallback model keys and effort values. Observation coverage must include explicit `high` and `none`, plus absent-field controls, for Responses, Chat Completions, Command Code, and Anthropic Messages provider-bound wire shapes.
