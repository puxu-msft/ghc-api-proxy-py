...
- **缺陷注入控制**：每次只改一侧，并先证明同入口正样本绿，再确认下列目标断言按预期变红；不建设常驻mutation framework。
  ...
  - **反向mapping与target profile**：把`none`省略、让always-on接受disabled、让extended-only无manual budget时猜一个budget、忽略`budget<max_tokens`、让首个而非最后一个正则命中、让第六条regex误收`sonnet-4-1／haiku-4-1／haiku-4`、立即拒绝不可渲染首mode而不扫描后续adaptive、或跳过profile未命中拒绝时，对应完整Anthropic对象／profile来源／400／零upstream断言红；删除用户override时，默认与override产生不同thinking shape的静态expected红；把`minimal`原样发出、把downward写成upward、把floor写成省略、或给missing／empty／unrankable猜默认档时，对应effort与loss断言红；把effort缺席／`null`当成无intent时，profile选择后的thinking断言红；接受unknown或literal`ultracode`时，400与零upstream断言红。
