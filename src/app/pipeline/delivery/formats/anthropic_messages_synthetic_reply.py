...
Claude Code issues a web search as its own sub-request: `tools` holds nothing but the search, the user turn reads `Perform a web search for the query: X`, and whatever comes back is rendered into the main conversation under a `Web search results for query:` heading attached unconditionally. A refusal reaches that conversation as `tool_result` with `is_error: true` and the model handles it well — measured. But it reaches it *after three attempts*, in the one case on record, and a search that cannot run is not going to start working on the third.

**Whose attempts they are was recorded wrongly here until 2026-08-30**, and the correction is worth keeping because the old version was checkable and false. The claim was that the client's transport retries an HTTP error. It does not: it retries 408, 409, 401, 5xx and usually 429 — ten *retries* by default, or 300 in its watchdog mode; `CLAUDE_CODE_MAX_RETRIES` overrides the count and is capped at 15 only outside watchdog mode — and a 400 not at all. The three were the main-conversation model calling `WebSearch` again after each `is_error` tool result.

The conclusion survives, and is worth stating at the width the evidence supports. Three sub-requests are three either way. An error string in a `tool_result` demonstrably drew three repeat calls from the model; no client *mechanism* repeats a failed tool result, and the `*_tool_result_error` literals appear nowhere in its bundle. **Whether the model repeats a failed tool result anyway has not been measured** — which is the same limit this module's last paragraph already records for the reply as a whole.

So the reply is synthesised instead, in the shape Anthropic defines for exactly this: `server_tool_use` paired with a `web_search_tool_result` whose content is a single `web_search_tool_result_error` object. That is a 200 carrying a failed tool, not a failed request — the documented contract is that a search error "still returns a 200 (success) response", with `content` as one object rather than a list. No client mechanism repeats it — the qualification two paragraphs up applies here and is not restated as a certainty — and the model is told in its own protocol that the search failed rather than being handed an HTTP error string to interpret.
