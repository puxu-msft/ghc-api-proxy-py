...
class _UpstreamSource:
    ...
    **What that iterator is, is the caller's answer, and in production it is not the raw response.** `inference.py` hands over `response.aiter_bytes()` under three wrappers: two guards that exist to represent an upstream condition and do belong on this side of the line, and `_counted_upstream`, which is bookkeeping and does not. A bug in the counter is therefore tagged here as upstream's — the same misattribution the marker-per-site version made, unchanged by the inversion rather than introduced by it. Closing it means the caller naming the raw source rather than this constructor guessing at it; registered in `deferred.md` §22之六 rather than papered over here.
