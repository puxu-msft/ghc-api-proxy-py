...
class _UpstreamSource:
    ...
    Positive identification, and on the *upstream* side rather than this one. It used to be the other way round: a marker set at each place this side ran code, so that everything unmarked defaulted to upstream's. That list is unbounded by construction — an independent review made this loop's own SSE reader raise and watched the bug get handed to the client as upstream's — while the list on this side of the boundary has exactly one member. Every byte upstream produces enters through this iterator and nothing else does.

    A class rather than a generator, because a generator would put the tagging `try` around a `yield`: closing it delivers `GeneratorExit`, and a consumer that throws in would have its own exception recorded as upstream's. `__anext__` has no such window. `CancelledError` is not an `Exception` and is not tagged, which is what keeps `finish_stream_cleanup` cancelling the in-flight pull from reading as an upstream tear.
    """
...
async def _deliver(
    ...
    *,
    ...
    while True:
        ...
        #
        # **Upstream is what is positively identified**, and it is the only half that can be. Everything upstream produces arrives through `_UpstreamSource` and nothing else does, so `torn is upstream.tear` is exact. The other half — everything this side runs — has no bounded list: assembling, committing, framing, the keep-alive, the SSE reader, the pull scheduler, and whatever is added next. Marking that side is what this used to do, and it was incomplete twice over: framing was left out with a stated reason that did not hold, and after that was fixed an independent review made the SSE reader raise and watched the bug reach the client as upstream's.
        #
