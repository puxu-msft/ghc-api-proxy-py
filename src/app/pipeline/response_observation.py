...
class ResponsesObserver:
    ...
    _terminal_items_sealed: bool = False
    ...
    def _observe_response_mapping(
        ...
        *,
        ...
        if self._terminal_items_sealed and not complete_body:
            return

        output = response.get("output")
        if isinstance(output, Sequence) and not isinstance(output, (str, bytes, bytearray)):
            if complete_body:
                # The terminal output array is the complete, authoritative item set. Earlier added/done drafts may supply observations before the terminal arrives, but they cannot add items to it or fill fields it omitted.
                self._items = {}
            ...
            # Missing, null, or malformed terminal output is unclassified, not an observed empty set. Discard earlier drafts so they cannot impersonate the terminal snapshot.
            self._items = None
