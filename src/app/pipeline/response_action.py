...
class ClientActionBasis(StrEnum):
    ...
    SHELL_ENVIRONMENT_UNRECOGNIZED = "shell_environment_unrecognized"
...
class ClientActionObservation:
    ...
    # The delivery projection is explicit so the observation remains self-contained; UNKNOWN projects to True rather than being flattened into a factual client action.
...
def classify_responses_client_action(item: Mapping[str, Any]) -> ClientActionObservation:
    ...
    Unknown future types and unrecognised conditional discriminators stay unknown to observation. Delivery projects every unknown conservatively toward release so a new client action cannot be held behind a taxonomy this proxy has not learned yet.
    """
    ...
    if item_type == "shell_call":
        ...
        if not isinstance(environment, Mapping):
            return ClientActionObservation(
                requirement=ClientActionRequirement.UNKNOWN,
                basis=ClientActionBasis.SHELL_ENVIRONMENT_UNRECOGNIZED,
                delivery_required=True,
            )
        environment_mapping = cast(Mapping[str, Any], environment)
        environment_type = str(environment_mapping.get("type", ""))
        ...
        if "container" in environment_type:
            return ClientActionObservation(
                requirement=ClientActionRequirement.NOT_REQUIRED,
