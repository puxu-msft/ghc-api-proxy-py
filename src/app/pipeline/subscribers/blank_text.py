...
async def drop_blank_text_blocks(context: RequestContext) -> None:
    ...
    for index, message in enumerate(messages):
        ...
        if entry.get("role") == "assistant":
            # An assistant turn that said nothing is allowed to say nothing: `content: []` is accepted here, measured on 2026-08-20 both mid-conversation and as the final turn (`.dev/exp/260820-empty-text-probe/`, F6 and F4, 200 each), while the blank block it replaces is refused (F3, 400). The two spellings mean the same and only one of them travels, so this is the rewrite the `system` branch gets for the same reason.
        ...
        # This used to travel unchanged, on the grounds that dropping a turn puts two same-role turns next to each other and that had not been measured. It has since: `.dev/exp/260820-tool-pair-probe/` G4 sends two assistant turns in a row and gets 200. The same reasoning now runs the other way — a turn saying nothing is worth less than the request it was costing.
        doomed.append(index)
