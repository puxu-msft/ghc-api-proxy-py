...
A `web_search` call reaches a client through two different routes and has to look the same on both. On the Anthropic leg `builtin:server-tool-capability` flattens the blocks a past turn left in the history, because that endpoint refuses them. On the Responses leg the upstream executes the search itself and reports it as a `web_search_call` item, which has no Anthropic spelling, so it is rendered here too.
