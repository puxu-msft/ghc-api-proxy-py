...
def format_count_provider(provider: str, reason: str = "", *, color: bool = False) -> str:
    """`provider(upstream)` / `provider(upstream-failed,local)` — which counting leg produced the number on this line, and what was tried before it.

    Named for the thing `inbound.anthropic_count_tokens.providers` configures, because that is what it reports: `upstream` is upstream's own measurement and `local` is this proxy's calibrated estimate, and they are the two values of `CountTokensProvider`. **Neither is a `model_providers` key**, and the value said `ghc` until 2026-08-27 precisely because that distinction cost nothing while only one provider could be configured — see spec §1.3 of `.dev/docs/multi-provider-routing/`. The word `count` was the first spelling and became redundant once the format prefix started carrying `-count-tokens`, where it belongs — that says what kind of request this was, which is not what this field is for.

    The parenthetical is the trail, in the order it happened: what was tried and did not answer, then the leg that did. `provider(upstream-failed,local)` is an upstream that was asked and could not answer; `provider(no-counter,local)` is a route with no upstream counter, which estimates every time and is working as configured; a bare `provider(local)` is an operator who configured the estimate rather than asking. Those three were one word until 2026-08-20, and two of them are incidents — this line's own defect one level up, where the failure was never absent, it was wearing the ordinary case's clothes.
