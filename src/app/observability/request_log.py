...
def format_count_provider(provider: str, reason: str = "", *, color: bool = False) -> str:
    """`provider(ghc)` / `provider(ghc-failed,local)` — which counting leg produced the number on this line, and what was tried before it.

    The routed model provider's name means upstream's own measurement, and `local` means this proxy's calibrated estimate. The word `count` was the first spelling and became redundant once the format prefix started carrying `-count-tokens`, where it belongs — that says what kind of request this was, which is not what this field is for.

    The parenthetical is the trail, in the order it happened: what was tried and did not answer, then the leg that did. `provider(ghc-failed,local)` is a routed upstream that could not answer; `provider(no-counter,local)` is a route with no upstream counter, which estimates every time; a bare `provider(local)` is a local answer with no upstream attempt. Those three were one word until 2026-08-20, and two of them are incidents — this line's own defect one level up, where the failure was never absent, it was wearing the ordinary case's clothes.
