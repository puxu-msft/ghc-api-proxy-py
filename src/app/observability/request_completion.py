...
class FinalizedRequest:
    ...
    def request_line(self) -> RequestLine:
        ...
        return RequestLine(
            ...
            reasoning_effort=_string(raw, "reasoning_effort") or None,
            status_code=_optional_int(raw, "status_code"),
            started_at=_string(raw, "started_at"),
            duration_s=_optional_number(raw, "duration_s"),
            last_retry_duration_s=_optional_number(raw, "last_retry_duration_s"),
            ...
            terminal_status=_string(raw, "terminal_status"),
            client_actions=_client_actions(raw),
            client_action_classification_complete=_bool(
                raw,
    ...
    frozen = freeze_json(
        {
            ...
            "last_retry_duration_s": line.last_retry_duration_s,
            ...
            "client_actions": [
                {
                    "requirement": action.requirement.value,
                    "type": action.type,
                    "name": action.name,
                    "output_index": action.output_index,
                }
                for action in line.client_actions
            ],
            "client_action_classification_complete": line.client_action_classification_complete,
            "blocks": line.blocks,
...
def _client_actions(mapping: dict[str, Any]) -> tuple[ClientAction, ...]:
    actions: list[ClientAction] = []
    for value in _array(mapping, "client_actions"):
        if not isinstance(value, dict):
            continue
        raw = cast(dict[str, Any], value)
        actions.append(
            ClientAction(
                requirement=ClientActionRequirement(_string(raw, "requirement")),
                type=_string(raw, "type"),
                name=_string(raw, "name"),
                output_index=_integer(raw, "output_index"),
            )
        )
