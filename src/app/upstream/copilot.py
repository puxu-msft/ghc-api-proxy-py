...
The actual Copilot API access lives in the standalone `app.model_provider.ghc` library.
...
from app.model_provider.ghc.auth.providers import GitHubTokenManager
from app.upstream.ghc_settings import ghc_config_from_settings


class GitHubTokenSourceAdapter:
    """Adapts `GitHubTokenManager` to the `GitHubTokenSource` protocol of `app.model_provider.ghc`.
