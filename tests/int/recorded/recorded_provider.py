...
This is the stand-in tests ask for when they want the real thing without the network: a genuine `GithubCopilotProvider`, over the real SDKs, over a transport that answers from a cassette. Nothing about the provider or the client is faked, so everything below the recording — token exchange, catalog parsing, SSE framing, block assembly — runs exactly as it does in production.

The distinction matters. A hand-written stand-in encodes what we *believe* upstream does, and it was that belief which hid a defect: real Copilot sends a different `item.id` on `output_item.added` and `output_item.done`, no fake ever did, and streaming on the primary path returned zero bytes.
"""
...
def recorded_provider(name: str, http_client: httpx2.AsyncClient) -> GithubCopilotProvider:
    ...
    Built the way `composition.build_copilot_provider` builds it, minus the credential: the token exchange is itself in the cassette, so the manager runs its real code path against a recorded answer rather than being replaced.
    """
...
def pinned_config(base_url: str = BASE_URL) -> ProxyConfig:
    ...
    No `model_mappings`: whatever the operator has configured locally must not decide which model a cassette was recorded against, or the recording and the replay ask for different things.
    """
