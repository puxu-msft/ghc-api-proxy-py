...
API, or the proxy never started and the client fell back somewhere, every later test could still pass while measuring nothing. These two check the plumbing itself.
"""
...
def test_the_client_reaches_the_model_through_the_proxy(config_dir: Path) -> None:
    ...
    `MARKER` is unguessable rather than plausible on purpose. A test asserting the reply merely looks like an answer would pass just as well if the request had gone to the real API.
    """
...
def test_the_proxy_and_not_the_client_decides_what_upstream_receives(config_dir: Path) -> None:
    ...
    This is the assertion that separates a real end-to-end from a client talking to a fake: it inspects the body the *proxy* sent, which only exists if the proxy was in the path at all.
    """
