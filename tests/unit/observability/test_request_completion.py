...
def test_finalized_request_is_one_immutable_source_for_store_json_and_console(
    ...
    trace.client_action_classification_complete = True
    trace.facts = (
        {
            "code": "thinking-profile-selected",
            "detail": "resolved_model=gpt-model; pattern=gpt-model",
        },
    )
    ...
    assert record["client_action_classification_complete"] is True
    assert record["facts"] == [
        {
            "code": "thinking-profile-selected",
            "detail": "resolved_model=gpt-model; pattern=gpt-model",
    ...
    assert line.client_actions == (
        ClientAction(ClientActionRequirement.REQUIRED, "function_call", "Bash", 0),
    )
    assert line.client_action_classification_complete is True
    assert line.facts == trace.facts
