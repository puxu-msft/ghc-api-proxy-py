...
from app.config.schema import AutoModeClassifierConfig, ProxyConfig
...
def config(**overrides: Any) -> AutoModeClassifierConfig:
    return AutoModeClassifierConfig(**overrides)
...
class TestRecognition:
    ...
    def test_the_predicates_are_configurable(self) -> None:
        """Why they are settings: a client-side rewording must be fixable without a release here."""
        request = classifier_request(system_text="Nouvelle formulation du moniteur.")
        assert classify(request, config(decision="allow", transcript_open="")) is None
        reconfigured = config(
            decision="allow",
            system_prompt_prefix="Nouvelle formulation du moniteur.",
            transcript_open="",
        )
        assert classify(request, reconfigured) is not None
...
class TestTheClientCanParseTheAnswer:
    ...
    def test_a_block_protocol_decision_round_trips(self, decision: str, expected_block: bool) -> None:
        verdict = classify(classifier_request(), config(decision=decision))
        assert verdict is not None
        assert parses_as_block(verdict_text(verdict, config().reason)) is expected_block

    @pytest.mark.parametrize(
        ("decision", "above_boundary"), [("allow", False), ("block", True)]
    )
    def test_a_severity_decision_lands_on_the_intended_side(
        self, decision: str, above_boundary: bool
    ) -> None:
        ...
        assert verdict is not None
        score = parses_as_severity(verdict_text(verdict, config().reason))
    ...
    def test_an_allow_carries_no_reason(self) -> None:
        ...
        verdict = classify(classifier_request(), config(decision="allow"))
        assert verdict is not None
        assert "<reason>" not in verdict_text(verdict, config().reason)
...
def chain_with(decision: str, *, endpoint: ModelEndpoint = ModelEndpoint.ANTHROPIC_MESSAGES) -> Any:
    config = ProxyConfig.model_validate(
        {
            "default_model_provider": "ghc",
            "model_providers": {"ghc": {"type": "github_copilot"}},
            "inbound": {"auto_mode_classifier": {"decision": decision}},
        }
    )
