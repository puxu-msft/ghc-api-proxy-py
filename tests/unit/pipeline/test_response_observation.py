...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"type": "shell_call"},
            ClientActionRequirement.UNKNOWN,
            ClientActionBasis.SHELL_ENVIRONMENT_UNRECOGNIZED,
            True,
        ),
        (
            {"type": "shell_call", "environment": {"type": "future"}},
            ClientActionRequirement.UNKNOWN,
            ClientActionBasis.SHELL_ENVIRONMENT_UNRECOGNIZED,
            True,
        ),
...
def test_terminal_item_cannot_be_overwritten_by_a_late_partial_event() -> None:
    ...
    observer.observe_event(
        SseEvent(
            ...
            data=orjson.dumps({
                "output_index": 1,
                "item": {"type": "function_call", "name": "late-new-index"},
            }).decode(),
        )
    )
...
def test_terminal_output_replaces_the_stream_draft_set() -> None:
    observer = ResponsesObserver()
    observer.observe_event(
        SseEvent(
            event="response.output_item.done",
            data=orjson.dumps({
                "output_index": 3,
                "item": {"type": "function_call", "name": "stale"},
            }).decode(),
        )
    )
    observer.observe_event(
    ...
    observer.observe_event(
        SseEvent(
            ...
            data=orjson.dumps({
                "output_index": 4,
                "item": {"type": "function_call", "name": "late"},
            }).decode(),
        )
    )

    assert observer.snapshot().output_items == ()


def test_non_object_terminal_output_item_remains_an_unknown_position() -> None:
    observer = ResponsesObserver()
    observer.observe_response({"status": "completed", "output": [None]})

    observed = observer.snapshot()

    assert observed.output_items is not None
    assert len(observed.output_items) == 1
    assert observed.output_items[0].output_index == 0
    assert observed.output_items[0].client_action.requirement is ClientActionRequirement.UNKNOWN
    assert "output_item_not_object" in [issue.code for issue in observed.issues]


@pytest.mark.parametrize("terminal_response", [{}, {"output": None}, {"output": {}}])
def test_unclassified_terminal_output_stays_sealed_against_late_items(
    ...
    observer.observe_event(
        SseEvent(
            ...
            data=orjson.dumps({
                "output_index": 0,
                "item": {"type": "function_call", "name": "before"},
            }).decode(),
        )
    )
    observer.observe_event(
        SseEvent(
            event="response.completed",
            data=orjson.dumps({"response": terminal_response}).decode(),
        )
    )
    observer.observe_event(
        SseEvent(
    ...
    assert observer.snapshot().output_items is None


def test_non_object_terminal_response_discards_and_seals_item_drafts() -> None:
    observer = ResponsesObserver()
    observer.observe_event(
        SseEvent(
            event="response.output_item.done",
    ...
    observer.observe_event(
        SseEvent(event="response.completed", data='{"response":[]}')
    )
    observer.observe_event(
        SseEvent(
            event="response.output_item.done",
    ...
    observed = observer.snapshot()
    assert observed.output_items is None
    assert "terminal_response_not_object" in [issue.code for issue in observed.issues]
...
def test_terminal_missing_execution_does_not_inherit_an_earlier_fact() -> None:
    observer = ResponsesObserver()
    observer.observe_event(
        SseEvent(
            event="response.output_item.done",
    ...
    items = observer.snapshot().output_items
    assert items is not None
    assert items[0].execution is None
    assert items[0].client_action.requirement is ClientActionRequirement.UNKNOWN
    assert items[0].client_action.basis is ClientActionBasis.EXECUTION_UNRECOGNIZED
    assert items[0].client_action.delivery_required is True
...
def test_terminal_item_presence_and_action_are_derived_from_the_terminal_snapshot(
