...
- Development documents live in `.dev/docs/<topic>/` — living documents at the topic root, report originals under `<topic>/reports/`. New reports use an actual `YYMMDD-` prefix and are temporary evidence, not a truth source; file them under the owning topic, or under `.dev/docs/tmp/` when the topic is unclear. `docs/tmp/` and `docs/agents/` no longer exist — do not recreate them. Distil conclusions into living documents and archive point-in-time evidence by topic.
