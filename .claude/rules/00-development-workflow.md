...
- Development documents live in `.dev/docs/`, one directory per topic. A topic's living documents (`spec.md`, `status.md`, `plan.md`, `deferred.md`, …) sit at the topic root; its report originals sit under `<topic>/reports/`; finished work is rewritten into `<topic>/archive-<subtopic>/` with the originals kept verbatim in that archive's own `reports/`. `.dev/` is a separate repository, ignored by this one — commit there freely and never push. The main repository's `docs/` now holds only `docs/.human-controlled/`, which the user writes.
- Subagent investigations, reviews, and PoCs exchange full reports through repository files. New reports use a `YYMMDD-` prefix and are never overwritten: file one under the owning topic's `.dev/docs/<topic>/reports/`, or under `.dev/docs/tmp/` when the topic is not yet clear. **Do not create `docs/tmp/` or `docs/agents/` again** — both were emptied into `.dev/docs/` on 2026-08-21 (`.dev/docs/docs-tmp-migration/`). Distil current conclusions into living documents promptly; do not let a report become the only source of truth.
