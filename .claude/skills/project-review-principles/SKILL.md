...
⚠️ docstring 那一类是 2026-08-22 补进定义的：本条的实例集里一直有两处 docstring（下面的 driver docstring 与 `blocks.py:20`），而原定义只写了「运维填的值」和「读到的那一行」，盖不住它们。补定义而非删实例，因为二者的缺陷同形——`blocks.py:20` 那句 `which is retryable` 断言的是**重试策略**，那是需求层的承诺，不是这个异常类说了算的。按 user 级 Axis 2：docstring 不因为是注释就属中间层，**判它作的是哪一类主张**。

判据轴是**可行动性**，不是「说不说为什么」——后者太钝，会把正当的解释一起判死。`TranslationRefused` 把 `code` 与 `field_path` 送进客户端 400 是刻意的（`src/app/pipeline/translation_driver/semantic.py:51-57`），因为客户端要据此改自己的请求；`composition.py:105` 告诉运维他填的值此刻不生效，同理。这些都说了「为什么」，都正当。不正当的是把**实现的推导**写上去：某段代码为何在这一层返回、某次测量得到了什么。
...
同时它建议的**拆分方向仍然被采纳**，因为通用那半确实值得在项目之外复用：通用形态已写入 `~/.claude/skills/organizing-project-docs`「Axis 2 — claim role」。**本条保留，不缩写**——它承载的是通用判据在本仓的具体落法：本仓的两端具体是哪几个面（运维填的配置值、客户端收到的错误体）、怎么用命令查、命令的已知精度与漏检。**复查时若发现两边说的不一样，以 user 级那份为准并回来改这里**；仅仅「听着像同一件事」不是修改理由。
...
**已退役一条**：`a-failure-path-must-not-produce-the-ordinary-value`，随 `a469dba`（2026-08-20）删除，同一提交把 `a-setting-says-what-it-bounds-not-what-follows` 改写成了现在的 `explanation-...`。本节此前一直写着「六条」并保留着讲那条的整段，是 `a469dba` 没同步本节留下的残句——2026-08-22 由一次评审发现。**退役时正文不留残句这条纪律，第一次执行就漏了**，且漏的地方正是「当前状态」，即最容易被下一个人当现状读的那一节。
...
**合规现状**：`declined-and-adopted-...`、`a-broken-test-...`、`explanation-...` 三条按此办，具名到文件与行处都带日期。**`one-reply-fact-...` 与 `assertions-about-copilot-wire-...` 立于本节之前，其中的行号是导航用途、未带时点，不作对账依据**——已实测腐坏两处：`one-reply-fact-...` 写的 `pipeline_app.py:313/:316`，当天就已移到 `:417/:420`；`explanation-...` 写的 `composition.py:98/:217`，2026-08-22 已重指为 `:105/:239`。下次动到那两条时顺手补齐，不必为此单开一次改动。
