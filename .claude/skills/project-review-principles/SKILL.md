...
上位形态由用户 2026-08-20 归纳：信息有层次，中间层丰富，**两端则可能是精炼的、克制的**。本条只取其中可复查的一半——`docs/.human-controlled/` 那一端我不得修改，也无从事后 diff（该目录未被 git 追踪），复查跑到那里没有动作可做，因此**本条的检索只覆盖实现面这一端**。通用形态已落在 user 级 `organizing-project-docs` 的「Axis 2 — claim role」一节（需求层／中间层／产物层，含两个方向的误置）。**本条不复述它，只做本仓这一端的落地与检索**；两边允许重叠，见文末「归属」。
...
**该 blocker 已由用户裁决解除**：项目级与用户级允许交叉重复，两个家本身不是缺陷，**只有矛盾才必须解决或澄清**（同日已写入门槛 2）。
...
若 `docs/.human-controlled/` 不再作为独立契约面（本条的两端之一因此消失），连同实证移进 `.dev/docs/<topic>/archive-*/`。**「通用判据已经迁出到 user 级」不再是退役条件**——2026-08-22 裁决允许两级重叠，本条的存在理由是那半通用判据在本仓的落法与检索命令，那是 user 级那份没有也不该有的。
