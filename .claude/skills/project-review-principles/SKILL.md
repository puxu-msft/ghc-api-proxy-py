...
**两条命令都实测过，缺陷也一并写在这里。** 命令一精度尚可：2026-08-20 跑出 5 处（**2026-08-22 复跑为 8 处，多出的 3 处未逐处判定**——记在这里是因为它是这组命令唯一一条有两个时点的召回数据：两天涨了 60%，说明这类字符串在活跃增长，首次复查应当从命令一开始），三处是下面列的真违背，另两处（`ghc_client/errors.py:101` 转述上游自己的拒绝、`cli.py:436` 告诉运维要改什么）读一眼即排除——正是「命令给候选、不给结论」。**命令二精度差**：`-B20` 窗口太宽，会把与端点无关的注释里的 `because` 一并抓出（实测多数命中属此类），它只能当**人读的定位器**，不能当过滤器。两条都会漏：一按词表、二按行窗口。实证——归属评审只把结局词表换了个键名，就打出命令没抓到的 `src/app/pipeline/delivery/blocks.py:20`（`"""A delivery-side failure. Distinct from an upstream one, which is retryable."""`，2026-08-16 提交 `172cbe9`，早于本条的成因四天）。**首次复查时按人读补一遍，并记下命令该抓而没抓到的位置。**
