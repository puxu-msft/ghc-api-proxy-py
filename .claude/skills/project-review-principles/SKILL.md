...
**本质是信息有层次，而两端要克制**（用户 2026-08-20 的归纳）。判据是：**端点承诺，中间层解释。**

两端是运维**写下的那个值**（`docs/.human-controlled/config.example.yaml`）与运维或客户端**读到的那一行**（日志行、错误消息、SSE 帧）。两者都是承诺面——读它的人正据此决定填什么、怎么处理——所以每一句都要兑现，必须薄。中间是一条由外向内越来越厚的带：代码与注释、`docs/` 的结论、`docs/tmp/` 的报告、`.dev/exp/` 的探针。它不承诺任何东西，只负责让人看懂，所以可以厚。

**在这个仓库里代码属于中间带，不是端点**——长注释承担「为什么这样写」是立过的家法。这条原则不收紧注释的厚度，它只管一件事：解释不要写到承诺面上。

推论，也是这条原则的动力：**越往中间越厚，也越易腐**。探针几天作废、报告几周、`docs/` 的结论几个月，而两端以年计。薄的东西才守得住那么久，所以把解释挪进中间带不是降级，是把它放到能随代码一起更新的地方。

失效的形态是**在承诺面上写解释**：「它同时也管 body」在作者眼里是解释，在运维眼里是承诺。
...
- **另一个端点**：日志行、错误消息、SSE 帧里出现「为什么」而不只是「发生了什么」。读它的人要据此决定下一步，不是来听推导的。（2026-08-20 这一侧是干净的：`attempt exceeded its deadline`、`no response headers within 1s` 都只报事实——列在这里是为了让检查有个反例可比，不是因为它发作过。）
...
1. **`pipeline_app.py:78` 与 `:114`（2026-08-20 实测，行号取于当日）**：两层同形。里层 `_extra_info` 有**三条**路径都返回 `None`——`network_stream is None`、`get_extra_info` 查无此键、吞掉异常；外层 `_snapshot_upstream_connection` 的异常兜底返回 `{"local": "", "peer": "", "alpn": "", "stream_id": None}`，与成功路径 `:106` 逐键同形，而成功路径在里层全返回 `None` 时也产出同一个全空字典。落进 JSONL 之后，「这个传输不暴露这些」「读的时候炸了」「确实是空」三件事同形，且无告警。**这条是本条目立条时由命令 A 抓出来的，当天即修（`c57a15d`）。** 修法落在阶梯的第二档「分形」而不是第三档「加字段」：只记观测到的键，缺的那些不填占位、改在 `unavailable` 里说明原因（`no-transport-identity` / `socket-unreadable` / `snapshot-failed: …`），空字典保留它原有的含义——从未快照。

**促成判定的是消费者而不是代码**：`.dev/docs/upstream/h2-goaway/` 明写「四行的 `local` 相同 ⇒ 同一条连接」，而空串填充让每一条无身份的行都彼此相等，于是那条规则会**从缺席里读出一个正面结论**。**复查时先找消费者**——同一段代码有没有问题，取决于有没有人按某条规则去读它。

实测支撑（2026-08-20）：真实记录 2527 行里 152 行是这个形状（全部 H1，其余 2311 行 H2 皆有真值）；变异验证——把「声明缺失」那几行删掉，两条测试变红，且退化后的空字典与 `_Trace` 的默认值相撞，测试也抓到了。
