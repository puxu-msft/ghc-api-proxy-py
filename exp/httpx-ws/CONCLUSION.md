...
运行 `uv run python .dev/exp/httpx-ws/poc.py` 通过全部断言。正式实现采用 httpx-ws；上游连接复用项目 HTTPX client，连接生命周期由 async context manager 持有，客户端桥接逐消息发送，不创建无管理 fire-and-forget task。