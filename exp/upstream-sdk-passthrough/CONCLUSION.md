...
命令：`uv run python .dev/exp/upstream-sdk-passthrough/poc_passthrough.py`（先在后台起 `uv run python .dev/exp/upstream-sdk-passthrough/mock_server.py`）。
...
- `.dev/exp/upstream-sdk-passthrough/mock_server.py` —— 本地 mock SSE 上游（FastAPI + uvicorn）。
- `.dev/exp/upstream-sdk-passthrough/poc_passthrough.py` —— 实测脚本（含全部判据断言）。
- 运行方式：先 `uv run python .dev/exp/upstream-sdk-passthrough/mock_server.py &`，再 `uv run python .dev/exp/upstream-sdk-passthrough/poc_passthrough.py`。
