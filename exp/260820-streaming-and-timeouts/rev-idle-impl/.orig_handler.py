...
async def handle_count_tokens(chain: Chain, context: RequestContext) -> dict[str, Any]:
    ...
    # Whether upstream has a counter at all is decided here, from the route, rather than discovered by calling one that refuses. `tokenization.md` makes token counting a per-protocol wire contract: `POST /v1/messages/count_tokens` serves the Anthropic protocol, and the OpenAI family has no count endpoint of its own — it reports usage only on a completed response. Handing `None` here is what the chain already understands as "this counter cannot serve this request", so it hands over to the next one instead of failing the request. That leaves `ghc:unconfigured` in the attempts trail, which is true of the call but not of the operator's config file — `count_tokens_upstream_skipped` carries the reason, and is the one to read.
    #
    # Discovering it by calling was not merely wasteful, it was fatal: `provider.count_tokens` gates on the Messages capability and raises `EndpointNotSupported`, which is a `ProviderError`, which `count_tokens()` deliberately propagates rather than degrading. The reasoning there — that a refusal means the caller can never reach this model — does not hold for this one: the caller reaches the model perfectly well through `/responses`. What is missing is the counter, not the model.
    upstream_counts = route.target_format is WireFormat.ANTHROPIC_MESSAGES
    if not upstream_counts:
        context.extras["count_tokens_upstream_skipped"] = (
            f"{route.target_format.value} has no upstream count endpoint"
        )
