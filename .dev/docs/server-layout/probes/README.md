...
⚠️ **本文所有绝对值都是时点读数，会随活树增长而漂移，对不上不等于坏了。** 实测：`app.cli` 在归档时是 151，十二小时后同伴加了一个模块就变成 152。**耐用的是对照，不是数字**——`import app.server` 只拉进 1 个模块（它自己）、`app.cli` 的可达集里 `app.routes` 为 0，这两条只要布局不倒退就一直成立，而 83／106／151 每天都可能不同。**引用本文时引对照，不要引数字**；数字只在「与它当天的对照一起读」时有意义。
