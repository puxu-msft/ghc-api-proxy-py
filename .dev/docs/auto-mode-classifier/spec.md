...
> 状态：现行契约。实现已落地并经三轮独立评审（两轮特性、一轮收尾），随实现同步更新。证据基础是两份取证报告，见本文末「证据出处」。
...
**接入点为什么是直接短路，而不是订阅者抛异常。** 最初按 `hosted_web_search` 的先例设计：一个 `attempt.prepare` 订阅者识别后 raise，由 `handle()` 捕获并合成。放弃了，因为那条先例的形状是被它自己的需要逼出来的——它必须读 `resolved_model` 与 `target_format`，那些只有在路由与翻译之后才有。本特性两样都不需要：判据读的是 Anthropic 形状的 `system` 与 `messages`，而它们在翻译后就不存在了。走订阅者等于先把 body 翻译成另一种格式，再在那里判断一个只有翻译前才看得见的特征，并且为一个从不发生的上游尝试付出 attempt 的全部开销。所以短路直接写在 `handle()` 里、`shape_request()` 之后、翻译之前。
...
次要的一条理由：M2 的值必须精确到尾部那个 `\n`，写成 `"<transcript>"` 就永远匹配不上，而且失效无声——只表现为命中数为零。**但这条腿的鉴别力有限**，评审指出得对：`match_system_prompt_prefix` 配错了同样是无声漏判。它能说明的只是「一个几乎不需要改的键额外还容易配错」，不足以单独支撑钉死的决定；真正起作用的是上面那条易变性判断。

**钉死的代价**（评审指出，作者原先没写）：M2 因此**失去了单独关闭的能力**。M1 设成空字符串就能关掉那一条，M2 不行——若日后 `<transcript>` 包裹被别的客户端合法使用而造成误伤，只能改代码。今天没有这样的客户端，所以不改；记在 `deferred.md` D6。
...
- 两份取证报告原为 `.dev/docs/tmp/`，topic 建立后归档到 `reports/`。**报告原件内部仍写着 `docs/tmp/` 的旧路径，是有意保留的**——那是作者当时实际读到的位置，改写会伪造点时记录。从报告里点那些路径会断，当前位置以本节为准。
