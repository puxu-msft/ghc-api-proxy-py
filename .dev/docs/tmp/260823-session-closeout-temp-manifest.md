...
**处置已关闭**（2026-08-23）：零删除，5 个探针与 1 份删除授权书逐字持久化，其余原地弃置留给 harness 过期；差集判据两向为空。
...
**关闭时实测**：**75 个文件，组 1–9 合计 75，两个方向的差集都为空 ✓**（2026-08-23，`.dev` = `34a126b` 之后）。
...
**这条判据在本次就抓到了东西，也在本次证明了自己不添乱**：首次枚举 66 → 关闭前重列 71 → 74 → 关闭时 75。

- 中间那 5 个提交信息文件按分类规则**自动落进组 6，未触发任何复审**；
- `28c1a7a-intended.txt`／`28c1a7a-actual.txt` 两个**是差集抓出来的**，回到分类另立组 9。若用总数判据，这 2 行会被一次「把 72 改成 74」抹平；
- 最后落地本轮提交时产生的 `dotdev-msg6.txt` 同样按规则自动落位，**差集仍为空，评审结论不作废**。

**这就是「总数相等」与「差集为空」的实际区别**：前者每写一个文件就要求人重新确认一次，而重新确认最省事的方式恰好是改数字；后者只在**出现没人分类过的东西**时才叫停。
