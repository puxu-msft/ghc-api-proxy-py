...
>
> **状态**：2026-07-16 实施完成。Phase 0～8 全部落地并通过独立评审与最终黑盒验收；Phase 9 是条件性 BACKLOG，不属于强制实施范围。
>
> **2026-07-17 订正**：本文是历史计划。其中代理侧历史截断和原生 server-tool 相关阶段已被 [Hooks/Tokenization 规格](../hooks-tokenization-spec.md) 取代并从代码删除；不得按旧章节恢复。
...
> **状态（2026-07-16）**：按设计保留。默认单层 History、FTS-free 搜索和 OTel/外部 TSDB 方案已满足强制范围；冷归档、内容寻址去重和进程内分层遥测仅在明确需求与基准数据出现时启动。
